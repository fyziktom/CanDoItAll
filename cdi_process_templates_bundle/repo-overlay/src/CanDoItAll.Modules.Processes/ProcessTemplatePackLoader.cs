...
public sealed class ProcessTemplatePackLoader
{
    ...
    private ProcessTemplatePack LoadCore()
    {
        ...
        var sharedRoles = LoadJsonDirectory(Path.Combine(root, "shared", "roles"), static item => item.Key);
        var sharedArtifacts = LoadJsonDirectory(Path.Combine(root, "shared", "artifacts"), static item => item.Key);
        var sharedChecklists = LoadJsonDirectory(Path.Combine(root, "shared", "checklists"), static item => item.Key);
        var sharedValidations = LoadJsonDirectory(Path.Combine(root, "shared", "validations"), static item => item.Key);
        var sharedPrompts = LoadJsonDirectory(Path.Combine(root, "shared", "prompts"), static item => item.Key);
        ...
        foreach (var entry in manifest.Processes.OrderBy(item => item.Key, StringComparer.OrdinalIgnoreCase))
        {
            ...
            definition.LocalRoles = LoadJsonDirectory(Path.Combine(processRoot, "roles"), static item => item.Key).Values.OrderBy(item => item.Key, StringComparer.OrdinalIgnoreCase).ToList();
            definition.LocalArtifacts = LoadJsonDirectory(Path.Combine(processRoot, "artifacts"), static item => item.Key).Values.OrderBy(item => item.Key, StringComparer.OrdinalIgnoreCase).ToList();
            definition.LocalChecklists = LoadJsonDirectory(Path.Combine(processRoot, "checklists"), static item => item.Key).Values.OrderBy(item => item.Key, StringComparer.OrdinalIgnoreCase).ToList();
            definition.LocalValidations = LoadJsonDirectory(Path.Combine(processRoot, "validations"), static item => item.Key).Values.OrderBy(item => item.Key, StringComparer.OrdinalIgnoreCase).ToList();
            definition.LocalPrompts = LoadJsonDirectory(Path.Combine(processRoot, "prompts"), static item => item.Key).Values.OrderBy(item => item.Key, StringComparer.OrdinalIgnoreCase).ToList();
