...
For runtime command writeback steps that explicitly allow `ExecuteExternalAction`, create or reuse a `Run command` parent node under the current process run node. Add explicit child nodes named `Run app` and `Run tests` with command, working directory, source evidence, applicability, and cleanup notes. If the target has no runnable app command, still create `Run app` with the not-applicable reason instead of omitting the node. For resolve or manifest-only runtime command steps, do not call project-structure mutation tools; write planned node payloads into the required managed artifact and leave node creation to the writeback step.
