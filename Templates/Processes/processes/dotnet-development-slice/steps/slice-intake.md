...
`DotNetProductBaselineContract` is a runtime-owned, bounded, read-only launch observation for this .NET process. A `discovered` contract with `discoveryComplete: true` is authoritative current-run evidence that a baseline exists, so select `existing`. Copy the listed relative topology directly only when `topologySampleComplete: true`. When that flag is false, the counts describe the bounded discovery while the path arrays are samples; perform bounded read-only topology discovery before architecture and do not treat the samples as the whole solution. Trust the sampled target-framework and test-project metadata only when `metadataInspectionComplete: true`; otherwise reread the affected project files before making those decisions. A `not-found` contract proves no .NET baseline only when `discoveryComplete: true`. A `partial`, `unavailable`, duplicate-name, incomplete-sample, or incomplete-metadata contract is a concrete topology warning: perform only the bounded read-only clarification allowed by this step and do not silently invent a new layout over it.
