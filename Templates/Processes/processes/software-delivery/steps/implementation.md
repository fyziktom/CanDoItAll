...
Pass the architecture decision record and acceptance-driven validation plan to the child as durable inputs. Preserve literal `ProductAcceptanceCriteriaContract` ids when present. Only criteria with `kind=ProductAcceptance` and `required=true` gate the child implementation or accepted handoff; a child may refine the implementation approach, but it must not narrow one of those required behaviors or treat planned proof as executed acceptance evidence. Preserve `kind=DeliveryPlanning` items as nonblocking context, but do not require product proof or human confirmation for them unless a separate typed decision gate explicitly requests that decision.
