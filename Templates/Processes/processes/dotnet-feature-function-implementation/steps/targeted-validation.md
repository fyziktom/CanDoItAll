...
Read at least one representative production source file and the mapped test file in this validation execution. Verify that the test invokes an owning production component, service, or domain/application type. A test-only helper that copies the intended algorithm or state transition is fabricated proof even when it passes; choose `feature-repair-required`, cite both files, and describe the missing production boundary. Because this step does not own browser tools, missing browser receipts alone are not a repair defect. Judge whether the implementation and focused tests contain the contracted behavior, then leave live interaction proof to the parent QA/runtime steps.

If `code-change` completed with branch `implementation-attempt-incomplete`, do not send the same implementation strategy back for another blind retry. Read the runtime gate findings and failed helper receipts, record the concrete mutation blocker in this validation artifact, and choose `feature-repair-required`. The repair specialist owns correcting or replacing the failed implementation strategy.
...
When `ProductAcceptanceCriteriaContract` is present, add an `Acceptance-criteria coverage` table with every literal criterion id, the proof owner, current-run proof ref when this step owns it, and one of `Focused pass`, `Parent runtime proof required`, or `Fail`. Browser-only, screenshot-only, or other parent-owned runtime proof is not a failed focused implementation criterion. Record it as `Parent runtime proof required` and carry the exact criterion id and needed proof to parent QA.
...
- `feature-accepted` means the bounded implementation is ready for parent QA, not that the overall product is released. Select it when current-execution build/test receipts and owning-source readback prove every behavior owned by this focused step. Preserve browser-only or parent-runtime proof as an explicit parent follow-up instead of fabricating a failure or a pass here.
- Do not accept an interaction, state transition, calculation, persistence, reload, timing, or recovery criterion from a source-text or markup-content assertion. A green test that only finds expected words, selectors, or file content proves content presence, not product behavior. Inspect the mapped test and representative production source; choose `feature-repair-required` when the accepted behavior is unwired, unexecuted, replaced by fabricated test-only state, or deferred to a later slice.
- Select `feature-repair-required` when build/test receipts are missing after this step attempted them, proof fails, the completed code-change artifact records failed proof, implementation is incomplete, or evidence reveals an owning source/test defect. Do not use this branch solely because a parent-owned runtime proof has not yet run.
