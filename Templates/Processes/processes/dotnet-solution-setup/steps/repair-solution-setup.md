...
- If `DotNetAddTestProjectExecutionPlan` is present, prefer that deterministic helper plan for solution membership and project-reference repair instead of inventing a new helper. Write `DotNetAddTestProjectScript` verbatim to `DotNetAddTestProjectScriptRef`, verify that `.ps1` ref, invoke it with `workspace_pwsh_run_script` and `DotNetAddTestProjectSideEffectManifest`, then read back the solution and test project files.
