...
Review the design and validation plan before implementation. Ask explicitly: is logic properly split from Blazor/components/controllers; are models and DTOs well defined and complete for the user stories; do services expose the functions needed for required ProductAcceptance behavior; are functions testable without full UI/runtime; does every `kind=ProductAcceptance` and `required=true` criterion or explicit product invariant have an executable proof plan at the correct production boundary; are `kind=DeliveryPlanning` items preserved separately as nonblocking; are persistence, integration, security, and deployment boundaries clear; is runtime command and screenshot applicability known; and are risks or trade-offs recorded. DeliveryPlanning items cannot cause a no-go, design block, escalation, or human reconfirmation unless a separate typed decision gate explicitly requests that decision. Do not implement code or mutate product files.
