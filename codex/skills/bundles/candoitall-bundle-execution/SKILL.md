...
8. For every critical subbundle, complete the Semantic Adequacy Gate before accepting local tests as proof:
...
9. Validate with the proof defined in the bundle:
...
10. Record browser-validation analytics and the subbundle gate result while validation is happening.
11. Run `candoitall-subbundle-validator` as the closure gate before moving to the next subbundle.
12. Update the bundle after each completed or reopened subbundle:
...
13. Move to the next subbundle only after the current one is proven.
14. Before declaring the bundle complete, run `candoitall-bundle-validator` and `scripts/validate_bundle.py --stage completed`; completed-stage validation includes the structural checks and proof-depth checks for semantic adequacy evidence.
...
Critical subbundles must include a semantic proof block in the execution report or a linked proof artifact before closure. The block must cover raw-note ownership, the shallow-pass trap, adversarial negative proof, semantic positive proof, and anti-stub audit. See `references/semantic-adequacy-proof.md`.

If any required semantic proof element is absent, fixture-specific, or only verifies scaffolding, the subbundle remains `In progress` or `Blocked`; it must not be marked `Completed`.
