...
10. Write reusable implementation and QA prompts under `shared-prompts`.
11. Pre-create browser-validation logging instructions for each subbundle and seed the execution report with browser analytics and subbundle gate sections.
12. Complete traceability so every requirement points to at least one concrete bundle file and one owning subbundle.
13. Run `scripts/validate_bundle.py --stage prepared` before declaring the bundle ready.
14. Run `candoitall-bundle-validator` as the readiness gate and repair the bundle until it passes.
15. Finish the self-review from QA, architect, and manager perspectives. Do not mark the bundle ready while any of those three reviews is incomplete or inconclusive.
