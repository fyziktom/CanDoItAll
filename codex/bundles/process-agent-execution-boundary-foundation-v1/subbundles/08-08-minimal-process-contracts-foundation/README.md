# SB08: Minimal process contracts foundation
...
Introduce minimal neutral process contracts/abstractions only where they support future core extraction.
...
Previous subbundle SB07 closure gate passed.
...
1. Create minimal `CanDoItAll.Processes.Contracts` or `CanDoItAll.Processes.Abstractions` if justified by SB03.
2. Add only stable identity/source/policy snapshot records needed by the execution boundary.
3. Add architecture tests forbidding EF, Razor, MAF, Workbench, and product module references.
4. Do not move EF entities or UI view models.
5. Update solution file and README.
...
- `proof/SB08/source-assertions/*.txt`
...
Implement SB08 from `process-agent-execution-boundary-foundation-v1`. Follow the exact source references, do not broaden scope, and do not proceed to the next subbundle until the progression gate passes.
