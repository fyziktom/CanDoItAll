...
const out = path.join(root, '.mcp-state/overview01d/browser-web-r2');
...
(async () => {
 ...
 const chatHost = page.getByTestId('floating-agent-chat-host');
 assert((await chatHost.innerText()).includes('Agents \u00b7 Overview'));
 assert(!(await page.locator('body').innerText()).includes('Agents \u00c2\u00b7'));
 await page.getByTestId('agents-shell-tabs').getByRole('button', { name: /^Governance/ }).click();
 await until(async () => (await chatHost.innerText()).includes('Agents \u00b7 Governance'), 'exact Governance context name');
 await page.screenshot({ path: path.join(out, 'GovernanceContextName.jpeg'), type: 'jpeg', quality: 80 });
 fs.writeFileSync(path.join(out, 'GovernanceContextName-dom.txt'), await page.locator('body').innerText());
 result.scenarios.push({mode:'GovernanceContextName',overviewDisplay:'Agents \u00b7 Overview',governanceDisplay:'Agents \u00b7 Governance'});

 await setMode('Normal');
 await page.goto(origin + '/agents');
 await ready();
 const providerHistory = new URL(page.url());
 providerHistory.searchParams.set('tab', 'providers');
 const overviewHistory = new URL(page.url());
 overviewHistory.searchParams.set('tab', 'overview');
 await page.evaluate(({ previous, current }) => {
  history.pushState(null, '', previous);
  history.pushState(null, '', current);
 }, { previous: providerHistory.href, current: overviewHistory.href });
 ...
 const confirmation = page.getByTestId('agents-feed-defaults-confirmation-content');
 const confirmationNode = await confirmation.elementHandle();
 await page.goto(page.url().split('#')[0] + '#delivery-check');
 assert(await confirmation.isVisible());
 assert(await confirmation.evaluate((node, prior) => node === prior, confirmationNode));
 await page.goBack();
 await page.goBack();
 await until(async () => new URL(page.url()).searchParams.get('tab') === 'providers', 'same-page Back to Providers');
 assert(await confirmation.isVisible());
 assert(await confirmation.evaluate((node, prior) => node === prior, confirmationNode));
 await page.goForward();
 await until(async () => new URL(page.url()).searchParams.get('tab') === 'overview', 'Forward to Overview');
 await ready();
 await capture('DefaultsSurvivesQueryAndFragment');
 ...
 await page.getByTestId('agents-team-edit').click();
 await page.getByTestId('agents-team-details-dialog-content').waitFor();
 await page.getByTestId('agents-team-choose-icon').click();
 await page.getByTestId('material-icon-picker-dialog-shell').waitFor();
 await page.screenshot({path:path.join(out,'TeamOwnedIconPicker.jpeg'),type:'jpeg',quality:80});
 await page.goBack();
 await page.getByTestId('material-icon-picker-dialog-shell').waitFor({state:'hidden'});
 await page.getByTestId('agents-team-details-dialog-content').waitFor({state:'hidden'});
 await ready();
 await capture('TeamOwnedIconPickerCanceled');
 ...
 await page.evaluate(({ previous, current }) => {
  history.pushState(null, '', previous);
  history.pushState(null, '', current);
 }, { previous: origin + '/', current: page.url() });
 await page.getByTestId('agents-shell-feed-defaults').click();
 await page.getByTestId('agents-feed-defaults-confirmation-content').waitFor();
 await page.goBack();
 await page.getByTestId('agents-feed-defaults-confirmation-content').waitFor({ state: 'hidden' });
 await page.getByTestId('agents-shell-tabs').waitFor({ state: 'hidden' });
 assert.equal(new URL(page.url()).pathname, '/');
 await page.screenshot({path:path.join(out,'ActualPageDeparture.jpeg'),type:'jpeg',quality:80});
 result.scenarios.push({mode:'ActualPageDeparture',ownedDialogClosed:true});
