...
const out = path.join(root, '.mcp-state/overview01d/browser-web');
...
(async () => {
 ...
 await page.getByTestId('agents-shell-tabs').getByRole('button', { name: 'Governance', exact: true }).click();
 ...
 await page.goto(origin + '/agents');
 await ready();
 await page.getByTestId('agents-shell-tabs').getByRole('button', { name: 'Providers', exact: true }).click();
