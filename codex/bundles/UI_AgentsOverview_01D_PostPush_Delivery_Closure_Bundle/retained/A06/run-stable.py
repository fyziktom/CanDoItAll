...
out=Path('.mcp-state/overview01d/stable');ns={'t':'http://microsoft.com/schemas/VisualStudio/TeamTest/2010'}
...
def scrub(s):
 ...
 return s
assert Path('codex/bundles/UI_AgentsOverview_01D_PostPush_Delivery_Closure_Bundle/retained/A06/stable-prerequisites/commands.json').exists(), 'Fresh prerequisites are required.'
...
assert len(names) == 10199, f'Expected frozen discovery 10199, found {len(names)}'
(out/'stable-discovery.txt').write_text(scrub(raw),encoding='utf-8')
frozen={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'reason':'A03 shared DialogService cancellation registration correction changes lifecycle semantics for every dialog consumer; see A06 stable-invalidation.json.',
