# SB041 — Core API snapshot refresh

## Status
Prepared.

## Objective
Refresh public API snapshot only if legitimate pure rules changed.
...
Implement SB041 in `process-runtime-live-e2e-openai-hardening-v1`. Keep the process runtime source-backed, avoid transient bundle paths, preserve generic Process Core, and capture artifact-backed proof before moving on.
