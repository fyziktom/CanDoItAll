# SB015 — Gate E lifecycle closure

## Status
Prepared.

## Objective
End-to-end deterministic run closes or reaches expected blocked state with explainable diagnostics.
...
- `proof/SB015/manifest.md` and `proof/SB015/semantic-invariants.md` with shallow-pass trap, adversarial negative proof, semantic positive proof, and changed-file hashes.
...
Implement SB015 in `process-runtime-live-e2e-openai-hardening-v1`. Keep the process runtime source-backed, avoid transient bundle paths, preserve generic Process Core, and capture artifact-backed proof before moving on.
