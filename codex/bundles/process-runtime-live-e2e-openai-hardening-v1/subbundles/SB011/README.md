# SB011 — Project-structure node start path proof

## Status
Completed.

## Objective
Exercise `/api/project-structure/projects/{projectId}/nodes/{nodeId}/process/start` and bridge context.
...
## Proof Required
- Source assertion transcript: `bundle://proof/SB011/transcripts/project-structure-process-start-source-assertions.txt`.
- Test transcript: `bundle://proof/SB011/transcripts/project-structure-process-start-integration.txt`.
- Test result: `bundle://proof/SB011/test-results/SB011-project-structure-process-start.trx`.
- Anti-stub scan: `bundle://proof/SB011/transcripts/anti-stub-and-runtime-host-drift-scan.txt`.
...
- Large desktop Playwright proof required if this subbundle changes or validates a browser-visible process flow.
- Not required for SB011; the subbundle objective targets the HTTP API endpoint and bridge context. The integration proof validates the returned project process workspace route.
...
Implement SB011 in `process-runtime-live-e2e-openai-hardening-v1`. Keep the process runtime source-backed, avoid transient bundle paths, preserve generic Process Core, and capture artifact-backed proof before moving on.
