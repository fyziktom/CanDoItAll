# SB007 - Focused route test selection
...
- Updated execution report row for SB007.

## Dependency Impact

- Downstream subbundles depend on SB007 preserving behavior and not widening route dependencies.
- If SB007 changes route order, route result semantics, claim behavior, finalizer behavior, or side-effect ownership, downstream proof is invalid and the affected earlier phase must be reopened.

## Validation Depth

- Focused validation for Focused route test selection..
...
- [ ] Execution report contains a row for SB007.
...
- May continue only after local checks for SB007 pass and the execution report has an individual SB007 row.
- Stop and reopen prerequisites if source scans, tests, or critical-gate proof contradict earlier assumptions.

## Suggested Agent Prompt

Implement SB007 from this bundle. Keep changes narrow, preserve existing process route behavior, and do not introduce Process Core or driver APIs. Record exact proof before proceeding.
