# SB080 - Gate P: start/workflow/direct proof
...
Gate P: start/workflow/direct proof.

This subbundle belongs to **P7: Start/workflow/direct-agent service split**.
...
- Updated execution report row for SB080.

## Dependency Impact

- Downstream subbundles depend on SB080 preserving behavior and not widening route dependencies.
- If SB080 changes route order, route result semantics, claim behavior, finalizer behavior, or side-effect ownership, downstream proof is invalid and the affected earlier phase must be reopened.

## Validation Depth

- Focused validation for Gate P: start/workflow/direct proof..
- Cumulative behavior proof is consolidated at the next critical gate for P7: Start/workflow/direct-agent service split.
...
- [ ] Execution report contains a row for SB080.
...
- Critical manifest and semantic-invariants file for SB080.
...
- May continue only after local checks for SB080 pass and the execution report has an individual SB080 row.
- Stop and reopen prerequisites if source scans, tests, or critical-gate proof contradict earlier assumptions.

## Suggested Agent Prompt

Implement SB080 from this bundle. Keep changes narrow, preserve existing process route behavior, and do not introduce Process Core or driver APIs. Record exact proof before proceeding.

## Closure Proof

- Manifest: `bundle://proof/SB080/manifest.md`
- Semantic invariants: `bundle://proof/SB080/semantic-invariants.md`
