# SB008 - Gate B: baseline proof and reopen triggers
...
Gate B: baseline proof and reopen triggers.

This subbundle belongs to **P1: Baseline and guardrails**.
...
- Updated execution report row for SB008.

## Dependency Impact

- Downstream subbundles depend on SB008 preserving behavior and not widening route dependencies.
- If SB008 changes route order, route result semantics, claim behavior, finalizer behavior, or side-effect ownership, downstream proof is invalid and the affected earlier phase must be reopened.

## Validation Depth

- Focused validation for Gate B: baseline proof and reopen triggers..
...
- [ ] Execution report contains a row for SB008.
...
- Critical manifest and semantic-invariants file for SB008.
...
- May continue only after local checks for SB008 pass and the execution report has an individual SB008 row.
- Stop and reopen prerequisites if source scans, tests, or critical-gate proof contradict earlier assumptions.

## Suggested Agent Prompt

Implement SB008 from this bundle. Keep changes narrow, preserve existing process route behavior, and do not introduce Process Core or driver APIs. Record exact proof before proceeding.
