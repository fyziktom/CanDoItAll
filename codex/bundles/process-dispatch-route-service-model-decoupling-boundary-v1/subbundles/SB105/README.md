# SB105 - Replace route factory all-service input with route facet set
...
Replace route factory all-service input with route facet set.
...
- Updated execution report row for SB105.

## Dependency Impact

- Downstream subbundles depend on SB105 preserving behavior and not widening route dependencies.
- If SB105 changes route order, route result semantics, claim behavior, finalizer behavior, or side-effect ownership, downstream proof is invalid and the affected earlier phase must be reopened.

## Validation Depth

- Focused validation for Replace route factory all-service input with route facet set..
...
- [ ] Execution report contains a row for SB105.
...
- May continue only after local checks for SB105 pass and the execution report has an individual SB105 row.
- Stop and reopen prerequisites if source scans, tests, or critical-gate proof contradict earlier assumptions.

## Suggested Agent Prompt

Implement SB105 from this bundle. Keep changes narrow, preserve existing process route behavior, and do not introduce Process Core or driver APIs. Record exact proof before proceeding.
