# SB032 - Gate H: context/test closure
...
- Updated execution report row for SB032.

## Dependency Impact

- Downstream subbundles depend on SB032 preserving behavior and not widening route dependencies.
- If SB032 changes route order, route result semantics, claim behavior, finalizer behavior, or side-effect ownership, downstream proof is invalid and the affected earlier phase must be reopened.

## Validation Depth

- Focused validation for Gate H: context/test closure..
...
- [ ] Execution report contains a row for SB032.
...
- Critical manifest and semantic-invariants file for SB032.
...
- May continue only after local checks for SB032 pass and the execution report has an individual SB032 row.
- Stop and reopen prerequisites if source scans, tests, or critical-gate proof contradict earlier assumptions.

## Suggested Agent Prompt

Implement SB032 from this bundle. Keep changes narrow, preserve existing process route behavior, and do not introduce Process Core or driver APIs. Record exact proof before proceeding.
