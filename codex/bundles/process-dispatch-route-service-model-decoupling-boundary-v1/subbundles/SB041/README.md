# SB041 - Split database requirement route service
...
Split database requirement route service.
...
- Updated execution report row for SB041.

## Dependency Impact

- Downstream subbundles depend on SB041 preserving behavior and not widening route dependencies.
- If SB041 changes route order, route result semantics, claim behavior, finalizer behavior, or side-effect ownership, downstream proof is invalid and the affected earlier phase must be reopened.

## Validation Depth

- Focused validation for Split database requirement route service..
...
- [ ] Execution report contains a row for SB041.
...
- May continue only after local checks for SB041 pass and the execution report has an individual SB041 row.
- Stop and reopen prerequisites if source scans, tests, or critical-gate proof contradict earlier assumptions.

## Suggested Agent Prompt

Implement SB041 from this bundle. Keep changes narrow, preserve existing process route behavior, and do not introduce Process Core or driver APIs. Record exact proof before proceeding.
