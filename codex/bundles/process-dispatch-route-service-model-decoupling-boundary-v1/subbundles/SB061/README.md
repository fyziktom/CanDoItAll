# SB061 - Subprocess artifact projection adapter review
...
Subprocess artifact projection adapter review.
...
- Updated execution report row for SB061.

## Dependency Impact

- Downstream subbundles depend on SB061 preserving behavior and not widening route dependencies.
- If SB061 changes route order, route result semantics, claim behavior, finalizer behavior, or side-effect ownership, downstream proof is invalid and the affected earlier phase must be reopened.

## Validation Depth

- Focused validation for Subprocess artifact projection adapter review..
...
- [ ] Execution report contains a row for SB061.
...
- May continue only after local checks for SB061 pass and the execution report has an individual SB061 row.
- Stop and reopen prerequisites if source scans, tests, or critical-gate proof contradict earlier assumptions.

## Suggested Agent Prompt

Implement SB061 from this bundle. Keep changes narrow, preserve existing process route behavior, and do not introduce Process Core or driver APIs. Record exact proof before proceeding.
