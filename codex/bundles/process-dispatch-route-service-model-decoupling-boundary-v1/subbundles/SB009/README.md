...
- Downstream subbundles depend on SB009 preserving behavior and not widening route dependencies.
- If SB009 changes route order, route result semantics, claim behavior, finalizer behavior, or side-effect ownership, downstream proof is invalid and the affected earlier phase must be reopened.

## Validation Depth

- Focused validation for Introduce route-owned candidate/run/step snapshots..
...
- May continue only after local checks for SB009 pass and the execution report has an individual SB009 row.
