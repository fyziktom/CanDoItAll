# SB024 - Gate F: context/result decoupling
...
Gate F: context/result decoupling.
...
- Downstream subbundles depend on SB024 preserving behavior and not widening route dependencies.
- If SB024 changes route order, route result semantics, claim behavior, finalizer behavior, or side-effect ownership, downstream proof is invalid and the affected earlier phase must be reopened.

## Validation Depth

- Focused validation for Gate F: context/result decoupling..
...
- [ ] Execution report contains a row for SB024.
...
- Critical manifest and semantic-invariants file for SB024.
...
- May continue only after local checks for SB024 pass and the execution report has an individual SB024 row.
- Stop and reopen prerequisites if source scans, tests, or critical-gate proof contradict earlier assumptions.

## Suggested Agent Prompt

Implement SB024 from this bundle. Keep changes narrow, preserve existing process route behavior, and do not introduce Process Core or driver APIs. Record exact proof before proceeding.
