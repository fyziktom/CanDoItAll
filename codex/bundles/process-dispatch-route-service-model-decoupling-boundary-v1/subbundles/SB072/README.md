# SB072 - Gate O: recovery/subprocess closure
...
Gate O: recovery/subprocess closure.

This subbundle belongs to **P6: Recovery and subprocess service split**.
...
- Updated execution report row for SB072.

## Dependency Impact

- Downstream subbundles depend on SB072 preserving behavior and not widening route dependencies.
- If SB072 changes route order, route result semantics, claim behavior, finalizer behavior, or side-effect ownership, downstream proof is invalid and the affected earlier phase must be reopened.

## Validation Depth

- Focused validation for Gate O: recovery/subprocess closure..
- Cumulative behavior proof is consolidated at the next critical gate for P6: Recovery and subprocess service split.
...
- [ ] Execution report contains a row for SB072.
...
- Critical manifest and semantic-invariants file for SB072.
...
- May continue only after local checks for SB072 pass and the execution report has an individual SB072 row.
- Stop and reopen prerequisites if source scans, tests, or critical-gate proof contradict earlier assumptions.

## Suggested Agent Prompt

Implement SB072 from this bundle. Keep changes narrow, preserve existing process route behavior, and do not introduce Process Core or driver APIs. Record exact proof before proceeding.
