# SB034 — Process evidence provider interface rehearsal in tests only

## Status
- Status: Completed

## Objective
Test-only shape for supplying transcript/evidence content; no production interface or service registration.
...
- Updated proof material under `proof/SB034/`.
...
Implement SB034 for `process-driver-runtime-evidence-verifier-integration-hardening-v1`. Keep changes within scope, preserve all previous behavior, and record artifact-backed proof. Stop if any forbidden runtime/Core/driver/UI boundary is crossed.
