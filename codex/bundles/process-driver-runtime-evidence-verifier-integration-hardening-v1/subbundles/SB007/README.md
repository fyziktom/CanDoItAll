# SB007 — Evidence URI allowlist policy object

## Status
- Status: Completed

## Objective
Extract supplied evidence URI rules from adapter into module-local policy with explicit deny reasons and no filesystem reads.
...
- Updated proof material under `proof/SB007/`.
...
Implement SB007 for `process-driver-runtime-evidence-verifier-integration-hardening-v1`. Keep changes within scope, preserve all previous behavior, and record artifact-backed proof. Stop if any forbidden runtime/Core/driver/UI boundary is crossed.
