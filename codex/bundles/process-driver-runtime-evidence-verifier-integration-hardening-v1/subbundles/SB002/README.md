# SB002 — Active guardrail repair and failing-first baselines

## Status
- Status: Completed

## Objective
Add/refresh active architecture tests for no runtime registry/DI/selector/manager command, no Core reverse dependency, no UI/media drift, no stub markers.
...
- Updated proof material under `proof/SB002/`.
...
Implement SB002 for `process-driver-runtime-evidence-verifier-integration-hardening-v1`. Keep changes within scope, preserve all previous behavior, and record artifact-backed proof. Stop if any forbidden runtime/Core/driver/UI boundary is crossed.
