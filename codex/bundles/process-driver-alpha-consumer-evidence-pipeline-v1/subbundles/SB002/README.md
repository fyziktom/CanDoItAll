# SB002 — Guardrail baseline and failing-first scans

## Status
Prepared.

## Objective
Add/refresh active tests for no runtime registry, no DI, no manager command, no Core reverse dependency, no UI/media drift.
...
Implement `SB002 — Guardrail baseline and failing-first scans` from `process-driver-alpha-consumer-evidence-pipeline-v1`. Preserve all hard constraints and update proof before continuing.
