# SB05 — Validate artifact content through storage-backed reads
...
Make finalizer read stored artifact content for JSON/YAML/Markdown/image/evidence validation instead of only checking extension or inline summaries.
...
- `repo://src/CanDoItAll.Infrastructure`
...
1. Add `IProcessArtifactContentReader` or equivalent.
2. Resolve managed storage paths through storage service/workspace root safely.
3. Validate malformed JSON from a relative managed storage path and fail.
4. Add max-size and content-type handling.
5. Persist readable diagnostic when content cannot be loaded.
...
Implement SB05 from `codex/bundles/processes-hardening-followup-runtime-integrity-v4`. Preserve generic process semantics and capture artifact-backed proof before moving on.
