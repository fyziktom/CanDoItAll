...
## Objective
Make read-only verification jobs executable through process-owned scheduler/workflow paths without driver hooks.
...
## Deliverables
- Introduce job lifecycle records/readback if needed.
- Route scheduler/workflow-origin jobs through process services and manager facade.
- Add tests for scheduler-origin and workflow-origin read-only verification job execution, audit, and no mutation.
...
## Do Not Do
- Do not add scheduler hooks directly into driver packages.
- Do not allow execution-capable operations.
...
Implement SB06 as a code-first production/test change. Keep proof concise. Preserve Process Core genericity, deny all execution-capable effects, and keep the runtime-host roadmap explicit.
