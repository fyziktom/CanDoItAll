...
## Objective
Make audit production-grade enough for runtime-host governance.
...
## Deliverables
- Add EF configuration/indexes for process run, step run, lane, recorded time, and hash where appropriate.
- Add retention-ready query model and tests for time windows, limit, lane, process, step.
- Prove EF audit survives scope/profile readback.
- Keep in-memory store only as explicit test override.
...
## Do Not Do
- Do not silently fallback to in-memory in production module registration.
- Do not persist raw secrets in requester or diagnostic fields.
...
Implement SB04 as a code-first production/test change. Keep proof concise. Preserve Process Core genericity, deny all execution-capable effects, and keep the runtime-host roadmap explicit.
