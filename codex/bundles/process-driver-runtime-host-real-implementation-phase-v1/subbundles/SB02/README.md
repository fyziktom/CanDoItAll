...
## Objective
Move runtime host request/result/plan/snapshot concepts into a stable generic contract boundary.
...
## Deliverables
- Expand contracts to cover host request identity, effect surface, operation category, sandbox decision, denial, audit reference, and capability descriptor references.
- Keep contracts independent of EF, modules, OpenAI, UI, AgentFramework, and Process Core domain specifics.
...
## Do Not Do
- Do not add process-module references into Process Core.
- Do not model domain-specific names like .NET or Office in generic contracts.
...
Implement SB02 as a code-first production/test change. Keep proof concise. Preserve Process Core genericity, deny all execution-capable effects, and keep the runtime-host roadmap explicit.
