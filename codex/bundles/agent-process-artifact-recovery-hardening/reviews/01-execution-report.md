...
- Generalized reusable seeded Blazor resources away from calculator defaults; calculator-specific guidance remains only in the explicitly task-specific calculator skill.
...
| `git grep -n -I -i -E "calculator|calcapp|calculatorengine|simplecalculator" -- src/CanDoItAll.AgentFramework.Persistence/SeedAssets src/CanDoItAll.AgentFramework.Persistence/Seeds ':!src/CanDoItAll.AgentFramework.Persistence/SeedAssets/instructions/skills/blazor-calculator-build.md'` | Passed, no matches | Reusable seed assets are clean; the explicit calculator skill remains task-specific by design. |
...
- Browser proof is not required for subbundle 07 because it changes process orchestration, seeded guidance, and tests rather than a UI route.
...
| Core process must not contain calculator, Blazor, or .NET-specific hardcoded guidelines. | Solved | Dispatch source scans are clean; reusable seed scans are clean outside the explicit calculator skill; focused tests prove generic validation-after-mutation including an `.xlsx` deliverable. |
