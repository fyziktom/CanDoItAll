...
```text
dotnet test ./tests/Unit/CanDoItAll.Tests.Unit/CanDoItAll.Tests.Unit.csproj -c Release --filter 'FullyQualifiedName~Composition|FullyQualifiedName~Capability|FullyQualifiedName~Readiness|FullyQualifiedName~Architecture'
```
```text
dotnet build ./CanDoItAll.slnx -c Release --no-restore /m:1
```
```text
python ./scripts/scan_portability.py --repo-root <repo> --output <repo>/artifacts/unix-portability/A05/os-branch-scan.json
```
