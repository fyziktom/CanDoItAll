...
```text
dotnet test ./tests/Unit/CanDoItAll.Tests.Unit/CanDoItAll.Tests.Unit.csproj -c Release --filter 'FullyQualifiedName~Path|FullyQualifiedName~Workspace|FullyQualifiedName~Storage'
```
```text
dotnet build ./src/App/CanDoItAll.Web/CanDoItAll.Web.csproj -c Release
```
```text
python ./scripts/scan_portability.py --repo-root <repo> --output <repo>/artifacts/unix-portability/A01/post-scan.json
```
