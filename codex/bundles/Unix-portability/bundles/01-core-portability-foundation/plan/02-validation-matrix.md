...
| A04 | A03 | dotnet test ./tests/Unit/CanDoItAll.Tests.Unit/CanDoItAll.Tests.Unit.csproj -c Release --filter 'FullyQualifiedName~Secret\|FullyQualifiedName~Vault\|FullyQualifiedName~DataProtection\|FullyQualifiedName~DatabaseProfile'; dotnet test ./tests/Integration/CanDoItAll.Tests.Integration/CanDoItAll.Tests.Integration.csproj -c Release --filter 'Category=SecretPortability\|Category=SecretMigration' | Gate C2 is GO from architect, security reviewer, and runtime validator.; Auto selects only the declared platform tier and every weaker tier is typed and warned. |
