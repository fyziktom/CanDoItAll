...
B07 implementation is complete in the working branch and locally green. Final Gate R4 remains deferred by operator instruction because the configured hosted Windows/Ubuntu/macOS matrix has not executed against this branch. No macOS or hosted support claim is made.

## Fast validation contract

`tools/Validation/Test-RuntimePortability.ps1` is the single developer and CI entry point. It uses prebuilt assemblies (`--no-build --no-restore`), supports `Unit`, `Integration`, `Browser`, or `All`, and rejects class-selection or case-count drift.

Expected totals:

- unit: 12 governed classes, 422 cases;
- integration: 7 governed classes, 33 cases;
- browser: one `AppSmokeTests` method, 1 case;
- full host gate: 456 cases.

The active `.github/workflows/ci.yml` matrix invokes this runner on `windows-latest`, `ubuntu-24.04`, and `macos-14`, installs Chromium first, and uploads TRXs, headless-host evidence, and B07 browser screenshots.
...
|---|---|---|---|
| Windows desktop/headless | 456/456 focused gate green | Workflow configured, not yet executed for this snapshot | Local implementation proven; final hosted claim deferred |
| Ubuntu headless | 455 unit/integration cases green; browser not run locally | Workflow configured with Chromium, not yet executed | Local non-browser implementation proven; browser/final claim deferred |
...
Final Gate R4 requires retained artifacts from the configured three-host workflow, source/artifact reconciliation for that exact snapshot, and independent architecture/security/runtime/QA/operations review. Until then, RCI-001–005 and RCI-007 remain in progress; RCI-006 is locally solved.
