...
```text
dotnet test ./tests/Unit/CanDoItAll.Tests.Unit/CanDoItAll.Tests.Unit.csproj -c Release --filter 'FullyQualifiedName~DockerHostTool|FullyQualifiedName~DesktopFile|FullyQualifiedName~FileTools|FullyQualifiedName~ProjectStructureLocalFile'
```
```text
dotnet test ./tests/Integration/CanDoItAll.Tests.Integration/CanDoItAll.Tests.Integration.csproj -c Release --filter 'Category=PluginPortability|Category=DesktopPortability'
```
