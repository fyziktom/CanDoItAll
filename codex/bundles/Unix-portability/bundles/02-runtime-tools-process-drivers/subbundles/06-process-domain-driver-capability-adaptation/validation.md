...
```text
dotnet test ./tests/Unit/CanDoItAll.Tests.Unit/CanDoItAll.Tests.Unit.csproj -c Release --filter 'FullyQualifiedName~ProcessDriver|FullyQualifiedName~ProcessStrategy|FullyQualifiedName~ToolReceipt|FullyQualifiedName~Authority|FullyQualifiedName~WorkspaceScope'
```
```text
dotnet test ./tests/Integration/CanDoItAll.Tests.Integration/CanDoItAll.Tests.Integration.csproj -c Release --filter 'Category=ProcessCapabilityPortability'
```
