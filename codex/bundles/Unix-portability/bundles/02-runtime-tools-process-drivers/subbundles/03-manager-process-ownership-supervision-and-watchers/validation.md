...
```text
dotnet test ./tests/Unit/CanDoItAll.Tests.Unit/CanDoItAll.Tests.Unit.csproj -c Release --filter 'FullyQualifiedName~WorkspaceRuntimeProcessTools|FullyQualifiedName~WatchSupervisor|FullyQualifiedName~Tailwind|FullyQualifiedName~Manager'
```
```text
dotnet test ./tests/Integration/CanDoItAll.Tests.Integration/CanDoItAll.Tests.Integration.csproj -c Release --filter 'Category=ManagerPortability'
```
