...
- Expected behavior: TBD by subbundle README.
- Disallowed shallow implementation: prose-only changes, tests that only assert non-empty records, or validators not used by production finalizer.
...
List adversarial cases and expected rejection/blocking behavior.
