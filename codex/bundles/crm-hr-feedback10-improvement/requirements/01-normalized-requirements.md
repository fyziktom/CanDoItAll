...
| `R016` | Follow-up | Make Directory and Workforce ordinary searchable lists match the Agents catalogue interaction while retaining real server paging. | Each route shows a full-width card catalogue with compact controls, a bounded internal result-scroll owner, fixed pager, loading/empty/error/retry states, and no permanent detail pane or full-list fallback. |
| `R017` | Follow-up | Move Directory and Workforce record details/editing into controlled dialogs and prefix CRM-HR subpage workbench titles with `CRM`. | Card/deep-link selection opens the existing tabbed record workspace in a usable dialog; close preserves list usability; workbench labels such as `CRM Directory` and `CRM Workforce` are clear while secondary navigation stays concise. |
| `R018` | Follow-up | Add the CRM-HR HTTP query/command contract and a reusable Codex API skill needed to operate realistic party, workforce, and recruiting scenarios. | `/api/crm-hr` delegates to canonical application services under existing authorization; bounded/safe queries and structured failures are tested; the repo skill validates, is synchronized to the active root, and documents exact requests. |
| `R019` | Follow-up | Populate realistic, complex hiring and workforce scenarios through CRM-HR APIs only. | Persistent multi-stage recruiting, interviews, onboarding, workforce, skill, manager/unit, contractor, and capacity cases appear in the actual UI; a repeated run is identity-idempotent; no direct SQL/startup seed/fake financial data is used. |
| `R020` | Follow-up | Keep affected implementation, API, operator-skill, architecture, validation, and bundle documentation current. | Module README, API/skill references, bundle requirements/traceability/architecture/status, design proposals, and final proof agree with shipped behavior and the restarted host. |
