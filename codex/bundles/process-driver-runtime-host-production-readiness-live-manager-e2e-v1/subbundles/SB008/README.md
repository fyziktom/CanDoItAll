# SB008 - Convert expected validation errors to structured denials across host/facade/API
...
## Objective
Convert expected validation errors to structured denials across host/facade/API
...
5. Record proof under `proof/SB008/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB008`.
...
Implement `SB008` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
