# SB015 - Critical Gate E: manager API returns diagnostics/audit/denials and no mutation flags
...
## Objective
Critical Gate E: manager API returns diagnostics/audit/denials and no mutation flags
...
5. Record proof under `proof/SB015/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB015`.
- [x] Semantic Adequacy Gate proof exists for this critical gate.

## Proof Required
- Completed critical proof: bundle://proof/SB015/manifest.md and bundle://proof/SB015/semantic-invariants.md
...
Implement `SB015` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
