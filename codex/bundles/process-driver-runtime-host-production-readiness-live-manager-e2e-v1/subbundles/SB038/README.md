# SB038 - Enforce evidence size, hash binding, URI allowlist, and redaction in host + manager API
...
## Objective
Enforce evidence size, hash binding, URI allowlist, and redaction in host + manager API
...
5. Record proof under `proof/SB038/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB038`.
...
Implement `SB038` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
