# SB013 - Add explicit manager-readonly verification API endpoint or service endpoint map
...
## Objective
Add explicit manager-readonly verification API endpoint or service endpoint map
...
5. Record proof under `proof/SB013/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB013`.
...
Implement `SB013` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
