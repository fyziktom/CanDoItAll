# SB009 - Critical Gate C: no exception-as-control-flow for expected host denials
...
5. Record proof under `proof/SB009/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB009`.
- [x] Semantic Adequacy Gate proof exists for this critical gate.

## Proof Required
- Completed critical proof: bundle://proof/SB009/manifest.md and bundle://proof/SB009/semantic-invariants.md
...
Implement `SB009` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
