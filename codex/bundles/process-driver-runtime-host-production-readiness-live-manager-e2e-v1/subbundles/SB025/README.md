# SB025 - Require explicit model, timeout, and total-token budget in live process-run test
...
## Objective
Require explicit model, timeout, and total-token budget in live process-run test
...
5. Record proof under `proof/SB025/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB025`.
...
Implement `SB025` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
