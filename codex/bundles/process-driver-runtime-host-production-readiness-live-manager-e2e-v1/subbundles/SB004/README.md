# SB004 - Replace default production DI from in-memory audit store to EF-backed audit store
...
## Objective
Replace default production DI from in-memory audit store to EF-backed audit store
...
5. Record proof under `proof/SB004/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB004`.
...
Implement `SB004` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
