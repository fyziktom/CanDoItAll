# SB006 - Critical Gate B: durable audit persists sanitized records and survives scope restart
...
## Objective
Critical Gate B: durable audit persists sanitized records and survives scope restart
...
5. Record proof under `proof/SB006/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB006`.
- [x] Semantic Adequacy Gate proof exists for this critical gate.

## Proof Required
- Completed critical proof: bundle://proof/SB006/manifest.md and bundle://proof/SB006/semantic-invariants.md
...
Implement `SB006` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
