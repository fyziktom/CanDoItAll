# SB047 - Add API parity and screenshot review checklist for operator-critical pages
...
## Objective
Add API parity and screenshot review checklist for operator-critical pages
...
5. Record proof under `proof/SB047/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB047`.
...
Implement `SB047` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
