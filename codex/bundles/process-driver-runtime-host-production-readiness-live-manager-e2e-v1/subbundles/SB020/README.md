# SB020 - Add blocked/failed run auto-diagnostic readback using supplied runtime/artifact facts
...
## Objective
Add blocked/failed run auto-diagnostic readback using supplied runtime/artifact facts
...
5. Record proof under `proof/SB020/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB020`.
...
Implement `SB020` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
