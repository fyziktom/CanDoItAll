# SB003 - Critical Gate A: source-backed baseline and stale/bundle-path proof rejection
...
## Objective
Critical Gate A: source-backed baseline and stale/bundle-path proof rejection
...
5. Record proof under `proof/SB003/` if this is a critical gate, otherwise under the nearest phase proof area.
...
- [x] Execution report has an individual row for `SB003`.
- [x] Semantic Adequacy Gate proof exists for this critical gate.

## Proof Required
- Completed critical proof: bundle://proof/SB003/manifest.md and bundle://proof/SB003/semantic-invariants.md
...
Implement `SB003` for `process-driver-runtime-host-production-readiness-live-manager-e2e-v1`. Preserve read-only verification boundaries, use real source/tests, and do not collapse proof into report-only claims.
