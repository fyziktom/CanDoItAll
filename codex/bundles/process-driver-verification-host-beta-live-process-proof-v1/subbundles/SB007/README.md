...
## Objective
Add live process-run test setup as part of P03: Live process-run OpenAI smoke.
...
- Proof transcript paths under `proof/SB007/transcripts/`.
...
## Proof Captured
- Added `repo://tests/CanDoItAll.Tests.Integration/LiveProcessRunOpenAiSmokeIntegrationTests.cs`.
- Source setup assertion transcript: `bundle://proof/SB007/transcripts/live-process-run-setup-source-assertions.txt`.
- Focused compile/disabled-path test transcript: `bundle://proof/SB008/transcripts/live-process-run-strict-skip-test.txt`.
...
Implement SB007 for `process-driver-verification-host-beta-live-process-proof-v1`. Re-read current source first, avoid report-only closure, preserve no-mutation and Core genericity guarantees, and capture proof before updating status.
