# SB013 — Command policy allow/deny model
...
Define docs/test-only command policy: no shell execution, no Graph/Office calls, no workspace/storage writes, no process mutation for current bundle.
...
Implement SB013 for phase P05 (Sandbox and command policy denial). Keep changes scoped to: Define docs/test-only command policy: no shell execution, no Graph/Office calls, no workspace/storage writes, no process mutation for current bundle.
