# SB016 — Verification evidence request/response rehearsal
...
Create test-only/readme contract shape for verification-only requests and results, using existing Core descriptors as inputs.
...
Implement SB016 for phase P06 (Verification-only contract rehearsal without production runtime). Keep changes scoped to: Create test-only/readme contract shape for verification-only requests and results, using existing Core descriptors as inputs.
