# SB010 — Audit fact schema proposal
...
Define audit fact shape: caller, mode, lane, process/run/step/artifact ids, input evidence ids, denial reason, output hash, and redaction status.
...
Implement SB010 for phase P04 (Audit facts, redaction, and evidence traceability). Keep changes scoped to: Define audit fact shape: caller, mode, lane, process/run/step/artifact ids, input evidence ids, denial reason, output hash, and redaction status.
