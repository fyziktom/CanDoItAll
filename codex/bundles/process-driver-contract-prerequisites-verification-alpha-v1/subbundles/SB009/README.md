# SB009 — Gate C: permission/capability closure
...
Prove missing mode is denied, verification-only is read-only, manager-readonly cannot mutate, and execution-capable remains disabled.
...
Implement SB009 for phase P03 (Driver permission model as executable prerequisites). Keep changes scoped to: Prove missing mode is denied, verification-only is read-only, manager-readonly cannot mutate, and execution-capable remains disabled.
