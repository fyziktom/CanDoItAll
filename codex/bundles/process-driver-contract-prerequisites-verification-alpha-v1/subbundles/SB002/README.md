# SB002 — Active architecture guard baseline
...
Add or refresh architecture tests that lock no broad Core runtime extraction, no production driver API, no UI/media drift, and no collapsed execution-report rows for this bundle.
...
- Proof files under `proof/SB002/`.
- Updated execution report row.
- Updated architecture/analysis documents if this subbundle owns a decision.

## Dependency Impact

Standard. Downstream phases become invalid if this subbundle changes permission semantics, Core dependency hygiene, route order, driver API availability, or proof assumptions.
...
Implement SB002 for phase P01 (Baseline, active proof intake, and no-regression guardrails). Keep changes scoped to: Add or refresh architecture tests that lock no broad Core runtime extraction, no production driver API, no UI/media drift, and no collapsed execution-report rows for this bundle.
