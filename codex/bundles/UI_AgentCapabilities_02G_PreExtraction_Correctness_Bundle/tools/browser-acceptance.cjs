...
(async()=>{
 ...
 try {
  ...
  await page.getByTestId('floating-agent-chat-window').getByRole('button',{name:'Minimize window',exact:true}).click();
  await page.getByTestId('floating-agent-chat-content').waitFor({state:'hidden'});
  await shot('curator-warning-after-inspection');
