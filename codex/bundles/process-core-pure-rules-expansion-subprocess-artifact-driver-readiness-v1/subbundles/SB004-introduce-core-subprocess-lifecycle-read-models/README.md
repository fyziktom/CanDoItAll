# SB004 - Introduce Core subprocess lifecycle read models

## Status
- Status: `Completed`
## Objective
Create Core subprocess status/reason input DTOs that do not depend on module entities.
...
## Deliverables
- `Core/Subprocess` namespace
- Lifecycle status input/output records
- No transition request execution types in Core
...
- Unit tests for DTO construction
...
4. Run the subbundle proof commands and store transcripts under `proof/SB004/transcripts/`.
...
- [ ] Execution report has an individual `SB004` row.
...
Implement `SB004 - Introduce Core subprocess lifecycle read models` exactly as scoped. Preserve existing process behavior, keep Core pure, keep driver work docs/tests-only, and record proof before proceeding.
