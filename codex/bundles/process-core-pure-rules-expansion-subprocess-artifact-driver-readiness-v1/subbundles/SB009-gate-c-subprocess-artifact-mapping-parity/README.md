...
## Objective
Prove pure mapping moved safely and runtime projection remains module-local.
...
## Deliverables
- Critical manifest
- Negative Core side-effect scan
- Parity matrix
...
- Build
- Focused unit tests
- Focused subprocess projection integration tests
...
4. Run the subbundle proof commands and store transcripts under `proof/SB009/transcripts/`.
...
- [ ] Execution report has an individual `SB009` row.
...
Implement `SB009 - Gate C - subprocess artifact mapping parity` exactly as scoped. Preserve existing process behavior, keep Core pure, keep driver work docs/tests-only, and record proof before proceeding.
