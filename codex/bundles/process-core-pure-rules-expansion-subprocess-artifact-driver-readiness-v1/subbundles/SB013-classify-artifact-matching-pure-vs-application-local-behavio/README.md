# SB013 - Classify artifact matching pure vs application-local behavior

## Status
- Status: `Completed`
## Objective
Inventory artifact expectation matching and satisfaction rules and mark what can move to Core.
...
## Deliverables
- Pure/application classification table
- Reopen triggers
- No production code change unless safe
...
4. Run the subbundle proof commands and store transcripts under `proof/SB013/transcripts/`.
...
- [ ] Execution report has an individual `SB013` row.
...
Implement `SB013 - Classify artifact matching pure vs application-local behavior` exactly as scoped. Preserve existing process behavior, keep Core pure, keep driver work docs/tests-only, and record proof before proceeding.
