# SB016 - Consolidate module-local Core adapters

## Status
- Status: `Completed`
## Objective
Collect Core adapters for route/subprocess/artifact in clearly named module-local adapter files.
...
## Deliverables
- `ProcessCore*Adapter` naming convention
- Adapter source map
- No adapter logic inside Core
...
4. Run the subbundle proof commands and store transcripts under `proof/SB016/transcripts/`.
...
- [ ] Execution report has an individual `SB016` row.
...
Implement `SB016 - Consolidate module-local Core adapters` exactly as scoped. Preserve existing process behavior, keep Core pure, keep driver work docs/tests-only, and record proof before proceeding.
