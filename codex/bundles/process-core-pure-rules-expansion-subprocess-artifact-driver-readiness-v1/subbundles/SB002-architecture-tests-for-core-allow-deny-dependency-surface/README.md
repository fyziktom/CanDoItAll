# SB002 - Architecture tests for Core allow/deny dependency surface

## Status
Prepared.

## Objective
Add or strengthen failing-first architecture tests that reject broad Core extraction and runtime driver APIs.
...
## Deliverables
- Tests proving Core references only Contracts
- Tests rejecting forbidden namespaces/tokens in Core
- Tests rejecting production driver tokens
...
## Validation Depth
- Focused architecture tests fail before guard if violated and pass after
- No source production behavior change
...
4. Run the subbundle proof commands and store transcripts under `proof/SB002/transcripts/`.
...
- [ ] Execution report has an individual `SB002` row.
...
Implement `SB002 - Architecture tests for Core allow/deny dependency surface` exactly as scoped. Preserve existing process behavior, keep Core pure, keep driver work docs/tests-only, and record proof before proceeding.
