# SB005 - Move pure subprocess parent status/reason rules

## Status
Prepared.

## Objective
Move only deterministic subprocess parent status and reason building rules into Core.
...
## Deliverables
- Core lifecycle rule class
- Module adapter/wrapper for existing call sites
- No child-run observation or projection persistence moved
...
## Validation Depth
- Parity unit tests for Completed/Blocked/Cancelled/Failed/unknown statuses
- Existing integration subprocess tests
...
4. Run the subbundle proof commands and store transcripts under `proof/SB005/transcripts/`.
...
- [ ] Execution report has an individual `SB005` row.
...
Implement `SB005 - Move pure subprocess parent status/reason rules` exactly as scoped. Preserve existing process behavior, keep Core pure, keep driver work docs/tests-only, and record proof before proceeding.
