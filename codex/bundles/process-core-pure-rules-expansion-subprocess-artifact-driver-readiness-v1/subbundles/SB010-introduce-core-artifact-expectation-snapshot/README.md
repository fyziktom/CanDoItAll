# SB010 - Introduce Core artifact expectation snapshot

## Status
- Status: `Completed`
## Objective
Create Core snapshot for artifact expectation facts currently duplicated by validation/projection/satisfaction.
...
- `repo://src/CanDoItAll.Modules.Processes/Automation/Dispatch/ProcessArtifactValidationSnapshot.cs`

## Deliverables
- Core artifact expectation snapshot
- Module adapter from `ProcessArtifactExpectationSnapshot`
- No storage/projection semantics in Core
...
## Validation Depth
- Snapshot equivalence unit tests
...
4. Run the subbundle proof commands and store transcripts under `proof/SB010/transcripts/`.
...
- [ ] Execution report has an individual `SB010` row.
...
Implement `SB010 - Introduce Core artifact expectation snapshot` exactly as scoped. Preserve existing process behavior, keep Core pure, keep driver work docs/tests-only, and record proof before proceeding.
