...
| RQ-027 | Conversation origin is server-owned and cannot be spoofed by an HTTP client. | SB10, SB11 | SB10 HTTP rejection, direct Application-origin, and authoritative PostgreSQL Api-origin proof pass; SB11 confirmation pending |
| RQ-028 | Enforce LLM Chat read/manage/execute API scopes when bearer authorization is enabled. | SB10, SB11 | SB10 exact-scope real-host matrix passes, including broad-api and cross-scope denial plus auth-disabled compatibility; SB11 confirmation pending |
| RQ-029 | Do not expose prompts, system instructions, credentials, raw provider payloads, or raw provider errors through logs/API/SSE. | SB08, SB09, SB10, SB11 | SB08 journal, SB09 typed SSE/provider-secret proof, and SB10 definition/conflict/raw-log guards pass; SB11 confirmation pending |
...
| RQ-033 | Preserve a clean future LlmChatDeployment boundary for enterprise chatbot channels without dormant deployment fields now. | SB10, SB12 | SB10 deployment boundary/idempotency decision recorded with no dormant product/persistence fields; SB12 final documentation guard pending |
