...
## Execution refinement (SB00)

Assignment uses a private independent editor copy, stable AgentId/AttemptId, expected revision and immutable exact before/desired sets. Only the focused capability command adapter calls Save; the coordinator retains unresolved attempts across component reconstruction in the circuit. Known rejection releases the write lock; conflict requires canonical adoption. Unknown verification compares exact canonical identity, revision and sets: desired satisfied (not historical causation), unchanged before (deliberate same-submission retry), intervening (explicit adoption), or still unknown. Reconciliation never writes. Terminal attempts are removed only after matching reconciliation/adoption; stale completion cannot remove another entry. One assignment or proof operation per agent; different agents may proceed independently.

Extract diagnostic/persistence orchestration from the catalog partial into a cohesive Core operation with a typed execution result and immutable proof receipt. The existing Task facade remains source compatible and translates noncommitted outcomes into a typed sanitized exception; UI and API handle that exception explicitly. No new method is imposed on every workspace test double. Capture full semantic agent/capability and provider lease fingerprints before diagnostics; reacquire provider lease afterward, then compare inputs and synchronous provider lease inside the locked catalog update. This is optimistic validation across separate provider/catalog authorities, not a distributed transaction. Changes after the final provider observation remain an explicit consistency limit; do not claim a cross-store serializable lock. A diagnostic receipt contains target, checked timestamp and result fingerprint for read-only verification. No diagnostic replay.

Catalog write: DurableFileWriter flushes a temporary file, checks cancellation, atomically replaces the target, then applies/verifies metadata and cleanup. JSON history observer is relevant only to ProviderUsageObservation, not catalog payloads. FileSandboxWorkspaceStore saves catalog (with its own CatalogDataRevision), then writes workspace index. A surviving old index is not automatically repaired to the catalog revision: the revisions have different meaning. Assignment verification uses canonical agent revision, never workspace-index equality. No persistence redesign is needed.

Preview owns its own latest-wins CTS linked to the panel and canceled at target/read replacement. Details/setup own panel-level overlay slots and direct tokens; the audited current child markup opens no additional nested dialogs. Each dialog cancels its reads/diagnostics on disposal, fences notifications, and keeps a dispatched capability save observed without claiming rollback. Curator is panel-level single-flight, independent of selection. Once launch is dispatched, do not cancel the non-idempotent creation merely for panel removal; retain known chat in the existing application registry and suppress late presentation. An unresolved launch stays blocked in the circuit rather than inviting replay. No universal effect controller.

The surface remains service-free and retains local filters/raw preview draft. Existing SurfaceCard/Stack/Button provide inline outcome and explicit recovery actions above the list. No redesign, new CSS or project references. Components MCP recommendation failed with Transport closed; actual sibling component source is the fallback. Scoped CodeAnalytics entry snap-20260906200039-9c6cc38d has 203 module documents; use direct project source for transitive graph claims.
