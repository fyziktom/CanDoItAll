# SB011 - Direct-agent binding and cooperation boundary

## Status
Completed.

## Objective
Move technical-agent binding, project-structure access mutation, recovery execution selection, manual directive loading, and cooperation metadata into explicit collaborators.
...
- Proof transcript under `codex/bundles/process-core-contract-candidate-driver-readiness-prep-v1/proof/SB011/`.
- Execution report row for `SB011`.
...
## Closure Notes
- Entry gate: Passed. SB010 is completed.
- Closure gate: Passed. Direct-agent binding, project-structure access logging, execution-run blocking, recovery/directive lookup, and cooperation metadata are isolated in `ProcessDispatchDirectAgentCandidateAssembler` and existing binding/recovery/cooperation collaborators.
- Test proof: `bundle://proof/SB011/transcripts/direct-agent-boundary-architecture-tests.txt`.
- Source assertions and guard scans: `bundle://proof/SB011/transcripts/source-assertions-and-scans.txt`.

## Suggested Agent Prompt
Implement `SB011` from `process-core-contract-candidate-driver-readiness-prep-v1`. Preserve runtime behavior. Keep this work module-local and do not introduce Process Core or production driver APIs.
