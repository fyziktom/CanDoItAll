# SB005 - Route candidate source payload reduction

## Status
Completed.

## Objective
Replace route-facing logic that unwraps dispatcher candidates with explicit route model fields or route service methods; adapter may remain only at dispatcher-edge compatibility seams.
...
- Proof transcript under `codex/bundles/process-core-contract-candidate-driver-readiness-prep-v1/proof/SB005/`.
- Execution report row for `SB005`.
...
## Closure Notes
- Entry gate: Passed. SB004 mapped route source payload usage.
- Closure gate: Passed. `repo://src/CanDoItAll.Modules.Processes/Automation/Dispatch/ProcessRunAutomationDispatchService.RouteExecution.cs` now consumes `LoadRouteCandidateAsync` and no longer calls `FromDispatcherCandidate`.
- Proof: `bundle://proof/SB005/transcripts/route-boundary-architecture-tests.txt` and `bundle://proof/SB005/transcripts/source-assertions-and-scans.txt`.

## Suggested Agent Prompt
Implement `SB005` from `process-core-contract-candidate-driver-readiness-prep-v1`. Preserve runtime behavior. Keep this work module-local and do not introduce Process Core or production driver APIs.
