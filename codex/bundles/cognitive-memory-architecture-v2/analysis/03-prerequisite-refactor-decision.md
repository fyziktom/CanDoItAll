...
The original architecture review required a boundary refactor before Cognitive Memory implementation. In the supplied current code snapshot, this prerequisite appears to have been implemented and validated. Keep this document as the historical decision and use `analysis/04-current-code-delta-and-probing-gap-analysis.md` as the current status addendum.

The required prerequisite was not a large rewrite. It is a boundary refactor that prevents Cognitive Memory from being hardwired into current private MAF internals and prevents high-volume source ingestion from depending on UI-oriented or EF-entity-specific read paths.
...
- `codex/bundles/cognitive-memory-projection-boundary-hardening` has also passed; projection-backed recall must consume the completed typed RAG filter, payload-index, delete-by-filter, and embedding-profile contracts instead of direct Qdrant calls or unscoped vector post-filtering.
...
- Projection-boundary hardening targeted RAG/SemanticCompletion tests, related solution builds, architecture sync, and completed-stage bundle validation passed.
