...
assert(['acceptance', 'warm', 'context'].includes(phase));
const refresh = args.refresh || 'on';
assert(['on', 'off'].includes(refresh));
const modulePath = args['playwright-module'] || process.env.CATALOG_PLAYWRIGHT_MODULE || 'playwright';
const { chromium } = require(modulePath);
const runId = host + '-' + refresh + '-' + phase + '-' + new Date().toISOString().replace(/[:.]/g, '-');
const output = path.resolve(args.output || path.join(root, '.mcp-state/catalog-harden-02'), runId);
...
const url = base + (host === 'fullapp' ? '/agents?tab=agents' : '/agents?scenario=normal&layout=matched');
...
const browserEvents = [];
const recordBrowser = value => browserEvents.push({ ns: clock().toString(), ...value });
async function documentState() {
  return page.evaluate(() => ({ id: window.catalogDocumentIdentity, timeOrigin: performance.timeOrigin })).catch(() => ({ id: null }));
}
async function instrumentBrowser() {
  await page.exposeFunction('catalogDiagnosticEvent', kind => recordBrowser({ kind }));
  await page.addInitScript(() => {
    window.catalogDocumentIdentity = crypto.randomUUID();
    const timer = setInterval(() => {
      if (!window.Blazor?.addEventListener) return;
      clearInterval(timer);
      for (const name of ['enhancednavigationstart', 'enhancedload', 'enhancednavigationend']) {
        window.Blazor.addEventListener(name, () => window.catalogDiagnosticEvent(name));
      }
    }, 50);
  });
  page.on('websocket', socket => socket.on('framereceived', frame => {
    try {
      ...
      if (['RefreshBrowser', 'Reload', 'Wait', 'UpdateStaticFile', 'ApplyManagedCodeUpdates'].includes(value.type)) {
        recordBrowser({ kind: 'sdk-message', type: value.type });
      }
    } catch {}
  }));
  page.on('console', message => {
    if (message.text().startsWith('Enhanced navigation failed for destination')) recordBrowser({ kind: 'enhanced-navigation-fallback' });
  });
  page.on('response', async response => {
    try {
      const request = response.request();
      if ((request.headers().accept || '').includes('blazor-enhanced-nav')) {
        const headers = await response.allHeaders();
        recordBrowser({ kind: 'enhanced-response', status: response.status(), allowed: headers['blazor-enhanced-nav'] === 'allow', html: (headers['content-type'] || '').includes('text/html') });
      }
    } catch {}
  });
  page.on('framenavigated', frame => { if (frame === page.mainFrame()) recordBrowser({ kind: 'main-frame-navigation' }); });
}
async function contextAcceptance() {
  assert(host !== 'fullapp');
  const cases = [];
  await page.goto(base + '/agents?scenario=card-states&layout=flexible', { waitUntil: 'domcontentloaded' });
  await page.getByTestId('agent-favorite-toggle').waitFor({ timeout: 7000 });
  assert.equal(await page.locator('main').getAttribute('class'), 'catalog-sandbox__frame');
  await page.reload({ waitUntil: 'domcontentloaded' });
  await page.getByTestId('agent-favorite-toggle').waitFor();
  assert.equal(await page.locator('main').getAttribute('class'), 'catalog-sandbox__frame');
  cases.push('Explicit_card_scenario_and_flexible_layout_survive_reload');
  const history = await page.evaluate(() => window.history.length);
  for (const [button, selector, text] of [
    ['sandbox-loading', 'h3', 'Loading technical agents'],
    ['sandbox-empty', 'h3', 'Create the first technical agent'],
    ['sandbox-avatar-fallback', null, 'CA'],
    ['sandbox-card-states', null, null],
    ['sandbox-normal', null, null]]) {
    await page.getByTestId(button).click();
    const token = button.replace('sandbox-', '');
    await page.waitForURL(u => u.searchParams.get('scenario') === token);
    await page.reload({ waitUntil: 'domcontentloaded' });
    if (text) await page.getByText(text, { exact: true }).waitFor();
    else if (button === 'sandbox-normal') await page.getByTestId('agents-catalog-card').first().waitFor();
    else await page.getByTestId('agent-favorite-toggle').waitFor();
    assert.equal(await page.locator('main').getAttribute('class'), 'catalog-sandbox__frame');
    cases.push(token + '_restores_after_reload');
  }
  await page.getByTestId('sandbox-layout').click();
  await page.waitForURL(u => u.searchParams.get('layout') === 'matched');
  await page.getByTestId('agents-catalog-card').first().click();
  await page.waitForURL(u => !!u.searchParams.get('agentId'));
  const agentId = new URL(page.url()).searchParams.get('agentId');
  const fixtureData = JSON.parse(fs.readFileSync(path.join(root, 'src/Sandboxes/CanDoItAll.AgentFramework.UiSandbox/Fixtures/catalog.json'), 'utf8'));
  const team = (fixtureData.teams || fixtureData.Teams)[0];
  const teamId = team.id || team.Id;
  const teamName = team.name || team.Name;
  await page.getByTestId('agents-team-tree').getByText(teamName, { exact: true }).first().click();
  await page.waitForURL(u => u.searchParams.get('teamId') === teamId);
  assert.equal(await page.evaluate(() => window.history.length), history, 'Transient controls must replace history.');
  await page.reload({ waitUntil: 'domcontentloaded' });
  await page.getByTestId('agents-team-edit').waitFor();
  assert.equal(await page.getByTestId('agents-team-edit').isEnabled(), true);
  assert.equal(new URL(page.url()).searchParams.get('agentId'), agentId);
  assert.equal(new URL(page.url()).searchParams.get('teamId'), teamId);
  assert((await page.locator('main').getAttribute('class')).includes('--matched'));
  cases.push('Selection_team_and_matched_layout_restore_without_history_growth');
  await page.screenshot({ path: path.join(output, 'restored-selection.jpeg'), type: 'jpeg', quality: 80 });
  await page.goto(base + '/agents?scenario=unknown&layout=77&agentId=ffffffff-ffff-ffff-ffff-ffffffffffff&teamId=invalid');
  await page.getByTestId('agents-catalog-card').first().waitFor();
  await page.waitForURL(u => u.searchParams.get('scenario') === 'normal' && u.searchParams.get('layout') === 'matched' && !u.searchParams.has('agentId') && !u.searchParams.has('teamId'));
  cases.push('Invalid_query_normalizes_to_safe_fixture_context');
  log({ kind: 'context-acceptance', cases, historyLengthBefore: history, agentId, teamId });
}
...
async function settleNavigation(cursor, before, edit, expected) {
  const start = Date.now();
  let lastCount = browserEvents.length;
  let lastChange = start;
  while (Date.now() - start < 15000) {
    await confirmIsolatedDatabase();
    if (lastCount !== browserEvents.length) { lastCount = browserEvents.length; lastChange = Date.now(); }
    ...
    const settled = starts === 0 || ends >= starts || current.id !== before.id;
    if (current.id && settled && Date.now() - start >= 1500 && Date.now() - lastChange >= 350) {
      await predicate(edit, expected);
      return { starts, ends, documentChanged: current.id !== before.id, waitMs: Date.now() - start };
    }
    await pause(100);
  }
  throw new Error('Navigation did not settle after managed apply; the final rendering is unconfirmed.');
}
...
async function trial(edit, repetition) {
  ...
  try {
    ...
    const settlement = await settleNavigation(browserCursor, documentBefore, edit, edit.after);
    await predicate(edit, edit.after);
    const t1 = clock();
    const after = await probe();
    const documentAfter = await documentState();
    const observed = browserEvents.slice(browserCursor);
    const classification = after.runtimePid !== ready.runtimePid ? 'restart' : documentAfter.id !== documentBefore.id ? 'browser-reload' : observed.some(e => e.kind === 'enhancednavigationstart') ? 'enhanced-navigation' : sdk.length === 1 ? 'hot-reload' : 'missing-sdk-event';
    result = { kind: 'warm', host, editId: edit.id, category: edit.category, repetition, classification,
      success: ['hot-reload', 'browser-reload', 'enhanced-navigation', 'restart'].includes(classification), settlement, refresh, documentBefore, documentAfter, browserEvents: observed, sourceSha256: sha(original),
    ...
    result = { kind: 'warm', host, editId: edit.id, category: edit.category, repetition, success: false,
      classification: /Timeout/i.test(error.message) ? 'timeout' : 'harness-failure', refresh, documentBefore, documentAfter: await documentState(), browserEvents: browserEvents.slice(browserCursor), sdkEvents: events.slice(cursor).filter(e => /changes applied|rude edit|restart|error/i.test(e.text)), error: safeError(error), sourceSha256: sha(original),
    ...
    const undoBrowserCursor = browserEvents.length;
    const undoDocument = await documentState();
    flush(file, original);
    try { await predicate(edit, edit.before); result.undoSdkEvents = await sdkCompletion(undoCursor); result.undoSettlement = await settleNavigation(undoBrowserCursor, undoDocument, edit, edit.before); await predicate(edit, edit.before); result.undoVisible = result.undoSdkEvents.length === 1; result.undoBrowserEvents = browserEvents.slice(undoBrowserCursor); } catch (error) { result.undoVisible = false; result.undoError = safeError(error); }
...
(async () => {
  ...
  Object.assign(env, { ASPNETCORE_ENVIRONMENT: 'Development', DOTNET_ENVIRONMENT: 'Development',
    ...
    DOTNET_WATCH_SUPPRESS_LAUNCH_BROWSER: '1', DOTNET_WATCH_SUPPRESS_BROWSER_REFRESH: refresh === 'off' ? '1' : '0',
  ...
  const launch = ['--no-launch-profile'];
  ...
  log({ kind: 'protocol', runId, planSha256: sha(fs.readFileSync(path.join(bundle, 'plan/frozen-direct-edits.json'))),
    harnessSha256: sha(fs.readFileSync(__filename)), host, phase, refresh, mode: host === 'fullapp' ? 'Production' : mode, sdk: execFileSync('dotnet', ['--version']).toString().trim(),
  ...
  if (phase === 'context') await contextAcceptance();
  else if (phase === 'acceptance') { await acceptance(); await contextAcceptance(); }
  else {
    await assets();
    for (const edit of plan.edits) {
      for (let repetition = 1; repetition <= plan.successfulRepetitionsPerEdit; repetition++) {
        ...
        if (!result.undoVisible) throw new Error('Undo failed; retained explicitly. Review before continuing.');
        await pause(750);
      }
    }
  }
  fs.writeFileSync(path.join(output, 'browser-events.json'), JSON.stringify(browserEvents, null, 2));
