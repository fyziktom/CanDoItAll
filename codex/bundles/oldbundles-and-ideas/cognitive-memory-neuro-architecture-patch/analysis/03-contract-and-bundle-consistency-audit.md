...
The existing bundle has `subbundles/12-epistemic-drive-engine` and `subbundles/13-interactive-memory-probing-workbench`, while the plan path uses `12-interactive-memory-probing-workbench`. This can cause Codex to apply the wrong order.

Patch instruction: keep old folder names for compatibility, but add a clear execution-order table in README, MANIFEST, and `plan/01-phase-plan.md`.
