# SB15: Optional ordinary LLM conversation application foundation (no full chat UI)
...
- Depends on: `SB14`
- Checkpoint: No
- Optional: Yes — not a maf-refactor merge blocker
...
- Do not reuse ChatSessionRecord if doing so imports agent-specific semantics.
- Do not make provider conversation ID canonical.
- Do not add tools as a shortcut.
...
This subbundle is not a maf-refactor merge blocker and may be deferred with a recorded decision.
