...
- **Software-delivery evidence**: `CanDoItAll.Processes.Drivers.SoftwareDeliveryEvidence` owns stack-specific implementation proof policy, carried-proof decisions, and prompt/recovery guidance text. The process module maps process facts into driver snapshots and performs approved repository/file inspection at the module boundary; the driver remains read-only and has no dependency on Processes, workspace services, shell execution, package restore, network calls, storage writes, or process mutation.
...
- Software-delivery evidence policy and stack-specific prompt/recovery guidance belong in `CanDoItAll.Processes.Drivers.SoftwareDeliveryEvidence`; Processes may only adapt already-known process facts and approved file-inspection snapshots into that read-only driver.
