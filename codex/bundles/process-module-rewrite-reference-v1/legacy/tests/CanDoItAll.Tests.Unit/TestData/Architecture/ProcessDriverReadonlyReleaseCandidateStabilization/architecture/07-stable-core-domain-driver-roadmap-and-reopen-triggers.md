...
## Handoff Rule
The next bundle must start by reopening the current source, not by trusting this report. Use the Gate R manifest, completed validator transcript, bundle zip, and the source-backed tests as entry proof, then rerun build, focused tests, source scans, and validators before changing behavior.
