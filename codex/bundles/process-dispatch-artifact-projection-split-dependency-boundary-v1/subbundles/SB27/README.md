...
Extract projection-local managed path utility where it reduces duplication without changing `ProcessArtifactPathValidationRules`.
...
- ProcessArtifactProjectionPathRules.cs

## Dependency Impact

- Phase F production movement or proof step. Downstream subbundles depend on behavior-preserving completion of this slice.
...
5. Capture command transcripts under bundle://proof/SB27/transcripts/ when the subbundle produces proof.
...
Implement SB27 only. Do not jump ahead. Preserve behavior and update proof before moving on.
