...
> **Handoff template.** Intended destination: `tools/dev/Migrate-PostgreSql16To18.md`.
> The implementing agent must adapt this text and add the exact safe command examples exercised against the final implementation before publishing it as a maintained runbook. This draft has not been executed on a developer workstation.
...
Also read the maintained repository guides `docs/operations/installed-web-app.md` and `docs/operations/backup-and-restore.md`, including the separate retained manual-client recovery section. The implementing agent must reconcile any conflicting older backup sequence so writers are quiesced before the coherent final capture.
