# SB053 - Split subprocess projection invocation behind subprocess facet
...
Split subprocess projection invocation behind subprocess facet
...
- Critical gates before SB053 must be passed.
...
5. Record proof in the execution report row for `SB053`.
...
- [ ] Execution report has a distinct `SB053` row.
...
Implement `SB053 - Split subprocess projection invocation behind subprocess facet` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB053` row.
