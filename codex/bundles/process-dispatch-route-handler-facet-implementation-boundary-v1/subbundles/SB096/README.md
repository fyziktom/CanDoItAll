# SB096 - Remove handler construction list from dispatcher partial
...
Remove handler construction list from dispatcher partial
...
- Critical gates before SB096 must be passed.
...
5. Record proof in the execution report row for `SB096`.
...
- [ ] Execution report has a distinct `SB096` row.
...
Implement `SB096 - Remove handler construction list from dispatcher partial` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB096` row.
