# SB016 - Critical Gate D: route-order assertion proof
...
Critical Gate D: route-order assertion proof
...
- Critical gates before SB016 must be passed.
...
5. Record proof in the execution report row for `SB016`.
...
- [ ] Execution report has a distinct `SB016` row.
...
Implement `SB016 - Critical Gate D: route-order assertion proof` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB016` row.
