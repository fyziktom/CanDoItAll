# SB080 - Add workflow handled and not-handled tests
...
Add workflow handled and not-handled tests
...
- Critical gates before SB080 must be passed.
...
5. Record proof in the execution report row for `SB080`.
...
- [ ] Execution report has a distinct `SB080` row.
...
Implement `SB080 - Add workflow handled and not-handled tests` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB080` row.
