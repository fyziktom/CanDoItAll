# SB054 - Move StartTransitionRouteHandler to top-level
...
Move StartTransitionRouteHandler to top-level
...
- Critical gates before SB054 must be passed.
...
5. Record proof in the execution report row for `SB054`.
...
- [ ] Execution report has a distinct `SB054` row.
...
Implement `SB054 - Move StartTransitionRouteHandler to top-level` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB054` row.
