# SB010 - Extract route context snapshot to top-level module-local type
...
Extract route context snapshot to top-level module-local type
...
- Critical gates before SB010 must be passed.
...
5. Record proof in the execution report row for `SB010`.
...
- [ ] Execution report has a distinct `SB010` row.
...
Implement `SB010 - Extract route context snapshot to top-level module-local type` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB010` row.
