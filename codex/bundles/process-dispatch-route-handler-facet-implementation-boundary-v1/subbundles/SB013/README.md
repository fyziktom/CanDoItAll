# SB013 - Replace nested result references in tests
...
Replace nested result references in tests
...
- Critical gates before SB013 must be passed.
...
5. Record proof in the execution report row for `SB013`.
...
- [ ] Execution report has a distinct `SB013` row.
...
Implement `SB013 - Replace nested result references in tests` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB013` row.
