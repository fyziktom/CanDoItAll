# SB086 - Critical Gate O: workflow/direct/finalizer parity
...
Critical Gate O: workflow/direct/finalizer parity
...
- Critical gates before SB086 must be passed.
...
5. Record proof in the execution report row for `SB086`.
...
- [ ] Execution report has a distinct `SB086` row.
...
Implement `SB086 - Critical Gate O: workflow/direct/finalizer parity` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB086` row.
