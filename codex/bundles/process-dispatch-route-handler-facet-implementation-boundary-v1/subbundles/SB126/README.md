# SB126 - Map route facets to future driver families without APIs
...
Map route facets to future driver families without APIs
...
- Critical gates before SB126 must be passed.
...
5. Record proof in the execution report row for `SB126`.
...
- [ ] Execution report has a distinct `SB126` row.
...
Implement `SB126 - Map route facets to future driver families without APIs` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB126` row.
