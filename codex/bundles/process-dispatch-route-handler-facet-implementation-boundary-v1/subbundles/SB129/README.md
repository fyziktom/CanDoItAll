# SB129 - Scan for production driver API tokens
...
Scan for production driver API tokens
...
- Critical gates before SB129 must be passed.
...
5. Record proof in the execution report row for `SB129`.
...
- [ ] Execution report has a distinct `SB129` row.
...
Implement `SB129 - Scan for production driver API tokens` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB129` row.
