# SB099 - Introduce route host adapter grouping facets
...
Introduce route host adapter grouping facets
...
- Critical gates before SB099 must be passed.
...
5. Record proof in the execution report row for `SB099`.
...
- [ ] Execution report has a distinct `SB099` row.
...
Implement `SB099 - Introduce route host adapter grouping facets` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB099` row.
