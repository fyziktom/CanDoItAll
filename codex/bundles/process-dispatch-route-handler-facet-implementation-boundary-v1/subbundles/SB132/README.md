# SB132 - Critical Gate W: no-core/no-driver checkpoint
...
Critical Gate W: no-core/no-driver checkpoint
...
- Critical gates before SB132 must be passed.
...
5. Record proof in the execution report row for `SB132`.
...
- [ ] Execution report has a distinct `SB132` row.
...
Implement `SB132 - Critical Gate W: no-core/no-driver checkpoint` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB132` row.
