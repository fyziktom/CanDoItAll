# SB056 - Critical Gate K: recovery/subprocess/start topology proof
...
Critical Gate K: recovery/subprocess/start topology proof
...
- Critical gates before SB056 must be passed.
...
5. Record proof in the execution report row for `SB056`.
...
- [ ] Execution report has a distinct `SB056` row.
...
Implement `SB056 - Critical Gate K: recovery/subprocess/start topology proof` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB056` row.
