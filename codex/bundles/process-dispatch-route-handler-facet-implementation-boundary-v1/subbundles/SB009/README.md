# SB009 - Extract route result vocabulary to top-level module-local type
...
Extract route result vocabulary to top-level module-local type
...
- Critical gates before SB009 must be passed.
...
5. Record proof in the execution report row for `SB009`.
...
- [ ] Execution report has a distinct `SB009` row.
...
Implement `SB009 - Extract route result vocabulary to top-level module-local type` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB009` row.
