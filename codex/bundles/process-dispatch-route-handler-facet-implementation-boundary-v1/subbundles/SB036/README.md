# SB036 - Narrow database handler dependencies to pre-execution/transition facets
...
Narrow database handler dependencies to pre-execution/transition facets
...
- Critical gates before SB036 must be passed.
...
5. Record proof in the execution report row for `SB036`.
...
- [ ] Execution report has a distinct `SB036` row.
...
Implement `SB036 - Narrow database handler dependencies to pre-execution/transition facets` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB036` row.
