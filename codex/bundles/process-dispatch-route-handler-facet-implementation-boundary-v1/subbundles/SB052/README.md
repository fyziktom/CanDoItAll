# SB052 - Narrow subprocess dependencies to subprocess/transition/finalizer facets
...
Narrow subprocess dependencies to subprocess/transition/finalizer facets
...
- Previous subbundles in phase `P5` must be complete.
- Critical gates before SB052 must be passed.
...
- Small, reviewable source/doc/test update matching the objective.

## Dependency Impact

- This subbundle is part of phase `P5: Recovery, subprocess and start route handler split`. Downstream route-handler extraction depends on this subbundle preserving route order and side-effect ownership.
...
5. Record proof in the execution report row for `SB052`.
...
- [ ] Execution report has a distinct `SB052` row.
...
Implement `SB052 - Narrow subprocess dependencies to subprocess/transition/finalizer facets` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB052` row.
