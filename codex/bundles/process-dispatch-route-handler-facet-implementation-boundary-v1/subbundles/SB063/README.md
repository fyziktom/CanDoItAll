# SB063 - Add start transition reload success tests
...
Add start transition reload success tests
...
- Critical gates before SB063 must be passed.
...
5. Record proof in the execution report row for `SB063`.
...
- [ ] Execution report has a distinct `SB063` row.
...
Implement `SB063 - Add start transition reload success tests` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB063` row.
