# SB070 - Narrow workflow dependencies to workflow/finalizer facets
...
Narrow workflow dependencies to workflow/finalizer facets
...
- Critical gates before SB070 must be passed.
...
5. Record proof in the execution report row for `SB070`.
...
- [ ] Execution report has a distinct `SB070` row.
...
Implement `SB070 - Narrow workflow dependencies to workflow/finalizer facets` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB070` row.
