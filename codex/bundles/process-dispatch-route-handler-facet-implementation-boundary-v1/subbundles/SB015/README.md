# SB015 - Add route-order assertion test for all stages
...
Add route-order assertion test for all stages
...
- Critical gates before SB015 must be passed.
...
5. Record proof in the execution report row for `SB015`.
...
- [ ] Execution report has a distinct `SB015` row.
...
Implement `SB015 - Add route-order assertion test for all stages` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB015` row.
