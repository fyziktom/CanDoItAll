# SB075 - Move RunClosedGuardRouteHandler to top-level
...
Move RunClosedGuardRouteHandler to top-level
...
- Critical gates before SB075 must be passed.
...
5. Record proof in the execution report row for `SB075`.
...
- [ ] Execution report has a distinct `SB075` row.
...
Implement `SB075 - Move RunClosedGuardRouteHandler to top-level` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB075` row.
