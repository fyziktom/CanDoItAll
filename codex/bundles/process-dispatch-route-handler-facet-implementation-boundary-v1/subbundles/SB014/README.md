# SB014 - Replace nested route context references in tests
...
Replace nested route context references in tests
...
- Critical gates before SB014 must be passed.
...
5. Record proof in the execution report row for `SB014`.
...
- [ ] Execution report has a distinct `SB014` row.
...
Implement `SB014 - Replace nested route context references in tests` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB014` row.
