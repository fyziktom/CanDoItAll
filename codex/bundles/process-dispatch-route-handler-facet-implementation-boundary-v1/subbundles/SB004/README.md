# SB004 - Critical Gate A: architecture guardrails before movement
...
Critical Gate A: architecture guardrails before movement
...
- Critical gates before SB004 must be passed.
...
5. Record proof in the execution report row for `SB004`.
...
- [ ] Execution report has a distinct `SB004` row.
...
Implement `SB004 - Critical Gate A: architecture guardrails before movement` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB004` row.
