# SB079 - Critical Gate N: workflow/direct/finalizer topology proof
...
Critical Gate N: workflow/direct/finalizer topology proof
...
- Critical gates before SB079 must be passed.
...
5. Record proof in the execution report row for `SB079`.
...
- [ ] Execution report has a distinct `SB079` row.
...
Implement `SB079 - Critical Gate N: workflow/direct/finalizer topology proof` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB079` row.
