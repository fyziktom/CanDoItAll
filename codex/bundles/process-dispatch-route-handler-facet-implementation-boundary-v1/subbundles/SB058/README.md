# SB058 - Add subprocess non-terminal observation tests
...
Add subprocess non-terminal observation tests
...
- Critical gates before SB058 must be passed.
...
5. Record proof in the execution report row for `SB058`.
...
- [ ] Execution report has a distinct `SB058` row.
...
Implement `SB058 - Add subprocess non-terminal observation tests` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB058` row.
