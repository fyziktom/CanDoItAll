# SB083 - Add run closed skip tests
...
- Critical gates before SB083 must be passed.
...
5. Record proof in the execution report row for `SB083`.
...
- [ ] Execution report has a distinct `SB083` row.
...
Implement `SB083 - Add run closed skip tests` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB083` row.
