# SB033 - Move FreshRecoverySkipRouteHandler to top-level
...
Move FreshRecoverySkipRouteHandler to top-level
...
- Critical gates before SB033 must be passed.
...
5. Record proof in the execution report row for `SB033`.
...
- [ ] Execution report has a distinct `SB033` row.
...
Implement `SB033 - Move FreshRecoverySkipRouteHandler to top-level` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB033` row.
