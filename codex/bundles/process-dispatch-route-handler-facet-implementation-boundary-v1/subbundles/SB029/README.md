# SB029 - Define competing/run-closed guard facet
...
Define competing/run-closed guard facet
...
- Previous subbundles in phase `P3` must be complete.
- Critical gates before SB029 must be passed.
...
- This subbundle is part of phase `P3: Route facet interface foundation`. Downstream route-handler extraction depends on this subbundle preserving route order and side-effect ownership.
...
5. Record proof in the execution report row for `SB029`.
...
- [ ] Execution report has a distinct `SB029` row.
...
Implement `SB029 - Define competing/run-closed guard facet` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB029` row.
