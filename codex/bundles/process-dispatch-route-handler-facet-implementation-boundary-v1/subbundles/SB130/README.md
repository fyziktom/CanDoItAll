# SB130 - Scan for Process Core project/tokens
...
Scan for Process Core project/tokens
...
- Critical gates before SB130 must be passed.
...
5. Record proof in the execution report row for `SB130`.
...
- [ ] Execution report has a distinct `SB130` row.
...
Implement `SB130 - Scan for Process Core project/tokens` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB130` row.
