# SB065 - Remove dispatcher constructor from mid-route handlers
...
Remove dispatcher constructor from mid-route handlers
...
- Critical gates before SB065 must be passed.
...
5. Record proof in the execution report row for `SB065`.
...
- [ ] Execution report has a distinct `SB065` row.
...
Implement `SB065 - Remove dispatcher constructor from mid-route handlers` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB065` row.
