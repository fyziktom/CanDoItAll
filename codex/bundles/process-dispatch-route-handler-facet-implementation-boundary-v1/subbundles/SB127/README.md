# SB127 - Map no-Core blockers after route handler split
...
Map no-Core blockers after route handler split
...
- Critical gates before SB127 must be passed.
...
5. Record proof in the execution report row for `SB127`.
...
- [ ] Execution report has a distinct `SB127` row.
...
Implement `SB127 - Map no-Core blockers after route handler split` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB127` row.
