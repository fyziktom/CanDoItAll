# SB037 - Move UpstreamMaterializationRouteHandler to top-level
...
Move UpstreamMaterializationRouteHandler to top-level
...
- Critical gates before SB037 must be passed.
...
5. Record proof in the execution report row for `SB037`.
...
- [ ] Execution report has a distinct `SB037` row.
...
Implement `SB037 - Move UpstreamMaterializationRouteHandler to top-level` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB037` row.
