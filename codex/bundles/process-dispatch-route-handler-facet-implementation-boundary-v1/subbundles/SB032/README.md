# SB032 - Critical Gate G: route facet adapter proof
...
Critical Gate G: route facet adapter proof
...
- Critical gates before SB032 must be passed.
...
5. Record proof in the execution report row for `SB032`.
...
- [ ] Execution report has a distinct `SB032` row.
...
Implement `SB032 - Critical Gate G: route facet adapter proof` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB032` row.
