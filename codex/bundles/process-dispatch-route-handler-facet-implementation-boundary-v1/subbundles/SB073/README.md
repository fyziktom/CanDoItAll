# SB073 - Move CompetingExecutionGuardRouteHandler to top-level
...
Move CompetingExecutionGuardRouteHandler to top-level
...
- Critical gates before SB073 must be passed.
...
5. Record proof in the execution report row for `SB073`.
...
- [ ] Execution report has a distinct `SB073` row.
...
Implement `SB073 - Move CompetingExecutionGuardRouteHandler to top-level` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB073` row.
