# SB094 - Wire handler factory from dispatcher facade
...
Wire handler factory from dispatcher facade
...
- Critical gates before SB094 must be passed.
...
5. Record proof in the execution report row for `SB094`.
...
- [ ] Execution report has a distinct `SB094` row.
...
Implement `SB094 - Wire handler factory from dispatcher facade` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB094` row.
