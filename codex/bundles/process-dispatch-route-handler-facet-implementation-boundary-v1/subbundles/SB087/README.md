# SB087 - Remove dispatcher constructor from late-route handlers
...
Remove dispatcher constructor from late-route handlers
...
- Critical gates before SB087 must be passed.
...
5. Record proof in the execution report row for `SB087`.
...
- [ ] Execution report has a distinct `SB087` row.
...
Implement `SB087 - Remove dispatcher constructor from late-route handlers` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB087` row.
