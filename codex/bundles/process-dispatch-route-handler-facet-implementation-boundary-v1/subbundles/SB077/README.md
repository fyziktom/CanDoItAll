# SB077 - Move FinalizerTransitionRouteHandler to top-level
...
Move FinalizerTransitionRouteHandler to top-level
...
- Critical gates before SB077 must be passed.
...
5. Record proof in the execution report row for `SB077`.
...
- [ ] Execution report has a distinct `SB077` row.
...
Implement `SB077 - Move FinalizerTransitionRouteHandler to top-level` exactly. Preserve route behavior and route order. Do not create Process Core or production driver APIs. Record proof in the execution report under a distinct `SB077` row.
