...
- Failing-first test: bundle://proof/SB14/transcripts/failing-first.txt records an adversarial negative source assertion for omitted or placeholder contract mapping.
- Passing test: bundle://proof/SB14/transcripts/passing.txt records ApiIntegrationTests.Api_nested_process_runtime_routes_preserve_typed_contract_state and the focused process validation suites.
...
- Red-team negative case: bundle://proof/SB14/transcripts/failing-first.txt verifies omitted mapping sentinels are absent and fails if the shallow pattern is searched as a required artifact.
