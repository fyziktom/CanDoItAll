# SB04 - Gate A - no Core/no driver/no UI baseline

## Status
Prepared.

## Objective
Gate A - no Core/no driver/no UI baseline.
...
5. Record proof in `proof/SB04/` if implementing this bundle in-repo.
...
Implement SB04 from `process-dispatch-projection-model-rule-decoupling-boundary-v1`. Preserve behavior and projection order. Do not introduce Process Core, production driver APIs, UI changes, or mobile proof artifacts.
