...
## Objective
Architecture guard for adapter-only nested model usage.
...
5. Record proof in `proof/SB07/` if implementing this bundle in-repo.
...
Implement SB07 from `process-dispatch-projection-model-rule-decoupling-boundary-v1`. Preserve behavior and projection order. Do not introduce Process Core, production driver APIs, UI changes, or mobile proof artifacts.
