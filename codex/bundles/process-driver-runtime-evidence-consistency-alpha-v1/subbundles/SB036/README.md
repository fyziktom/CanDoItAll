# SB036 — Gate L: prove corpus exercises real production parsers/verifiers, not fixture-only checks.
...
## Objective
Gate L: prove corpus exercises real production parsers/verifiers, not fixture-only checks.
...
- Phase: `P12 — Malicious Transcript And Descriptor Corpus`.

## Prerequisites
- Previous subbundle: `SB035` must be closed unless this is SB001.
...
- Anti-stub audit.
- `proof/SB036/manifest.md` with changed-file hashes, command transcripts, source assertions, anti-stub output, and red-team notes.
- `proof/SB036/semantic-invariants.md` with shallow-pass trap, adversarial negative proof, semantic positive proof, and raw-note closure.
...
- Critical closure manifest: `bundle://proof/SB036/manifest.md`.
...
Implement `SB036` from `bundle://subbundles/SB036/README.md`. Preserve hard boundaries, update proof, and stop if implementation requires runtime host, DI, manager command, process mutation, storage/workspace writes, shell/Graph calls, or UI changes.
