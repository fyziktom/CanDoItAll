# SB042 — Gate N: prove shared harness catches shallow/non-empty-output fake implementations.
...
## Objective
Gate N: prove shared harness catches shallow/non-empty-output fake implementations.
...
- Phase: `P14 — Shared Verification Test Harness`.

## Prerequisites
- Previous subbundle: `SB041` must be closed unless this is SB001.
...
- Anti-stub audit.
- `proof/SB042/manifest.md` with changed-file hashes, command transcripts, source assertions, anti-stub output, and red-team notes.
- `proof/SB042/semantic-invariants.md` with shallow-pass trap, adversarial negative proof, semantic positive proof, and raw-note closure.
...
- Critical closure manifest: `bundle://proof/SB042/manifest.md`.
...
Implement `SB042` from `bundle://subbundles/SB042/README.md`. Preserve hard boundaries, update proof, and stop if implementation requires runtime host, DI, manager command, process mutation, storage/workspace writes, shell/Graph calls, or UI changes.
