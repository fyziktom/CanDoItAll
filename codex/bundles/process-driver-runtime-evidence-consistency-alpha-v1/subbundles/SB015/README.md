# SB015 — Gate E: prove evidence/hash policy is shared, deterministic, and cannot read arbitrary files.
...
## Objective
Gate E: prove evidence/hash policy is shared, deterministic, and cannot read arbitrary files.
...
## Prerequisites
- Previous subbundle: `SB014` must be closed unless this is SB001.
...
- Anti-stub audit.
- `proof/SB015/manifest.md` with changed-file hashes, command transcripts, source assertions, anti-stub output, and red-team notes.
- `proof/SB015/semantic-invariants.md` with shallow-pass trap, adversarial negative proof, semantic positive proof, and raw-note closure.
...
- Critical closure manifest: `bundle://proof/SB015/manifest.md`.
...
Implement `SB015` from `bundle://subbundles/SB015/README.md`. Preserve hard boundaries, update proof, and stop if implementation requires runtime host, DI, manager command, process mutation, storage/workspace writes, shell/Graph calls, or UI changes.
