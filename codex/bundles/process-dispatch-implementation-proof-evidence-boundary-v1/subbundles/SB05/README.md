# SB05 - Implementation contract snapshot helper
...
Extract normalized contract text gathering into a local snapshot/helper.
...
- Previous subbundle closure gate passed: `SB04`.
...
- Update proof manifest under `proof/SB05/manifest.md`.
- Update semantic invariants under `proof/SB05/semantic-invariants.md`.
- Update execution report row.

## Dependency Impact

Downstream subbundle: `SB06`.
...
Existing wrappers must remain.

## Suggested Agent Prompt

Execute `SB05 - Implementation contract snapshot helper` from `codex/bundles/process-dispatch-implementation-proof-evidence-boundary-v1`. Stay inside the declared scope, update proof, and stop if any behavior parity test fails.
