# SB18 - Refactor Gate D: runnable/dotnet parity
...
Focused tests for host path discovery, JS bypass, dotnet negation, invalid host shape, run-after-mutation ordering.
...
- Previous subbundle closure gate passed: `SB17`.
...
- Update proof manifest under `proof/SB18/manifest.md`.
- Update semantic invariants under `proof/SB18/semantic-invariants.md`.
- Update execution report row.

## Dependency Impact

- Downstream subbundle: `SB19`.
...
Execute `SB18 - Refactor Gate D: runnable/dotnet parity` from `codex/bundles/process-dispatch-implementation-proof-evidence-boundary-v1`. Stay inside the declared scope, update proof, and stop if any behavior parity test fails.
