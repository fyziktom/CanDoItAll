# SB02 - Implementation proof source inventory
...
Map every method/region in ImplementationProof.cs and all consumers in ToolValidation, Execution, RecoveryPackets, ArtifactValidation.
...
- Previous subbundle closure gate passed: `SB01`.
...
- Update proof manifest under `proof/SB02/manifest.md`.
- Update semantic invariants under `proof/SB02/semantic-invariants.md`.
- Update execution report row.

## Dependency Impact

Downstream subbundle: `SB03`.
...
Inventory must include inputs/outputs/side effects.

## Suggested Agent Prompt

Execute `SB02 - Implementation proof source inventory` from `codex/bundles/process-dispatch-implementation-proof-evidence-boundary-v1`. Stay inside the declared scope, update proof, and stop if any behavior parity test fails.
