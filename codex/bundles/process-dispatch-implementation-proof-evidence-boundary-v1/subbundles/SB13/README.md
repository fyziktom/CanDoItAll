# SB13 - Refactor Gate C: receipt/path/mutation parity
...
Tests for path classification, read/mutation, scaffold, validation-after-mutation.
...
- Previous subbundle closure gate passed: `SB12`.
...
- Update proof manifest under `proof/SB13/manifest.md`.
- Update semantic invariants under `proof/SB13/semantic-invariants.md`.
- Update execution report row.

## Dependency Impact

Downstream subbundle: `SB14`.
...
Execute `SB13 - Refactor Gate C: receipt/path/mutation parity` from `codex/bundles/process-dispatch-implementation-proof-evidence-boundary-v1`. Stay inside the declared scope, update proof, and stop if any behavior parity test fails.
