# SB27 - Refactor Gate F: build, focused tests, line-count review
...
Full build, focused integration/unit tests, line count and source scans.
...
- Previous subbundle closure gate passed: `SB26`.
...
- Update proof manifest under `proof/SB27/manifest.md`.
- Update semantic invariants under `proof/SB27/semantic-invariants.md`.
- Update execution report row.

## Dependency Impact

Downstream subbundle: `SB28`.
...
Execute `SB27 - Refactor Gate F: build, focused tests, line-count review` from `codex/bundles/process-dispatch-implementation-proof-evidence-boundary-v1`. Stay inside the declared scope, update proof, and stop if any behavior parity test fails.
