# SB06 - Stack token and negation rules
...
Extract affirmative/negated .NET and JS stack detection, including token/pattern logic.
...
- Previous subbundle closure gate passed: `SB05`.
...
- Update proof manifest under `proof/SB06/manifest.md`.
- Update semantic invariants under `proof/SB06/semantic-invariants.md`.
- Update execution report row.

## Dependency Impact

- Downstream subbundle: `SB07`.
...
- Preserve negation behavior exactly.

## Suggested Agent Prompt

Execute `SB06 - Stack token and negation rules` from `codex/bundles/process-dispatch-implementation-proof-evidence-boundary-v1`. Stay inside the declared scope, update proof, and stop if any behavior parity test fails.
