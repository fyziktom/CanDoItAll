# SB05 - Subprocess Status And Transition Rule Helper
...
Extract pure subprocess status mapping and transition reason rules without moving side effects or finalizer calls.
...
- Command transcripts under `proof/SB05/transcripts/`.

## Dependency Impact

Feeds SB06. If this subbundle changes route/projection semantics, reopen the previous gate.
...
Implement SB05 only. Keep changes module-local. Preserve subprocess dispatch/projection behavior. Do not start Process Core or production driver APIs. Record proof before moving to the next subbundle.
