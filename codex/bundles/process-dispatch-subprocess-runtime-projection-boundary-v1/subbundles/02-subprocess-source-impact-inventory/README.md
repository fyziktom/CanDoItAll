# SB02 - Subprocess Source Impact Inventory
...
Classify every subprocess dispatch/projection method, query, transition, artifact write, journal write, and helper dependency in Dispatch.cs.
...
- Manifest under `proof/SB02/manifest.md`.
- Semantic invariants under `proof/SB02/semantic-invariants.md`.
- Command transcripts under `proof/SB02/transcripts/`.

## Dependency Impact

- Feeds SB03. If this subbundle changes route/projection semantics, reopen the previous gate.
...
Implement SB02 only. Keep changes module-local. Preserve subprocess dispatch/projection behavior. Do not start Process Core or production driver APIs. Record proof before moving to the next subbundle.
