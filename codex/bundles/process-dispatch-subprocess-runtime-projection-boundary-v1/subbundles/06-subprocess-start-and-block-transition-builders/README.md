# SB06 - Subprocess Start And Block Transition Builders
...
Extract subprocess start transition and capability-gap block transition request builders with exact field parity.
...
- Manifest under `proof/SB06/manifest.md`.
- Semantic invariants under `proof/SB06/semantic-invariants.md`.
- Command transcripts under `proof/SB06/transcripts/`.

## Dependency Impact

- Feeds SB07. If this subbundle changes route/projection semantics, reopen the previous gate.
...
Implement SB06 only. Keep changes module-local. Preserve subprocess dispatch/projection behavior. Do not start Process Core or production driver APIs. Record proof before moving to the next subbundle.
