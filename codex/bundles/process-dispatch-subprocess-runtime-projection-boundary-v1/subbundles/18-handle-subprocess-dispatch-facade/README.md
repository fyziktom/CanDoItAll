# SB18 - Handle Subprocess Dispatch Facade
...
Create a small module-local subprocess dispatch facade or partial so Dispatch.cs delegates the subprocess branch without changing route order.
...
- Manifest under `proof/SB18/manifest.md`.
- Semantic invariants under `proof/SB18/semantic-invariants.md`.
- Command transcripts under `proof/SB18/transcripts/`.

## Dependency Impact

Feeds SB19. If this subbundle changes route/projection semantics, reopen the previous gate.
...
Implement SB18 only. Keep changes module-local. Preserve subprocess dispatch/projection behavior. Do not start Process Core or production driver APIs. Record proof before moving to the next subbundle.
