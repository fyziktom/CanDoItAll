# SB07 - Subprocess Run Observation Coordinator Foundation
...
Introduce a module-local subprocess run observation coordinator around EnsureSubprocessRunForStepAsync without hiding side effects.
...
- Manifest under `proof/SB07/manifest.md`.
- Semantic invariants under `proof/SB07/semantic-invariants.md`.
- Command transcripts under `proof/SB07/transcripts/`.

## Dependency Impact

Feeds SB08. If this subbundle changes route/projection semantics, reopen the previous gate.
...
Implement SB07 only. Keep changes module-local. Preserve subprocess dispatch/projection behavior. Do not start Process Core or production driver APIs. Record proof before moving to the next subbundle.
