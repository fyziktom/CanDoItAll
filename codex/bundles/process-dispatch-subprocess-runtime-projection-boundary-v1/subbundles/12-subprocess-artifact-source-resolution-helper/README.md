# SB12 - Subprocess Artifact Source Resolution Helper
...
Wrap subprocess source artifact selection and mapping diagnostics behind a module-local helper while preserving WorkflowSubprocessArtifactMapper behavior.
...
- Manifest under `proof/SB12/manifest.md`.
- Semantic invariants under `proof/SB12/semantic-invariants.md`.
- Command transcripts under `proof/SB12/transcripts/`.

## Dependency Impact

- Feeds SB13. If this subbundle changes route/projection semantics, reopen the previous gate.
...
Implement SB12 only. Keep changes module-local. Preserve subprocess dispatch/projection behavior. Do not start Process Core or production driver APIs. Record proof before moving to the next subbundle.
