# SB15 - Subprocess Projection Writer Coordinator
...
Extract parent-scoped markdown file write plus artifact record/journal creation into a side-effect coordinator with structured outcome.
...
- Manifest under `proof/SB15/manifest.md`.
- Semantic invariants under `proof/SB15/semantic-invariants.md`.
- Command transcripts under `proof/SB15/transcripts/`.

## Dependency Impact

- Feeds SB16. If this subbundle changes route/projection semantics, reopen the previous gate.
...
Implement SB15 only. Keep changes module-local. Preserve subprocess dispatch/projection behavior. Do not start Process Core or production driver APIs. Record proof before moving to the next subbundle.
