...
- Live run `cf086486-2424-487b-bd29-bfc3c111f307` proved that correct HR assignment is not enough; runtime tool attachment and runtime tool enforcement must share the same effective process-scoped workspace profile.
...
- Underexposed or inconsistently exposed dev tools can also block process delivery: if an implementation agent sees scaffold/build/test tools that the host denies, it will improvise without proof and produce artifacts QA cannot consume.
...
- Runtime capability tests must cover both configured workspace tools and catalog `workspace-plugin`; proving only one path leaves process agents exposed to the same failure class.
...
- Any live or test evidence that a governed implementation step cannot attach scaffold/build/test/run tools from its process-selected software-development profile must reopen `06-tool-availability-profiles` and `09-process-flow-integration`.
