# SB08: MAF16 Agent Tool Skill Runtime Regression Closure

## Status

- Status: Completed

## Objective

- Close MAF/tool/skill proof debt from prior blockers.

## Covered Inputs

- RN08 maps to RQ08.
- Preserve the original bundle scope for this subbundle.

## Prerequisites

- SB04 completed.

## Exact Source References

- repo://src/CanDoItAll.AgentFramework.Maf/README.md
- repo://src/CanDoItAll.AgentFramework.Maf/CanDoItAll.AgentFramework.Maf.csproj
- repo://tests/CanDoItAll.Tests.Unit/Maf16CapabilityReflectionTests.cs
- repo://tests/CanDoItAll.Tests.Integration/MafAgentRuntimeTests.cs

## Deliverables

- Named proof slices for tool-loop, context provider, finalizer, errors, approvals, MCP, A2A, workflow mapping, and trace correlation.

## Dependency Impact

- SB15 and SB18 rely on stable proof categories for agent/runtime features.

## Validation Depth

- Deep runtime proof with transcripts per named slice.
- Include failing-first or adversarial proof when behavior changes, passing proof, source assertions, anti-stub audit, changed-file hashes, classification, and proof-debt closure status.

## Implementation Steps

- Inspect the referenced source and nearby tests.
- Implement the smallest correct change set for this subbundle only.
- Update proof artifacts under bundle://proof/SB08/.

## Do Not Do

- Do not hard-code Blazor, Tetris, project ids, run ids, local paths, or user-specific paths.
- Do not use docs-only changes to satisfy runtime proof requirements.

## Acceptance Checklist

- Deliverables are complete.
- Required tests or source assertions are recorded.
- Execution report gate rows are updated.

## Closure Evidence

- MAF README proof map: repo://src/CanDoItAll.AgentFramework.Maf/README.md
- MAF package surface: repo://src/CanDoItAll.AgentFramework.Maf/CanDoItAll.AgentFramework.Maf.csproj
- Runtime proof: repo://tests/CanDoItAll.Tests.Integration/MafAgentRuntimeTests.cs
- MAF symbol proof: repo://tests/CanDoItAll.Tests.Unit/Maf16CapabilityReflectionTests.cs
- A2A proof: repo://tests/CanDoItAll.Tests.Unit/A2ARemoteAgentToolFactoryTests.cs
- Handoff proof: repo://tests/CanDoItAll.Tests.Integration/MafAgentRuntimeHandoffTests.cs
- Manifest: bundle://proof/SB08/manifest.md
- Semantic invariants: bundle://proof/SB08/semantic-invariants.md
- Passing proof: bundle://proof/SB08/transcripts/passing.txt
- Adversarial proof: bundle://proof/SB08/transcripts/failing-first.txt
- Named slice proof: bundle://proof/SB08/transcripts/tool-loop.txt; bundle://proof/SB08/transcripts/context-provider.txt; bundle://proof/SB08/transcripts/finalizer.txt; bundle://proof/SB08/transcripts/errors.txt; bundle://proof/SB08/transcripts/approvals.txt; bundle://proof/SB08/transcripts/mcp.txt; bundle://proof/SB08/transcripts/a2a.txt; bundle://proof/SB08/transcripts/workflow-mapping.txt; bundle://proof/SB08/transcripts/trace-correlation.txt
- Browser validation: N/A; no Agent Framework UI markup, CSS, route, layout, or visible UI rendering component changed.

## Proof Required

- bundle://proof/SB08/manifest.md
- bundle://proof/SB08/semantic-invariants.md when the subbundle is critical or behavior-changing.
- Command transcripts under bundle://proof/SB08/transcripts/.

## Browser Validation Logging

- N/A unless Agent Framework UI changes.
- Update bundle://reviews/01-execution-report.md if browser proof is applicable.

## Progression Gate

- SB15 may rely on MAF proof only after each slice has a result.

## Suggested Agent Prompt

- Execute SB08 literally, preserve runtime genericity, and close owned proof before moving downstream.
