...
- SB17 follow-up repair captured desktop full-width Steps canvas proof, artifact Clone/Highlight context-menu proof, clone-as-shared-artifact-reference behavior, and a dev DB TetrisGame launch attempt. The TetrisGame launch is not E2E-ready in this branch: `/process/start` returns HTTP 410 `ProcessModuleRewriteInProgress` because the rebuilt project-structure launch/runtime application layer is still pending in later subbundles.
