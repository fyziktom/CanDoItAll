...
- If the selected subbundle touches Process APIs, project-scoped process integration, Codex skills, or final E2E scenarios, read `architecture/21-process-api-codex-skill-and-e2e-source-scenarios.md`, `analysis/09-final-e2e-project-structure-source-scenarios.md`, and `validation/07-final-e2e-source-scenario-validation.md`.
...
- Do not load final E2E scenarios through direct database edits, hidden test-only stores, or UI-only manual setup. Use typed public APIs and document the Codex skill workflow.
- Do not introduce `TetrisGame`, Tetris, recipe, issue-dashboard, invoice-approval, or other scenario-specific terms into generic Process code or broad software/.NET driver contracts.
