...
- Process API and final E2E scenario gaps: undocumented or missing typed API routes, missing Codex API skill updates, direct database scenario loading, test-only scenario bypasses, final closure using only the TetrisGame source, or scenario-specific vocabulary leaking into generic Process projects or broad software/.NET driver contracts.
