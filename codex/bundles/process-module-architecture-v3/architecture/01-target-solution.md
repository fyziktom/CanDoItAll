...
| `CanDoItAll.Processes.Drivers.Abstractions` | Driver package, catalog, capability, strategy factory, and facet contracts | UI, persistence, concrete drivers |
| `CanDoItAll.Processes.Projections` | UI/read-model projection DTOs and projection contracts | EF, runtime implementation, UI components |
| `CanDoItAll.Git` | Typed wrapper over Git executable | Process UI, Process runtime |
...
The authoritative v3 project order and dependency graph are in `architecture/11-project-boundary-and-dependency-map.md`. That file fixes the v2 ordering ambiguity by placing `Processes.Drivers.Abstractions` before `Processes.Builder` and adding `Processes.Projections` before UI rebuild.
