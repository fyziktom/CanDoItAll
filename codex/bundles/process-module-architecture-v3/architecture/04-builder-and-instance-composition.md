...
- In v3 ordering, `Processes.Drivers.Abstractions` is created before Builder because Builder consumes driver catalog and strategy factory contracts.
- Builder uses driver catalog contracts and registered descriptors, never concrete driver implementations.
- Plan persistence is invoked through application/runtime persistence ports; Builder does not reference EF/PostgreSQL implementation.

## v3 Detail References

- Correct project order and dependency graph: `architecture/11-project-boundary-and-dependency-map.md`.
- Persistence/event/outbox transaction model: `architecture/12-runtime-persistence-event-store-and-outbox.md`.
- Branch route table contract: `architecture/13-branch-switch-and-loop-contract.md`.
