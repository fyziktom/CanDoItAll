...
- Execution adapters classify diagnostics and restricted evidence as described in `architecture/16-execution-adapters-and-integration-boundaries.md`.
- Manager decisions, approval checks, and escalation flow are controlled through `architecture/14-manager-runtime-and-control-loop.md`.
- Git change audits must use `CanDoItAll.Git`; UI displays restricted links through projection contracts described in `architecture/15-ui-ux-projection-contracts-and-reuse-plan.md`.
