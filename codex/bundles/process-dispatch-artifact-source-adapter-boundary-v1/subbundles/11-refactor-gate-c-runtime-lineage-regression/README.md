# Refactor Gate C runtime lineage regression
...
Run focused and broad artifact regression, source scans, line-count review, and build before final red-team.

## Covered Inputs

- User request to continue small dispatcher isolation steps and avoid Process Core.
- Large-screen-only proof policy.
- Requirements: RQ-011, RQ-012.

## Prerequisites

SB10 closure gate must pass.
...
- Updated proof manifest under `proof/SB11/manifest.md`.
- Updated semantic invariants under `proof/SB11/semantic-invariants.md`.
...
- `proof/SB11/source-assertions/*.txt` or `.md`
...
SB12 may start only after this closure gate passes.

## Suggested Agent Prompt

Execute subbundle `11-refactor-gate-c-runtime-lineage-regression` exactly. Keep the change small, preserve behavior, record proof, and stop if any guardrail fails.
