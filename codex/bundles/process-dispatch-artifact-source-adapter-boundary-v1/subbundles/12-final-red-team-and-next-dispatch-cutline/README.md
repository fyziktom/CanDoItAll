# Final red-team and next dispatch cutline
...
Close bundle with hidden dependency scans, no-core/no-driver proof, no viewport proof drift, and next target recommendation.

## Covered Inputs

- User request to continue small dispatcher isolation steps and avoid Process Core.
- Large-screen-only proof policy.
- Requirements: RQ-001, RQ-012, RQ-013.

## Prerequisites

- SB11 closure gate must pass.
...
- Updated semantic invariants under `proof/SB12/semantic-invariants.md`.
...
- `proof/SB12/source-assertions/*.txt` or `.md`
...
Execute subbundle `12-final-red-team-and-next-dispatch-cutline` exactly. Keep the change small, preserve behavior, record proof, and stop if any guardrail fails.
