# Entry audit, artifact boundary baseline, previous smoke proof
...
Verify current branch state, preserve the latest execution/artifact boundary, and collect baseline line counts and proof.

## Covered Inputs

- User request to continue small dispatcher isolation steps and avoid Process Core.
- Large-screen-only proof policy.
- Requirements: RQ-001, RQ-012.
...
- Updated proof manifest under `proof/SB01/manifest.md`.
- Updated semantic invariants under `proof/SB01/semantic-invariants.md`.
...
- `proof/SB01/source-assertions/*.txt` or `.md`
...
Execute subbundle `01-entry-audit-artifact-boundary-baseline` exactly. Keep the change small, preserve behavior, record proof, and stop if any guardrail fails.
