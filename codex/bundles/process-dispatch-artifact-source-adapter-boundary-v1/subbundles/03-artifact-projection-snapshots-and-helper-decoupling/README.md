# Artifact projection snapshots and helper decoupling
...
Introduce local projection source snapshots and remove practical dependencies on dispatcher nested types from helpers.

## Covered Inputs

- User request to continue small dispatcher isolation steps and avoid Process Core.
- Large-screen-only proof policy.
- Requirements: RQ-003, RQ-004.
...
- Updated proof manifest under `proof/SB03/manifest.md`.
- Updated semantic invariants under `proof/SB03/semantic-invariants.md`.
...
- `proof/SB03/source-assertions/*.txt` or `.md`
...
- SB04 may start only after this closure gate passes.

## Suggested Agent Prompt

Execute subbundle `03-artifact-projection-snapshots-and-helper-decoupling` exactly. Keep the change small, preserve behavior, record proof, and stop if any guardrail fails.
