# SB023 - Scheduler/workflow read-only verification jobs: behavior implementation and focused tests
...
- `proof/SB023/semantic-invariants.md` if this is a critical gate or introduces production behavior.
- Command transcripts under `proof/SB023/transcripts/`.
- Changed-file hashes.
...
Implement SB023 for `Scheduler/workflow read-only verification jobs`. Use the exact source references above. Preserve generic Process Core and verification-only host constraints. Record source-backed proof before claiming completion.