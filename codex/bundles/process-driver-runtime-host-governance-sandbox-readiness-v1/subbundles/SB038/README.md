# SB038 - Dry-run sandbox and allow-list contracts: behavior implementation and focused tests

## Status
Prepared.

## Objective
Advance `Dry-run sandbox and allow-list contracts` without weakening the restored process runtime, generic Process Core, or verification-only host boundary.
...
## Deliverables
- Source changes or proof artifacts for `Dry-run sandbox and allow-list contracts`.
...
## Dependency Impact
This subbundle is part of P13. If it is wrong, all downstream subbundles in later phases must be reopened because runtime-host governance and safety assumptions may be invalid.
...
5. Record proof under `proof/SB038/`.
...
- `proof/SB038/semantic-invariants.md` if this is a critical gate or introduces production behavior.
- Command transcripts under `proof/SB038/transcripts/`.
- Changed-file hashes.
...
Implement SB038 for `Dry-run sandbox and allow-list contracts`. Use the exact source references above. Preserve generic Process Core and verification-only host constraints. Record source-backed proof before claiming completion.