# SB017 - Manager API/readback facade: behavior implementation and focused tests

## Status
- Status: `Completed`

## Objective
Advance `Manager API/readback facade` without weakening the restored process runtime, generic Process Core, or verification-only host boundary.
...
## Deliverables
- Source changes or proof artifacts for `Manager API/readback facade`.
...
## Dependency Impact
- This subbundle is part of P06. If it is wrong, all downstream subbundles in later phases must be reopened because runtime-host governance and safety assumptions may be invalid.
...
## Proof Required
- `proof/SB017/manifest.md`
- `proof/SB017/semantic-invariants.md` if this is a critical gate or introduces production behavior.
- Command transcripts under `proof/SB017/transcripts/`.
- Changed-file hashes.
...
Implement SB017 for `Manager API/readback facade`. Use the exact source references above. Preserve generic Process Core and verification-only host constraints. Record source-backed proof before claiming completion.