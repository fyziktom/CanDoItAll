# SB008 - Async host and structured denial hardening: behavior implementation and focused tests

## Status
- Status: `Completed`

## Objective
Advance `Async host and structured denial hardening` without weakening the restored process runtime, generic Process Core, or verification-only host boundary.
...
## Deliverables
- Source changes or proof artifacts for `Async host and structured denial hardening`.
...
## Dependency Impact
- This subbundle is part of P03. If it is wrong, all downstream subbundles in later phases must be reopened because runtime-host governance and safety assumptions may be invalid.
...
## Proof Required
- `proof/SB008/manifest.md`
- `proof/SB008/semantic-invariants.md` if this is a critical gate or introduces production behavior.
- Command transcripts under `proof/SB008/transcripts/`.
- Changed-file hashes.
...
Implement SB008 for `Async host and structured denial hardening`. Use the exact source references above. Preserve generic Process Core and verification-only host constraints. Record source-backed proof before claiming completion.