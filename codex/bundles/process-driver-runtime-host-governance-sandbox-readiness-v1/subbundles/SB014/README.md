# SB014 - Exact registry/selector and lane options: behavior implementation and focused tests

## Status
Prepared.

## Objective
Advance `Exact registry/selector and lane options` without weakening the restored process runtime, generic Process Core, or verification-only host boundary.
...
## Deliverables
- Source changes or proof artifacts for `Exact registry/selector and lane options`.
...
## Dependency Impact
This subbundle is part of P05. If it is wrong, all downstream subbundles in later phases must be reopened because runtime-host governance and safety assumptions may be invalid.
...
- `proof/SB014/semantic-invariants.md` if this is a critical gate or introduces production behavior.
- Command transcripts under `proof/SB014/transcripts/`.
- Changed-file hashes.
...
Implement SB014 for `Exact registry/selector and lane options`. Use the exact source references above. Preserve generic Process Core and verification-only host constraints. Record source-backed proof before claiming completion.