# SB10 - Provider-native browser working-directory and output matching helper
...
Provider-native browser working-directory and output matching helper.
...
- Extract provider-native browser evidence facts and safe probe suppression helpers.
## Dependency Impact
...
6. Record proof under `proof/SB10/` during execution.
...
- Do not start SB11 until this subbundle's acceptance checklist is complete. Critical gates must include an explicit downstream dependency decision.
## Suggested Agent Prompt

Implement SB10 from `process-dispatch-artifact-validation-residual-boundary-v1`. Follow the exact source references and do not broaden scope. Preserve behavior, keep changes module-local, and record the required proof artifacts before proceeding.
