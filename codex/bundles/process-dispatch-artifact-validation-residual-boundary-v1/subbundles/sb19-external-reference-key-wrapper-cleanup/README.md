# SB19 - External reference key wrapper cleanup
...
External reference key wrapper cleanup.
...
6. Record proof under `proof/SB19/` during execution.
...
Do not start SB20 until this subbundle's acceptance checklist is complete. Critical gates must include an explicit downstream dependency decision.

## Suggested Agent Prompt

Implement SB19 from `process-dispatch-artifact-validation-residual-boundary-v1`. Follow the exact source references and do not broaden scope. Preserve behavior, keep changes module-local, and record the required proof artifacts before proceeding.
