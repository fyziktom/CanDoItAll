# SB18 - Technical-agent binding diagnostic helper
...
Technical-agent binding diagnostic helper.
...
- Extract execution artifact metadata, diagnostic, and external-reference helper responsibilities.
## Dependency Impact
...
6. Record proof under `proof/SB18/` during execution.
...
- Do not start SB19 until this subbundle's acceptance checklist is complete. Critical gates must include an explicit downstream dependency decision.
## Suggested Agent Prompt

Implement SB18 from `process-dispatch-artifact-validation-residual-boundary-v1`. Follow the exact source references and do not broaden scope. Preserve behavior, keep changes module-local, and record the required proof artifacts before proceeding.
