# SB11 - Browser output file probe failure suppression helper
...
Browser output file probe failure suppression helper.
...
6. Record proof under `proof/SB11/` during execution.
...
Do not start SB12 until this subbundle's acceptance checklist is complete. Critical gates must include an explicit downstream dependency decision.

## Suggested Agent Prompt

Implement SB11 from `process-dispatch-artifact-validation-residual-boundary-v1`. Follow the exact source references and do not broaden scope. Preserve behavior, keep changes module-local, and record the required proof artifacts before proceeding.
