...
- Invariant ID: SB16-CRITICAL-FAILURE-PARITY
- Source raw note: Extract critical tool failure suppression without changing completion status.
- Expected behavior: Recovered scaffold and superseded denied critical tool suppression use a typed context helper and the existing redundant-denied validation tests pass.
...
- Changed source files: repo://src/CanDoItAll.Modules.Processes/Automation/Dispatch/ProcessRunAutomationDispatchService.ArtifactValidation.cs plus helper files listed in bundle://proof/SB16/manifest.md.
