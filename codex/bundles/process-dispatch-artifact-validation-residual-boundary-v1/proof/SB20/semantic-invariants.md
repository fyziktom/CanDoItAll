...
- Invariant ID: SB20-METADATA-PARITY
- Source raw note: Preserve metadata, storage path, external key, trust status, and diagnostics.
- Expected behavior: Completed decision keys, trust mapping, storage-relative paths, workspace-written source paths, and technical-agent diagnostics are delegated with parity tests passing.
...
- Changed source files: repo://src/CanDoItAll.Modules.Processes/Automation/Dispatch/ProcessRunAutomationDispatchService.ArtifactValidation.cs plus helper files listed in bundle://proof/SB20/manifest.md.
