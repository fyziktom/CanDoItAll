# SB004 - Route service adapter burn-down: pre-execution services
...
Move database requirement and upstream materialization route service logic from dispatcher forwarding into explicit application services where safe; keep EF/journal writes application-local.
...
7. If this is a critical gate, create `proof/SB004/manifest.md` and `proof/SB004/semantic-invariants.md`.
...
Implement SB004: Route service adapter burn-down: pre-execution services. Preserve behavior, avoid Process Core and production driver APIs, update proof artifacts, and close the execution report row.
