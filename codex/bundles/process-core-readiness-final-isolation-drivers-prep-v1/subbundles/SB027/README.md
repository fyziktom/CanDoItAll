...
Run build/tests/scans, write final red-team, close every raw note, and explicitly recommend whether next bundle can start Core or needs one more application-boundary pass.

## Covered Inputs

- Raw user request in `inputs/raw-user-request.md`.
- Current-state analysis in `analysis/01-current-state-review.md`.
- Phase: P9 - Core/driver readiness closure
...
- Critical foundation gate for downstream phases.
- Downstream phases become untrustworthy if this subbundle changes behavior without artifact-backed proof.
## Validation Depth
...
7. If this is a critical gate, create `proof/SB027/manifest.md` and `proof/SB027/semantic-invariants.md`.
...
Implement SB027: Final red-team, proof closure, and next bundle cutline. Preserve behavior, avoid Process Core and production driver APIs, update proof artifacts, and close the execution report row.
