# SB022 - Route model adapter contraction
...
Reduce `ProcessDispatchRouteModelAdapters` to the smallest dispatcher-edge bridge; no route handler/service should need dispatcher source payload except where explicitly justified.
...
7. If this is a critical gate, create `proof/SB022/manifest.md` and `proof/SB022/semantic-invariants.md`.
...
Implement SB022: Route model adapter contraction. Preserve behavior, avoid Process Core and production driver APIs, update proof artifacts, and close the execution report row.
