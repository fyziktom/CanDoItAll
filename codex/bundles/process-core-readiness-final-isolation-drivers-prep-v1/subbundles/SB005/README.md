# SB005 - Route service adapter burn-down: workflow/direct/guard services
...
Split workflow, direct-agent, competing-execution, and run-closed services so route services no longer act as a single dispatcher adapter.
...
7. If this is a critical gate, create `proof/SB005/manifest.md` and `proof/SB005/semantic-invariants.md`.
...
Implement SB005: Route service adapter burn-down: workflow/direct/guard services. Preserve behavior, avoid Process Core and production driver APIs, update proof artifacts, and close the execution report row.
