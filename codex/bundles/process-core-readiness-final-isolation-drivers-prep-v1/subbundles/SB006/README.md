# SB006 - Gate B: route service ownership proof
...
Prove route services are narrow, route handler order is unchanged, and dispatcher method calls are reduced to explicit application ports only.

## Covered Inputs

- Raw user request in `inputs/raw-user-request.md`.
- Current-state analysis in `analysis/01-current-state-review.md`.
- Phase: P2 - Route adapter burn-down
...
7. If this is a critical gate, create `proof/SB006/manifest.md` and `proof/SB006/semantic-invariants.md`.
...
Implement SB006: Gate B: route service ownership proof. Preserve behavior, avoid Process Core and production driver APIs, update proof artifacts, and close the execution report row.
