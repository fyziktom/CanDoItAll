...
Phase8 improved many things enough that a real test is now plausible, but not yet safe without one more readiness pass.
...
- The process API skill now documents key governance fields and the Tetris checklist.
- A Tetris baseline scenario exists in `baseline-scenarios.json`.

## Gaps

- The Tetris baseline scenario is seeded with transitions and artifacts. That is not the same as a live agent-executed process.
