    ...
    public static async Task Main(string[] args) {
        ...
        await using (var scope = services.CreateAsyncScope()) {
            ...
            await workspace.SaveAgentTeamAsync(new AgentTeamEditorModel {
                Id = Guid.Parse("580e7395-0890-43ad-b27e-e19f0369d571"), Name = "Product research and synthesis", Description = "Safe fixture"
            });
            await workspace.SaveAgentTeamAsync(new AgentTeamEditorModel {
                Id = Guid.Parse("2ac59e8b-f96c-486e-a9ce-8a3a3d952fce"), Name = "Delivery governance", Description = "Safe fixture"
            });
        }
...
internal sealed class OverviewUsage(OverviewFixture fixture, ProviderUsageWorkloadKind kind) : IProviderUsageProjectionSource {
    ...
    public async ValueTask<ProviderUsageSourceResult> ReadAsync(CancellationToken cancellationToken = default) {
        ...
        var rows = fixture.Mode == OverviewMode.Empty ? [] : Enumerable.Range(1, 5).Select(index => new ProviderUsageContribution(
            ...
            ProviderKind.OpenAi, fixture.Mode == OverviewMode.Long ? "Long model for multilingual research and technical delivery with an extensive descriptive version label" : "Representative model", $"{kind}-{index}", index == 2 ? ProviderUsageExecutionOutcome.Failed : ProviderUsageExecutionOutcome.Succeeded,
