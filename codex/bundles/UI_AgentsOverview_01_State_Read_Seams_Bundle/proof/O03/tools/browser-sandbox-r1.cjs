...
const output = path.join(root, '.mcp-state/overview03/browser-sandbox');
...
async function run(mode, port) {
    ...
    try {
        ...
        assert((await page.getByTestId('sandbox-intent').innerText()).includes('No data-loading dialog opens'));
        ...
        assert((await page.getByTestId('sandbox-intent').innerText()).includes('No production navigation'));
