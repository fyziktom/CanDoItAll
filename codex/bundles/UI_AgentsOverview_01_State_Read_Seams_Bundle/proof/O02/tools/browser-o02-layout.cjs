...
const out = path.join(root, '.mcp-state/overview-execution/browser-o02-layout');
...
(async () => {
 ...
 for (const mode of ['Normal', 'Long']) {
  ...
  if (mode === 'Long') assert((await page.getByTestId('agents-overview-top-consumers').innerText()).includes('Long'));
  if (mode === 'Long') assert((await page.getByTestId('agents-overview-team-shortcut').first().getAttribute('title')).includes('interdisciplinary'));
  await page.waitForTimeout(1200);
  fs.writeFileSync(path.join(out, mode + '-geometry.json'), JSON.stringify(await page.locator('.apexcharts-legend').evaluateAll(elements => elements.map(e => ({ legend: e.getBoundingClientRect().toJSON(), children: [...e.children].map(c=>c.getBoundingClientRect().toJSON()), ancestors: [...(function*(n){for(let i=0;n&&i<6;i++,n=n.parentElement)yield n})(e.parentElement)].map(c=>({tag:c.tagName,cls:c.className,rect:c.getBoundingClientRect().toJSON(),overflow:getComputedStyle(c).overflow})) }))), null, 2));
 ...
 result.outcome = 'O02 layout probe: captured normal and long rendered geometry for inspection.';
