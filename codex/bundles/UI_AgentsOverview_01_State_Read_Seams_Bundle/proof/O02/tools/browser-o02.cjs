...
const out = path.join(root, '.mcp-state/overview-execution/browser-o02');
...
(async () => {
 ...
 for (const [kind, openId, shellId, contentId] of [
  ...
  await until(async () => await shell.locator('.apexcharts-canvas svg').count() > 0, kind + ' actual chart');
