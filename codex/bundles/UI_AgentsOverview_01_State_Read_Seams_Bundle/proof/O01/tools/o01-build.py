...
def run(args,name):
 ...
 records.append({'name':name,'args':args,'exit':r.returncode,'utc':start}); (out/'o01-builds.json').write_text(json.dumps(records,indent=2))
...
for p in projects:run(['dotnet','build',p,'-c','Release','--no-restore','/m:1','/p:UseSharedCompilation=false'],'o01-build-'+Path(p).stem)
for g in groups:
 name='o01-discovery-'+Path(g['project']).stem
...
(out/'o01-discovery.json').write_text(json.dumps(groups,indent=2))
