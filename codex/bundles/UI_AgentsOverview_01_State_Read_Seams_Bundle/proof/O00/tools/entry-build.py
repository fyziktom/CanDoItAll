...
def run(args,name):
 ...
 records.append({'name':name,'args':args,'exit':r.returncode,'utc':start}); (out/'entry-builds.json').write_text(json.dumps(records,indent=2))
...
for p in projects:run(['dotnet','build',p,'-c','Release','--no-restore','/m:1','/p:UseSharedCompilation=false'],'entry-build-'+Path(p).stem)
for g in groups:
 name='entry-discovery-'+Path(g['project']).stem
...
(out/'entry-discovery.json').write_text(json.dumps(groups,indent=2))
