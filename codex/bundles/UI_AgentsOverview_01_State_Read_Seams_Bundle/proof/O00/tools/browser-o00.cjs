...
const out = path.join(root, '.mcp-state/overview-execution/browser-o00');
...
(async () => {
 ...
 for (const mode of ['Normal', 'Partial', 'Empty', 'HoldOverview', 'OverviewFailure', 'UsageFailure', 'HeaderFailure', 'BoundFailure']) {
 ...
 result.outcome = 'Observed current behavior; failures are characterized, not accepted.';
