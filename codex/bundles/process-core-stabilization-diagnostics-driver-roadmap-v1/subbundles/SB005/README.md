# SB005 - Core API compatibility guard

## Status
- Status: `Completed`

## Objective
Add architecture/API snapshot tests for public Core surface.
...
## Deliverables
- Tests fail if Core public API grows outside approved namespaces.
...
5. Record proof under `proof/SB005/`.
...
## Progression Gate
- SB005 closes only when proof is recorded and downstream dependency impact is reviewed.

## Suggested Agent Prompt
Implement `SB005 - Core API compatibility guard` exactly as specified. Preserve all process runtime behavior. Keep production driver APIs out of scope. Record proof before moving on.

## Closure Proof
- Entry gate: Passed after SB004 inventory.
- Closure gate: Passed. API guard implemented in `repo://tests/CanDoItAll.Tests.Unit/ProcessAgentExecutionBoundaryArchitectureTests.cs`.
- Downstream check: SB006 owns build/test/source-scan gate.
