...
## Objective
Close Core API stabilization phase.
...
## Deliverables
- Core public API snapshot, docs, and dependency guard pass.
...
## Validation Depth
- Required proof: build + API guard.
...
5. Record proof under `proof/SB006/`.
...
## Proof Required
- Critical proof must include `bundle://proof/SB006/manifest.md` with changed-file hashes, command transcripts, source assertions, anti-stub audit output, and portable references.
- Critical proof must include `bundle://proof/SB006/semantic-invariants.md` with Semantic Adequacy Gate coverage: shallow-pass trap, adversarial negative proof, semantic positive proof, anti-stub audit, and raw-note literal closure.
...
## Progression Gate
- SB006 closes only when proof is recorded and downstream dependency impact is reviewed.

## Suggested Agent Prompt
Implement `SB006 - Gate B - Core API stability proof` exactly as specified. Preserve all process runtime behavior. Keep production driver APIs out of scope. Record proof before moving on.

## Closure Proof
- Entry gate: Passed after SB004 and SB005 completion.
- Closure gate: Passed. See `bundle://proof/SB006/manifest.md` and `bundle://proof/SB006/semantic-invariants.md`.
- Downstream check: SB007-SB009 diagnostics phase may proceed.
