...
Implement the `Add focused runtime tests for workflow-backed role and direct-agent route using deterministic/fake execution.` slice as part of `P08: MAF/workflow/direct-agent execution compatibility`.
...
Implement `SB023 — Add focused runtime tests for workflow-backed role and direct-agent route using deterministic/fake execution.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
