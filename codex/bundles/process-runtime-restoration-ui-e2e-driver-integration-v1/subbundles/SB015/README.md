...
Implement the `Gate E: UI process-start smoke passes; no small/medium/mobile proof.` slice as part of `P05: UI process-start E2E skeleton`.
...
Create `proof/SB015/manifest.md` and `proof/SB015/semantic-invariants.md` with shallow-pass trap, adversarial negative proof, semantic positive proof, anti-stub audit, raw-note closure, changed-file hashes, command transcripts, and production behavior artifact matrix if new signals are introduced.
...
Implement `SB015 — Gate E: UI process-start smoke passes; no small/medium/mobile proof.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
