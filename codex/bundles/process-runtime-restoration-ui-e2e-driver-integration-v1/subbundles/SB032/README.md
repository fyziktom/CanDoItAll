...
Implement the `Attach verification observations as diagnostics or read-only evidence envelope only when explicitly requested.` slice as part of `P11: Read-only driver verification in process manager path`.
...
Implement `SB032 — Attach verification observations as diagnostics or read-only evidence envelope only when explicitly requested.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
