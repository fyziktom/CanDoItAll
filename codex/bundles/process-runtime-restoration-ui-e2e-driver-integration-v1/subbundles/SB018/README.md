...
Implement the `Gate F: process run creation is reliable and generic.` slice as part of `P06: Process run creation and persistence`.
...
Create `proof/SB018/manifest.md` and `proof/SB018/semantic-invariants.md` with shallow-pass trap, adversarial negative proof, semantic positive proof, anti-stub audit, raw-note closure, changed-file hashes, command transcripts, and production behavior artifact matrix if new signals are introduced.
...
Implement `SB018 — Gate F: process run creation is reliable and generic.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
