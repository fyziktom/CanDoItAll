...
Implement the `Add safe test-only/manual process trigger path for scheduled or workflow-initiated process start.` slice as part of `P13: Scheduler and workflow launch readiness`.
...
Implement `SB038 — Add safe test-only/manual process trigger path for scheduled or workflow-initiated process start.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
