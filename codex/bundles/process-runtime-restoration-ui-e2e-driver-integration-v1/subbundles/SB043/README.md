...
Implement the `Scan generic Core/runtime for `.NET`, software-only, Office-only, business-only domain leakage.` slice as part of `P15: Process Core genericity audit`.
...
Implement `SB043 — Scan generic Core/runtime for `.NET`, software-only, Office-only, business-only domain leakage.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
