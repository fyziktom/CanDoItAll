...
Implement the `Inventory existing scheduler/planner/workflow trigger hooks for processes, not drivers.` slice as part of `P13: Scheduler and workflow launch readiness`.
...
Implement `SB037 — Inventory existing scheduler/planner/workflow trigger hooks for processes, not drivers.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
