...
Implement the `Run solution build, full unit, focused integration, Playwright large-screen UI process-start, and scenario smoke matrix.` slice as part of `P16: Release candidate smoke matrix`.
...
Implement `SB046 — Run solution build, full unit, focused integration, Playwright large-screen UI process-start, and scenario smoke matrix.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
