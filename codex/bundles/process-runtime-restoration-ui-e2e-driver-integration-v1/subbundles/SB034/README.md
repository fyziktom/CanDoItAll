...
Implement the `Split remaining large read-only orchestration files where necessary and enforce lane-specific builders/adapters.` slice as part of `P12: Process-level read-only orchestration hardening`.
...
Implement `SB034 — Split remaining large read-only orchestration files where necessary and enforce lane-specific builders/adapters.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
