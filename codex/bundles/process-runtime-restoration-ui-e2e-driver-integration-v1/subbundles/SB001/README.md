...
Implement the `Re-read current branch, latest commit, latest execution report, and source hotspots.` slice as part of `P01: Crash/source reconciliation and bundle-path test contamination inventory`.
...
Implement `SB001 — Re-read current branch, latest commit, latest execution report, and source hotspots.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
