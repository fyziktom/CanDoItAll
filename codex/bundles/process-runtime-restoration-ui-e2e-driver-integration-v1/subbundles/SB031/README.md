...
Implement the `Add process-manager diagnostic observation using supplied evidence only; no transition/finalizer mutation.` slice as part of `P11: Read-only driver verification in process manager path`.
...
Implement `SB031 — Add process-manager diagnostic observation using supplied evidence only; no transition/finalizer mutation.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
