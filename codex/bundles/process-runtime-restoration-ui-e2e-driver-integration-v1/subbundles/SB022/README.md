...
Implement the `Inventory current MAF workflow/direct-agent executor integration and fake-provider options.` slice as part of `P08: MAF/workflow/direct-agent execution compatibility`.
...
Implement `SB022 — Inventory current MAF workflow/direct-agent executor integration and fake-provider options.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
