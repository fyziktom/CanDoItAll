...
Implement the `Verify route execution, finalizer, state transition, and artifact validation paths still work with deterministic/fake executor.` slice as part of `P07: Dispatch/claim/route/finalizer runtime restoration`.
...
Implement `SB020 — Verify route execution, finalizer, state transition, and artifact validation paths still work with deterministic/fake executor.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
