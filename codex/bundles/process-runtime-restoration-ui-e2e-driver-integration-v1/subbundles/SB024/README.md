...
Implement the `Gate H: MAF/process integration is not broken by Core/driver refactors.` slice as part of `P08: MAF/workflow/direct-agent execution compatibility`.
...
Create `proof/SB024/manifest.md` and `proof/SB024/semantic-invariants.md` with shallow-pass trap, adversarial negative proof, semantic positive proof, anti-stub audit, raw-note closure, changed-file hashes, command transcripts, and production behavior artifact matrix if new signals are introduced.
...
Implement `SB024 — Gate H: MAF/process integration is not broken by Core/driver refactors.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
