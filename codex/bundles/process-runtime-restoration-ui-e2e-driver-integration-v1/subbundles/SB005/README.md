...
Implement the `Move any required durable fixture text to `tests/TestData/Architecture` or stable docs, not `codex/bundles`.` slice as part of `P02: Remove bundle-path coupling from tests`.
...
Implement `SB005 — Move any required durable fixture text to `tests/TestData/Architecture` or stable docs, not `codex/bundles`.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
