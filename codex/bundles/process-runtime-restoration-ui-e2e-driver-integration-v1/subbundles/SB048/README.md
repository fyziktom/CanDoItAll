...
Implement the `Gate P: release-candidate matrix passes.` slice as part of `P16: Release candidate smoke matrix`.
...
- Create `proof/SB048/manifest.md` and `proof/SB048/semantic-invariants.md` with shallow-pass trap, adversarial negative proof, semantic positive proof, anti-stub audit, raw-note closure, changed-file hashes, command transcripts, and production behavior artifact matrix if new signals are introduced.

## Closure Proof

- `bundle://proof/SB048/manifest.md`
- `bundle://proof/SB048/semantic-invariants.md`
...
- Required: large desktop Playwright proof for process UI route and process-start behavior.
...
Implement `SB048 — Gate P: release-candidate matrix passes.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
