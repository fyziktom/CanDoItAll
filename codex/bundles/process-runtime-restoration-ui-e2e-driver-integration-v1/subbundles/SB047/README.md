...
Implement the `Run source scans for bundle-path coupling, runtime-host drift, Core reverse dependency, driver mutation, UI/media drift.` slice as part of `P16: Release candidate smoke matrix`.
...
Implement `SB047 — Run source scans for bundle-path coupling, runtime-host drift, Core reverse dependency, driver mutation, UI/media drift.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
