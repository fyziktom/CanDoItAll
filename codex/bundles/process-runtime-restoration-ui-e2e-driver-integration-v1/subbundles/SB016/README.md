...
Implement the `Verify run/step creation persistence, statuses, process-run ownership, project association, and input payload persistence.` slice as part of `P06: Process run creation and persistence`.
...
Implement `SB016 — Verify run/step creation persistence, statuses, process-run ownership, project association, and input payload persistence.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
