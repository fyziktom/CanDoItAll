...
Implement the `Inventory every test/source reference to `codex/bundles/` and classify stable vs transient.` slice as part of `P01: Crash/source reconciliation and bundle-path test contamination inventory`.
...
Implement `SB002 — Inventory every test/source reference to `codex/bundles/` and classify stable vs transient.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
