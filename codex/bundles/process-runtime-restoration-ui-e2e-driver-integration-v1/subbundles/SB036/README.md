...
Implement the `Gate L: read-only orchestration remains bounded and maintainable.` slice as part of `P12: Process-level read-only orchestration hardening`.
...
- Create `proof/SB036/manifest.md` and `proof/SB036/semantic-invariants.md` with shallow-pass trap, adversarial negative proof, semantic positive proof, anti-stub audit, raw-note closure, changed-file hashes, command transcripts, and production behavior artifact matrix if new signals are introduced.
...
Implement `SB036 — Gate L: read-only orchestration remains bounded and maintainable.`. Preserve all hard constraints. Produce source-backed proof and update the execution report.
