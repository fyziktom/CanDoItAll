...
## Objective
Gate J: aggregation remains read-only and immutable
...
- `proof/SB030/semantic-invariants.md` for critical gates
...
Implement `SB030 - Gate J: aggregation remains read-only and immutable` exactly as specified. Preserve the read-only driver boundary and record artifact-backed proof before marking the row complete.
