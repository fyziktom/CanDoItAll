# SB032 - Add fake-proof rejection tests for non-empty-diagnostic-only closures

## Status
Prepared for Codex implementation. Standard phase work.

## Objective
Add fake-proof rejection tests for non-empty-diagnostic-only closures
...
- `proof/SB032/semantic-invariants.md` for critical gates
...
Implement `SB032 - Add fake-proof rejection tests for non-empty-diagnostic-only closures` exactly as specified. Preserve the read-only driver boundary and record artifact-backed proof before marking the row complete.
