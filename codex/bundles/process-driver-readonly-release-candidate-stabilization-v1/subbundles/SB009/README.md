...
## Objective
Gate C: split adapters preserve behavior and no-side-effect scans
...
- `proof/SB009/semantic-invariants.md` for critical gates
...
Implement `SB009 - Gate C: split adapters preserve behavior and no-side-effect scans` exactly as specified. Preserve the read-only driver boundary and record artifact-backed proof before marking the row complete.
