# SB02 - Projection facet implementation method inventory
...
Inventory every method in `ProcessRunAutomationDispatchService.ArtifactProjectionServices.cs` and classify as pure, file IO, dispatcher instance side effect, candidate mutation, matching/classification, or lineage.
...
Updated `inventories/02-projection-services-method-map.md`.
...
Inventory row count must match all public methods of the nested services class.
...
5. Record proof under `proof/SB02/` during implementation.
...
Implement SB02 only. Preserve behavior. Do not start downstream work early. Record proof before continuing.
