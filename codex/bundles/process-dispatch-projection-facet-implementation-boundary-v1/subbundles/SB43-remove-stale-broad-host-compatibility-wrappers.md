# SB43 - Remove stale broad host compatibility wrappers
...
Remove wrappers that exist only for old broad host compatibility, if unreferenced.
...
ProcessRunAutomationDispatchService.ArtifactProjectionServices.cs; ArtifactProjection.cs

## Deliverables

Deleted stale wrappers or documented deferrals.
...
Build + `rg` no stale wrapper references.
...
5. Record proof under `proof/SB43/` during implementation.
...
Implement SB43 only. Preserve behavior. Do not start downstream work early. Record proof before continuing.
