# SB06 - Extract explicit file IO facet implementation
...
Move `File.Exists`, `ReadAllBytesAsync`, `WriteAllTextAsync`, `CopyFile`, directory creation into a top-level `ProcessProjectionFileIo` implementation.
...
Top-level internal file IO facet implementation.
...
Unit/source scan proves file IO side effects are only in file IO facet.
...
5. Record proof under `proof/SB06/` during implementation.
...
Implement SB06 only. Preserve behavior. Do not start downstream work early. Record proof before continuing.
