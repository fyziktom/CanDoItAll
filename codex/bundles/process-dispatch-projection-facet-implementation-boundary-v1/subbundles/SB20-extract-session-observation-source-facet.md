# SB20 - Extract session observation source facet
...
Move session observation source methods to a focused implementation using `ProcessAutomationObservationSnapshot` where possible.
...
ProcessAutomationObservationSnapshot.cs; ProcessAutomationSessionObservation.cs; ProcessRunAutomationDispatchService.ArtifactProjectionServices.cs

## Deliverables

Session observation source implementation.
...
5. Record proof under `proof/SB20/` during implementation.
...
Implement SB20 only. Preserve behavior. Do not start downstream work early. Record proof before continuing.
