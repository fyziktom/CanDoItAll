...
Add/confirm guardrails for no Process Core, no driver API, no UI, no broad host resurrection, no all-facet implementation acceptance.
...
- Failing-first or adversarial source-scan proof plus passing guardrail proof.
## Dependency Impact
...
- Focused architecture test + source scans. Downstream blocked until green.
...
5. Record proof under `proof/SB04/` during implementation.
...
- Critical proof manifest: bundle://proof/SB04/manifest.md.
...
Implement SB04 only. Preserve behavior. Do not start downstream work early. Record proof before continuing.
