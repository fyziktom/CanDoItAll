# SB07 - Extract candidate-state updater facet implementation
...
Move `TryApplyExpectedWriteOutcome`, `TryApplyWriteOutcome`, and `TryApplyExpectedRecordOnlyOutcome` out of the all-facet implementation.
...
Top-level candidate-state updater implementation or clear module-local class.
...
Tests/source scans prove candidate mutation remains centralized.
...
5. Record proof under `proof/SB07/` during implementation.
...
Implement SB07 only. Preserve behavior. Do not start downstream work early. Record proof before continuing.
