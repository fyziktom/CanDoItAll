...
- Include unhappy-path behavior and structured diagnostics when this subbundle touches loading, setup, calls, lifecycle, UI/API, or runtime adapters.
...
- Confirm no silent fallback or generic error message is accepted where the subbundle can identify the failing key/path/transport/field.

## Proof Required

- List the commands, screenshots, artifact paths, or DOM checks required to prove completion.
- Include negative tests and diagnostics samples when external tools, MCPs, template loading, or runtime adapters are involved.
