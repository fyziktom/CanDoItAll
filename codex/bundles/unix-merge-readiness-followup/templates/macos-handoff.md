...
dotnet test ./tests/Unit/CanDoItAll.Tests.Unit/CanDoItAll.Tests.Unit.csproj \
  --configuration Release --no-build --no-restore \
  --filter 'FullyQualifiedName=CanDoItAll.Tests.Unit.SecretPortabilityTests.Local_user_file_preserves_legacy_payloads_and_restart_continuity|FullyQualifiedName=CanDoItAll.Tests.Unit.LocalWorkspaceProcessHostTests.ExecuteAsync_terminates_descendant_after_parent_exits|FullyQualifiedName=CanDoItAll.Tests.Unit.WorkspaceExecutableLocatorTests.MacOS_contract_uses_exact_executable_names_only|FullyQualifiedName=CanDoItAll.Tests.Unit.WorkspaceExecutableLocatorTests.Unix_actual_host_requires_execute_permission_and_resolves_final_symlink_target|FullyQualifiedName=CanDoItAll.Tests.Unit.RuntimeHostPlatformCapabilityTests.Launchd_system_daemon_template_requires_explicit_service_identity' \
