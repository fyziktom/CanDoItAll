...
| W07 | Rehydrate `ToolApprovalRequestContent` from custom record | **Rewrite for 1.15 binding** | Legacy session state and exact request authority must be explicit | Function/MCP/restart/legacy/integrity tests |
...
| W09 | Apply one `approved` boolean to all pending approvals | **Replace or strictly constrain** | 1.15 mixed-call handling is request-specific | Per-ID decisions and mixed pending tests |
