...
| Argument substitution | Correct ID, modified path/command/amount | Native MAF rebind to the restored request | Original arguments only |
...
| Cross-run/process-step approval | Old request applied to new execution | Exact server-held session and pending snapshot ownership | No invocation |
| Legacy state confusion | 1.13 response-only continuation under 1.15 | Native-session requirement plus drain/reissue | Typed reissue, no silent drop/execute |
| Persistent record tampering | Stored tool/arguments modified | HMAC/authenticated store + MAF native state | Tamper rejected |
| Snapshot race | A decision is applied after the pending set changes | Atomic session/pending-snapshot persistence and compare | Changed snapshot rejected |
...
| Reconstructed legacy execution | Application fabricates native binding state from a 1.13 record | No bridge; drain/reissue only | Legacy record cannot invoke a tool |
