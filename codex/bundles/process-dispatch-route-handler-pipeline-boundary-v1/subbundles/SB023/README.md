# SB023 - Pre-execution handler route order scan
...
Assert FreshRecoverySkip -> DatabaseRequirement -> UpstreamMaterialization order.
...
6. If this is a critical gate, create `proof/SB023/manifest.md` and `proof/SB023/semantic-invariants.md`.
...
Implement `SB023 - Pre-execution handler route order scan` from `process-dispatch-route-handler-pipeline-boundary-v1`. Preserve behavior, keep all work module-local, do not create Process Core or driver APIs, and update proof before continuing.
