# SB004 - Gate A: architecture guardrails before movement
...
Add/verify source scans that forbid Process Core, production driver APIs, UI drift, collapsed report rows, and route order drift.
...
Architecture tests/source scans for route-order, no-core/no-driver, no UI/mobile proof, no stub markers.

## Dependency Impact

- Phase: `P0 - Audit and guardrails`

- Critical foundation: Yes - critical foundation gate.
...
- Focused architecture guard passes before any production route handler movement.
...
6. If this is a critical gate, create `proof/SB004/manifest.md` and `proof/SB004/semantic-invariants.md`.
...
Implement `SB004 - Gate A: architecture guardrails before movement` from `process-dispatch-route-handler-pipeline-boundary-v1`. Preserve behavior, keep all work module-local, do not create Process Core or driver APIs, and update proof before continuing.
