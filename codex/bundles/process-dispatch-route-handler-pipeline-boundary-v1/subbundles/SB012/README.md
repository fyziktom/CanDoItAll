# SB012 - Route handler side-effect classification
...
Classify each host method as read, transition, external execution, finalizer, or persistence side effect.
...
6. If this is a critical gate, create `proof/SB012/manifest.md` and `proof/SB012/semantic-invariants.md`.
...
Implement `SB012 - Route handler side-effect classification` from `process-dispatch-route-handler-pipeline-boundary-v1`. Preserve behavior, keep all work module-local, do not create Process Core or driver APIs, and update proof before continuing.
