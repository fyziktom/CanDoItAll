# SB010 - Route stage order assertion utility
...
Add a utility/test assertion that source handler registration follows ProcessDispatchRoutePipeline.StageOrder exactly.
...
6. If this is a critical gate, create `proof/SB010/manifest.md` and `proof/SB010/semantic-invariants.md`.
...
Implement `SB010 - Route stage order assertion utility` from `process-dispatch-route-handler-pipeline-boundary-v1`. Preserve behavior, keep all work module-local, do not create Process Core or driver APIs, and update proof before continuing.
