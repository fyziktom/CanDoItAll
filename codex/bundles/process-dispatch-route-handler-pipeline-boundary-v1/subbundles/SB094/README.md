# SB094 - Architecture guard: route handler order
...
Add/verify tests fail if route handler order changes.
...
6. If this is a critical gate, create `proof/SB094/manifest.md` and `proof/SB094/semantic-invariants.md`.
...
Implement `SB094 - Architecture guard: route handler order` from `process-dispatch-route-handler-pipeline-boundary-v1`. Preserve behavior, keep all work module-local, do not create Process Core or driver APIs, and update proof before continuing.
