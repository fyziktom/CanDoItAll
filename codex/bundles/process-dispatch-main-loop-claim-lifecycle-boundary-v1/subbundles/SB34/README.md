# SB34 - Route context source assertions
...
Route context source assertions as part of Phase D: Dispatch route context and route-order contracts.
...
6. Record proof under `proof/SB34/`.
...
- [x] Execution report row for `SB34` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB34/manifest.md`
- `proof/SB34/semantic-invariants.md` for critical gates or production movement.
- `proof/SB34/transcripts/` with build/test/source-scan output.
...
Execute `SB34 - Route context source assertions` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
