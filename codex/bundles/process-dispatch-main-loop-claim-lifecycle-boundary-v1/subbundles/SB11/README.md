# SB11 - Claim store interface cutline
...
Claim store interface cutline as part of Phase B: Claim lease policy and claim store foundation.
...
6. Record proof under `proof/SB11/`.
...
- [x] Execution report row for `SB11` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB11/manifest.md`
- `proof/SB11/semantic-invariants.md` for critical gates or production movement.
- `proof/SB11/transcripts/` with build/test/source-scan output.
...
Execute `SB11 - Claim store interface cutline` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
