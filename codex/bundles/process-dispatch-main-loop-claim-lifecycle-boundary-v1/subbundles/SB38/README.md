...
Gate E: route context/order proof as part of Phase D: Dispatch route context and route-order contracts.
...
6. Record proof under `proof/SB38/`.
...
- [x] Execution report row for `SB38` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB38/manifest.md`
- `proof/SB38/semantic-invariants.md` for critical gates or production movement.
- `proof/SB38/transcripts/` with build/test/source-scan output.
...
Execute `SB38 - Gate E: route context/order proof` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
