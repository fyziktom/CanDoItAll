# SB56 - Subprocess/workflow/start tests
...
Subprocess/workflow/start tests as part of Phase F: Start/workflow/subprocess route handlers.
...
6. Record proof under `proof/SB56/`.
...
- [x] Execution report row for `SB56` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB56/manifest.md`
- `proof/SB56/semantic-invariants.md` for critical gates or production movement.
- `proof/SB56/transcripts/` with build/test/source-scan output.
...
Execute `SB56 - Subprocess/workflow/start tests` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
