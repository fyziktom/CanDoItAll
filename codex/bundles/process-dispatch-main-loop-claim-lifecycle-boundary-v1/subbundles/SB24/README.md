# SB24 - Release-on-finally behavior tests
...
Release-on-finally behavior tests as part of Phase C: Claim coordinator and heartbeat lifecycle.
...
6. Record proof under `proof/SB24/`.
...
- [x] Execution report row for `SB24` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB24/manifest.md`
- `proof/SB24/semantic-invariants.md` for critical gates or production movement.
- `proof/SB24/transcripts/` with build/test/source-scan output.
...
Execute `SB24 - Release-on-finally behavior tests` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
