# SB60 - Competing execution guard handler
...
Competing execution guard handler as part of Phase G: Direct-agent route and finalizer handoff.
...
6. Record proof under `proof/SB60/`.
...
- [x] Execution report row for `SB60` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB60/manifest.md`
- `proof/SB60/semantic-invariants.md` for critical gates or production movement.
- `proof/SB60/transcripts/` with build/test/source-scan output.
...
Execute `SB60 - Competing execution guard handler` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
