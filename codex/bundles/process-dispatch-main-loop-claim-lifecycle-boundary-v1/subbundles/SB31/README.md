# SB31 - Route stage enum and order contract
...
Route stage enum and order contract as part of Phase D: Dispatch route context and route-order contracts.
...
6. Record proof under `proof/SB31/`.
...
- [x] Execution report row for `SB31` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB31/manifest.md`
- `proof/SB31/semantic-invariants.md` for critical gates or production movement.
- `proof/SB31/transcripts/` with build/test/source-scan output.
...
Execute `SB31 - Route stage enum and order contract` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
