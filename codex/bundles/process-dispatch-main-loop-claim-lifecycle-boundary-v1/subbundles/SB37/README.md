# SB37 - Remove route-order magic booleans where safe
...
Remove route-order magic booleans where safe as part of Phase D: Dispatch route context and route-order contracts.
...
6. Record proof under `proof/SB37/`.
...
- [x] Execution report row for `SB37` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB37/manifest.md`
- `proof/SB37/semantic-invariants.md` for critical gates or production movement.
- `proof/SB37/transcripts/` with build/test/source-scan output.
...
Execute `SB37 - Remove route-order magic booleans where safe` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
