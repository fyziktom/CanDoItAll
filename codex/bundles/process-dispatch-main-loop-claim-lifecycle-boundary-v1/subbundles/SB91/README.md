# SB91 - Known unrelated failure inventory update
...
Known unrelated failure inventory update as part of Phase J: Driver-readiness docs, smoke matrix and final red-team closure.
...
6. Record proof under `proof/SB91/`.
...
- [x] Execution report row for `SB91` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB91/manifest.md`
- `proof/SB91/semantic-invariants.md` for critical gates or production movement.
- `proof/SB91/transcripts/` with build/test/source-scan output.
...
Execute `SB91 - Known unrelated failure inventory update` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
