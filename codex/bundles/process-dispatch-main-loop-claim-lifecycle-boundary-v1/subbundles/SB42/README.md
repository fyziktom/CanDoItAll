# SB42 - Fresh recovery skip route handler
...
Fresh recovery skip route handler as part of Phase E: Pre-execution route handlers.
...
6. Record proof under `proof/SB42/`.
...
- [x] Execution report row for `SB42` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB42/manifest.md`
- `proof/SB42/semantic-invariants.md` for critical gates or production movement.
- `proof/SB42/transcripts/` with build/test/source-scan output.
...
Execute `SB42 - Fresh recovery skip route handler` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
