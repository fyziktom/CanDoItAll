# SB39 - Database requirement route handler
...
Database requirement route handler as part of Phase E: Pre-execution route handlers.
...
6. Record proof under `proof/SB39/`.
...
- [x] Execution report row for `SB39` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB39/manifest.md`
- `proof/SB39/semantic-invariants.md` for critical gates or production movement.
- `proof/SB39/transcripts/` with build/test/source-scan output.
...
Execute `SB39 - Database requirement route handler` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
