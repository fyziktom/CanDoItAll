# SB49 - Subprocess route handler boundary
...
Subprocess route handler boundary as part of Phase F: Start/workflow/subprocess route handlers.
...
6. Record proof under `proof/SB49/`.
...
- [x] Execution report row for `SB49` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB49/manifest.md`
- `proof/SB49/semantic-invariants.md` for critical gates or production movement.
- `proof/SB49/transcripts/` with build/test/source-scan output.
...
Execute `SB49 - Subprocess route handler boundary` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
