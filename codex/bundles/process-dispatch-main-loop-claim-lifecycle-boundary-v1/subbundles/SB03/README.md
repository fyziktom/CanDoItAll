# SB03 - Route-order behavior inventory
...
Route-order behavior inventory as part of Phase A: Entry audit, current-state proof and guardrails.
...
6. Record proof under `proof/SB03/`.
...
- [x] Execution report row for `SB03` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB03/manifest.md`
- `proof/SB03/semantic-invariants.md` for critical gates or production movement.
- `proof/SB03/transcripts/` with build/test/source-scan output.
...
Execute `SB03 - Route-order behavior inventory` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
