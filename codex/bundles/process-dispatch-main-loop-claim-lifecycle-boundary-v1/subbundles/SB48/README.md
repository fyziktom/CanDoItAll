...
Gate F: pre-execution route proof as part of Phase E: Pre-execution route handlers.
...
6. Record proof under `proof/SB48/`.
...
- [x] Execution report row for `SB48` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB48/manifest.md`
- `proof/SB48/semantic-invariants.md` for critical gates or production movement.
- `proof/SB48/transcripts/` with build/test/source-scan output.
...
Execute `SB48 - Gate F: pre-execution route proof` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
