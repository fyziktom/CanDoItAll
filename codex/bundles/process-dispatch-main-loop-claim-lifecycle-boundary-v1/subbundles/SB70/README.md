# SB70 - Claim-lost exception handler extraction
...
Claim-lost exception handler extraction as part of Phase H: Exception closure and failure transition boundary.
...
6. Record proof under `proof/SB70/`.
...
- [x] Execution report row for `SB70` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB70/manifest.md`
- `proof/SB70/semantic-invariants.md` for critical gates or production movement.
- `proof/SB70/transcripts/` with build/test/source-scan output.
...
Execute `SB70 - Claim-lost exception handler extraction` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
