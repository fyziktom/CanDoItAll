# SB02 - Dispatch.cs line-count and method inventory
...
Dispatch.cs line-count and method inventory as part of Phase A: Entry audit, current-state proof and guardrails.
...
- Inventory/proof artefact only; no production movement unless explicitly stated by the subbundle title.
...
6. Record proof under `proof/SB02/`.
...
- [x] Execution report row for `SB02` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB02/manifest.md`
- `proof/SB02/semantic-invariants.md` for critical gates or production movement.
- `proof/SB02/transcripts/` with build/test/source-scan output.
...
Execute `SB02 - Dispatch.cs line-count and method inventory` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
