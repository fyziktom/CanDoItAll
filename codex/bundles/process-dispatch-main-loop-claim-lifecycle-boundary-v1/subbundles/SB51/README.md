# SB51 - Workflow route handler boundary
...
Workflow route handler boundary as part of Phase F: Start/workflow/subprocess route handlers.
...
6. Record proof under `proof/SB51/`.
...
- [x] Execution report row for `SB51` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB51/manifest.md`
- `proof/SB51/semantic-invariants.md` for critical gates or production movement.
- `proof/SB51/transcripts/` with build/test/source-scan output.
...
Execute `SB51 - Workflow route handler boundary` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
