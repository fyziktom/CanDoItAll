# SB63 - Direct route exception-free parity tests
...
Direct route exception-free parity tests as part of Phase G: Direct-agent route and finalizer handoff.
...
6. Record proof under `proof/SB63/`.
...
- [x] Execution report row for `SB63` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB63/manifest.md`
- `proof/SB63/semantic-invariants.md` for critical gates or production movement.
- `proof/SB63/transcripts/` with build/test/source-scan output.
...
Execute `SB63 - Direct route exception-free parity tests` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
