# SB55 - Route handler dependency narrowing
...
Route handler dependency narrowing as part of Phase F: Start/workflow/subprocess route handlers.
...
6. Record proof under `proof/SB55/`.
...
- [x] Execution report row for `SB55` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB55/manifest.md`
- `proof/SB55/semantic-invariants.md` for critical gates or production movement.
- `proof/SB55/transcripts/` with build/test/source-scan output.
...
Execute `SB55 - Route handler dependency narrowing` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
