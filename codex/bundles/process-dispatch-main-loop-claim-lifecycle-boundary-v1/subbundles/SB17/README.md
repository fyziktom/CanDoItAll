# SB17 - Move direct claim EF calls behind store
...
Move direct claim EF calls behind store as part of Phase B: Claim lease policy and claim store foundation.
...
6. Record proof under `proof/SB17/`.
...
- [x] Execution report row for `SB17` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB17/manifest.md`
- `proof/SB17/semantic-invariants.md` for critical gates or production movement.
- `proof/SB17/transcripts/` with build/test/source-scan output.
...
Execute `SB17 - Move direct claim EF calls behind store` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
