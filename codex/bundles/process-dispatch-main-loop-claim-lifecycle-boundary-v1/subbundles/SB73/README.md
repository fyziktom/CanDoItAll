# SB73 - Generic dispatch failure transition builder
...
Generic dispatch failure transition builder as part of Phase H: Exception closure and failure transition boundary.
...
6. Record proof under `proof/SB73/`.
...
- [x] Execution report row for `SB73` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB73/manifest.md`
- `proof/SB73/semantic-invariants.md` for critical gates or production movement.
- `proof/SB73/transcripts/` with build/test/source-scan output.
...
Execute `SB73 - Generic dispatch failure transition builder` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
