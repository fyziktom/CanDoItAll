# SB50 - Subprocess finalizer handoff parity
...
Subprocess finalizer handoff parity as part of Phase F: Start/workflow/subprocess route handlers.
...
6. Record proof under `proof/SB50/`.
...
- [x] Execution report row for `SB50` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB50/manifest.md`
- `proof/SB50/semantic-invariants.md` for critical gates or production movement.
- `proof/SB50/transcripts/` with build/test/source-scan output.
...
Execute `SB50 - Subprocess finalizer handoff parity` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
