# SB40 - Upstream materialization route handler
...
Upstream materialization route handler as part of Phase E: Pre-execution route handlers.
...
6. Record proof under `proof/SB40/`.
...
- [x] Execution report row for `SB40` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB40/manifest.md`
- `proof/SB40/semantic-invariants.md` for critical gates or production movement.
- `proof/SB40/transcripts/` with build/test/source-scan output.
...
Execute `SB40 - Upstream materialization route handler` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
