# SB18 - Remove duplicate claim query fragments
...
Remove duplicate claim query fragments as part of Phase B: Claim lease policy and claim store foundation.
...
6. Record proof under `proof/SB18/`.
...
- [x] Execution report row for `SB18` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB18/manifest.md`
- `proof/SB18/semantic-invariants.md` for critical gates or production movement.
- `proof/SB18/transcripts/` with build/test/source-scan output.
...
Execute `SB18 - Remove duplicate claim query fragments` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
