# SB12 - EF claim store first implementation
...
EF claim store first implementation as part of Phase B: Claim lease policy and claim store foundation.
...
6. Record proof under `proof/SB12/`.
...
- [x] Execution report row for `SB12` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB12/manifest.md`
- `proof/SB12/semantic-invariants.md` for critical gates or production movement.
- `proof/SB12/transcripts/` with build/test/source-scan output.
...
Execute `SB12 - EF claim store first implementation` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
