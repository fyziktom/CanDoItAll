# SB10 - Claim token and lease value model
...
Claim token and lease value model as part of Phase B: Claim lease policy and claim store foundation.
...
6. Record proof under `proof/SB10/`.
...
- [x] Execution report row for `SB10` is updated.
- [x] Critical gate proof exists if this is a gate.

## Proof Required

- `proof/SB10/manifest.md`
- `proof/SB10/semantic-invariants.md` for critical gates or production movement.
- `proof/SB10/transcripts/` with build/test/source-scan output.
...
Execute `SB10 - Claim token and lease value model` only. Preserve current behavior. Do not start Process Core or driver APIs. Update proof and execution report before proceeding.
