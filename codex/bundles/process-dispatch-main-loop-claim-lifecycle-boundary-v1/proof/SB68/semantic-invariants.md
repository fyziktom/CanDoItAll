# SB68 Semantic Invariants

- Invariant ID: SB68-INV-001
- Source raw note: Preserve direct-agent execution and finalizer handoff behavior.
...
- Failing-first test: N/A for process/non-production boundary refactor; negative coverage comes from command transcript bundle://proof/SB68/transcripts/source-boundary-scan.txt.
- Passing test: dotnet build, focused unit tests, and focused integration tests are recorded under bundle://proof/SB68/transcripts/.
...
- Red-team negative case: bundle://proof/SB68/transcripts/anti-stub-scan.txt proves no stubs, TODOs, not-implemented paths, Process Core references, or driver API tokens in changed production dispatch files.
- Downstream dependency check: bundle://proof/SB68/transcripts/source-boundary-scan.txt and focused tests cover route order, finalizer handoff, claim boundary placement, and UI no-change scan.
