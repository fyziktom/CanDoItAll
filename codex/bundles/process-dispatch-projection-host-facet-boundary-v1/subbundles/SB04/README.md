...
Lock no-core, no-driver, no-UI, no viewport-proof, and no functionality-loss constraints before movement.
...
- Implement or verify the scoped change for SB04.
...
- Critical manifest under `proof/SB04/manifest.md`.
...
Implement SB04 only. Preserve behavior. Update proof. Do not start the next subbundle until this subbundle is closed.
