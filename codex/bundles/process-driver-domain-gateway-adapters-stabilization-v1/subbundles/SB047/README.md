# SB047 - Focused unit/integration matrix

## Status
Prepared.

## Objective
Implement the `Focused unit/integration matrix` slice in phase `P16 Broad smoke` while preserving read-only driver boundaries and stable Process Core.
...
- Updated proof artifacts under `proof/SB047/`.
...
Implement `SB047 - Focused unit/integration matrix` from `process-driver-domain-gateway-adapters-stabilization-v1`. Preserve hard constraints. Use live source, tests, and proof artifacts; do not rely on report-only status.
