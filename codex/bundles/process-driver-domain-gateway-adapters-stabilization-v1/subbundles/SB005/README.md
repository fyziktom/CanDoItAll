# SB005 - Replace stale fixture tests with current source guards

## Status
Prepared.

## Objective
Implement the `Replace stale fixture tests with current source guards` slice in phase `P02 Full-unit debt burn-down` while preserving read-only driver boundaries and stable Process Core.
...
- Updated proof artifacts under `proof/SB005/`.
...
Implement `SB005 - Replace stale fixture tests with current source guards` from `process-driver-domain-gateway-adapters-stabilization-v1`. Preserve hard constraints. Use live source, tests, and proof artifacts; do not rely on report-only status.
