# SB006 - Gate B full-unit debt closure

## Status
Prepared.

## Objective
Implement the `Gate B full-unit debt closure` slice in phase `P02 Full-unit debt burn-down` while preserving read-only driver boundaries and stable Process Core.
...
- Updated proof artifacts under `proof/SB006/`.
...
Implement `SB006 - Gate B full-unit debt closure` from `process-driver-domain-gateway-adapters-stabilization-v1`. Preserve hard constraints. Use live source, tests, and proof artifacts; do not rely on report-only status.
