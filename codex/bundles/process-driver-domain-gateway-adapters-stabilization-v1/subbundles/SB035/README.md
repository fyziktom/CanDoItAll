# SB035 - Negative/malicious corpus for all lanes

## Status
- Completed

## Objective
Implement the `Negative/malicious corpus for all lanes` slice in phase `P12 Multi-domain corpus` while preserving read-only driver boundaries and stable Process Core.
...
- Updated proof artifacts under `proof/SB035/`.
...
Implement `SB035 - Negative/malicious corpus for all lanes` from `process-driver-domain-gateway-adapters-stabilization-v1`. Preserve hard constraints. Use live source, tests, and proof artifacts; do not rely on report-only status.
