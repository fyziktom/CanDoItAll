# SB022 - Office adapter implementation

## Status
Prepared.

## Objective
Implement the `Office adapter implementation` slice in phase `P08 Process adapters for Office/business` while preserving read-only driver boundaries and stable Process Core.
...
- Updated proof artifacts under `proof/SB022/`.
...
Implement `SB022 - Office adapter implementation` from `process-driver-domain-gateway-adapters-stabilization-v1`. Preserve hard constraints. Use live source, tests, and proof artifacts; do not rely on report-only status.
