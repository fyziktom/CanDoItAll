# SB004 - Historical fixture skip ledger

## Status
Prepared.

## Objective
Implement the `Historical fixture skip ledger` slice in phase `P02 Full-unit debt burn-down` while preserving read-only driver boundaries and stable Process Core.
...
- Updated proof artifacts under `proof/SB004/`.
...
Implement `SB004 - Historical fixture skip ledger` from `process-driver-domain-gateway-adapters-stabilization-v1`. Preserve hard constraints. Use live source, tests, and proof artifacts; do not rely on report-only status.
