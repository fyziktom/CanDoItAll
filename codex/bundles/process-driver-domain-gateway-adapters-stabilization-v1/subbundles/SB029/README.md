# SB029 - Apply policy across adapters

## Status
Prepared.

## Objective
Implement the `Apply policy across adapters` slice in phase `P10 Shared evidence policy hardening` while preserving read-only driver boundaries and stable Process Core.
...
- Updated proof artifacts under `proof/SB029/`.
...
Implement `SB029 - Apply policy across adapters` from `process-driver-domain-gateway-adapters-stabilization-v1`. Preserve hard constraints. Use live source, tests, and proof artifacts; do not rely on report-only status.
