...
- Normalized requirements mapped in `bundle://traceability/01-requirement-traceability.md` for SB06.
- Failed process run `9bbc0667-9d12-4506-ba81-654ef924cad6` where applicable to process-runtime phases.

## Prerequisites

- SB05 completed or explicitly reopened/blocked with dependency-safe notes.
...
- Critical behavior changes must update `bundle://proof/SB06/manifest.md` and `bundle://proof/SB06/semantic-invariants.md`.
...
- Required proof artifacts exist under `bundle://proof/SB06/`.
...
- Add or update the SB06 row in `## Browser Validation Analytics` with route, viewport, evidence path, screenshots, and result, using N/A only when no UI/browser behavior changed.
...
- Execute SB06 from `bundle://subbundles/06-handoff-a2a-and-workflow-migration/README.md`, keep scope limited to this phase, update proof and execution report before moving on.
