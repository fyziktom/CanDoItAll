...
- Normalized requirements mapped in `bundle://traceability/01-requirement-traceability.md` for SB14.
- Failed process run `9bbc0667-9d12-4506-ba81-654ef924cad6` where applicable to process-runtime phases.

## Prerequisites

- SB13 completed or explicitly reopened/blocked with dependency-safe notes.
...
- Critical behavior changes must update `bundle://proof/SB14/manifest.md` and `bundle://proof/SB14/semantic-invariants.md`.
...
- Required proof artifacts exist under `bundle://proof/SB14/`.
...
- Add or update the SB14 row in `## Browser Validation Analytics` with route, viewport, evidence path, screenshots, and result, using N/A only when no UI/browser behavior changed.
...
- Execute SB14 from `bundle://subbundles/14-skills-tools-and-agent-capability-regression/README.md`, keep scope limited to this phase, update proof and execution report before moving on.
