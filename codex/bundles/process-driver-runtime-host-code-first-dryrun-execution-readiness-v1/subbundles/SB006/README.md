# SB006 - Gate B: durable audit production proof

## Status
Prepared.

## Objective
Critical proof that production DI uses EF audit store by default and in-memory only in explicit test helpers.
...
## Validation Depth
DI test + PostgreSQL audit query test.
...
Implement SB006 as a code-first slice. Do not close with report-only proof. Make source/test changes first, then record concise evidence.
