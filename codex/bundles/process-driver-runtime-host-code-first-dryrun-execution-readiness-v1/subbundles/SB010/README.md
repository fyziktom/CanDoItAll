# SB010 - Async-only production path migration

## Status
Prepared.

## Objective
Replace production calls to sync Verify/Run wrappers with async APIs; keep sync wrappers only as compatibility/test-only where justified.
...
## Validation Depth
Source scan forbids sync wrapper in production paths.
...
Implement SB010 as a code-first slice. Do not close with report-only proof. Make source/test changes first, then record concise evidence.
