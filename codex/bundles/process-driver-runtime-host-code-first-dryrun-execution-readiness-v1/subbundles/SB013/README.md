# SB013 - Read-only verification job service

## Status
Prepared.

## Objective
Promote job runner into a typed service capable of executing scheduled/workflow-origin read-only jobs with provenance.
...
Implement SB013 as a code-first slice. Do not close with report-only proof. Make source/test changes first, then record concise evidence.
