# SB019 — Direct-agent execution input/output DTO hardening
...
Remove full dispatcher payloads from direct-agent runtime boundary except one adapter edge.
...
- Proof transcript under `bundle://proof/SB019/transcripts/`.
- Semantic invariants under `bundle://proof/SB019/semantic-invariants.md`.
- Manifest under `bundle://proof/SB019/manifest.md`.
...
- Entry gate: `Passed`; `SB018` is completed.
- Closure gate: `Passed`; direct-agent runtime consumes route-owned input/output DTOs with one dispatcher adapter edge.
- Proof: `bundle://proof/SB019/manifest.md` and `bundle://proof/SB019/semantic-invariants.md`.
- Downstream dependencies checked: `Passed`; `SB020` may proceed with execution proof/readiness snapshot.

## Suggested Agent Prompt

Implement `SB019 — Direct-agent execution input/output DTO hardening` from `process-core-pre-extraction-consolidation-driver-readiness-v1`. Preserve behavior, update tests, record proof, and do not create Process Core or production driver APIs.
