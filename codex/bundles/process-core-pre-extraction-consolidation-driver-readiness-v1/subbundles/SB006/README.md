...
Prove route order, start transition reload, direct-agent/finalizer handoff, and no adapter leaks.
...
- Proof transcript under `bundle://proof/SB006/transcripts/`.
- Semantic invariants under `bundle://proof/SB006/semantic-invariants.md`.
- Manifest under `bundle://proof/SB006/manifest.md`.
...
- Entry gate: `Passed`; `SB004` and `SB005` are completed.
- Closure gate: `Passed`; route order, start-transition reload, finalizer/direct handoff, adapter confinement, and no-Core/no-driver/no-UI/no-stub scans passed.
- Proof: `bundle://proof/SB006/manifest.md` and `bundle://proof/SB006/semantic-invariants.md`.
- Downstream dependencies checked: `Passed`; `SB007` may start finalizer intent work.

## Suggested Agent Prompt

Implement `SB006 — Gate B route DTO parity` from `process-core-pre-extraction-consolidation-driver-readiness-v1`. Preserve behavior, update tests, record proof, and do not create Process Core or production driver APIs.
