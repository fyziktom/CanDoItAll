...
def extract_state(run, evidence):
    ...
    if isinstance(state, dict) and 'payloadJson' in state:
        payload = state['payloadJson']
        if not isinstance(payload, str) or len(payload.encode('utf-8')) > evidence.MAX_FILE_BYTES:
            raise ValueError('Session envelope payload is outside the bounded schema.')
        state = json.loads(payload, object_pairs_hook=evidence.reject_duplicate_keys)
    ...
    for call_id, result in result_nodes:
        if call_id in calls:
            ...
            if item not in pairs:
                pairs.append(item)
    return {'serializedStatePresent': True, 'runtimeEnvelopePayloadDecoded': envelope_decoded, 'calls': sorted(calls.values(), key=lambda item: item['callId']), 'resultPairs': pairs}
