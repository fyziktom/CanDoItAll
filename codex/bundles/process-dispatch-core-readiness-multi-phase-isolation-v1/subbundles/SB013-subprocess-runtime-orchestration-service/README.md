# SB013 - Subprocess runtime orchestration service

## Status
- Status: Completed

## Objective
Extract `HandleSubprocessDispatchAsync` into `ProcessSubprocessRuntimeService` with explicit dependencies for child-run observation, capability gaps, transitions, finalizer handoff.
...
## Dependency Impact
- Do not change child status to parent status mapping.
...
Implement SB013 from the bundle. Keep behavior identical, avoid Process Core and driver APIs, and produce the proof required above.
