# SB005 - Route model adapter reduction

## Status
- Status: Completed

## Objective
Reduce use of `ProcessDispatchRouteModelAdapters.ToDispatcher*` by moving downstream services to route-owned models where possible. Keep adapters only at unavoidable dispatcher edge.
...
## Dependency Impact
- If a service still needs dispatcher source handles, document why.

## Validation Depth
- Source scan for adapter call count delta and behavior tests.
...
Implement SB005 from the bundle. Keep behavior identical, avoid Process Core and driver APIs, and produce the proof required above.
