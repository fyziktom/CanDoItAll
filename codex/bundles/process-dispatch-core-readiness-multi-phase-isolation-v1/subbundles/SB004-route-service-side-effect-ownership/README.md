# SB004 - Route service side-effect ownership

## Status
- Status: Completed

## Objective
Move real ownership for the lowest-risk route service operations out of dispatcher forwarding where safe. Start with database requirement, upstream materialization planning, and run-closed guard.
...
## Dependency Impact
- Must not change route stage order. Must keep transition writes explicit.

## Validation Depth
- Focused unit tests for each moved route service plus integration smoke.
...
Implement SB004 from the bundle. Keep behavior identical, avoid Process Core and driver APIs, and produce the proof required above.
