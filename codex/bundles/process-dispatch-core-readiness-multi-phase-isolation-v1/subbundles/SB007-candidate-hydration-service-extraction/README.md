# SB007 - Candidate hydration service extraction

## Status
- Status: Completed

## Objective
Create module-local `ProcessDispatchCandidateHydrationService` owning `LoadDispatchCandidateAsync` readback and candidate assembly orchestration. Dispatcher becomes caller only.
...
## Dependency Impact
- Must preserve subprocess/workflow/direct-agent candidate selection order.

## Validation Depth
- Candidate hydration unit/integration tests and source scan.
...
Implement SB007 from the bundle. Keep behavior identical, avoid Process Core and driver APIs, and produce the proof required above.
