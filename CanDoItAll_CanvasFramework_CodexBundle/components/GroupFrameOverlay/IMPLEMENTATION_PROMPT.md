# Implementation Prompt — GroupFrameOverlay

Implement `GroupFrameOverlay` for the CanDoItAll shared canvas framework.
...
- Do not create a second parallel abstraction next to `GroupFrameOverlay` once the dedicated boundary exists.
...
- `GroupFrameOverlay` exists as a first-class boundary in the recommended location.
