# Implementation Prompt — PromptRunBranchLane

Implement `PromptRunBranchLane` for the CanDoItAll shared canvas framework.
...
- Do not create a second parallel abstraction next to `PromptRunBranchLane` once the dedicated boundary exists.
...
- `PromptRunBranchLane` exists as a first-class boundary in the recommended location.
