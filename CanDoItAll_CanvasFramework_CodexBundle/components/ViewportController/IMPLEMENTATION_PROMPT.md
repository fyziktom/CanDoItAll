# Implementation Prompt — ViewportController

Implement `ViewportController` for the CanDoItAll shared canvas framework.
...
- Do not create a second parallel abstraction next to `ViewportController` once the dedicated boundary exists.
...
- `ViewportController` exists as a first-class boundary in the recommended location.
