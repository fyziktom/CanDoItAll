# Implementation Prompt — ChipBadgePrimitive

Implement `ChipBadgePrimitive` for the CanDoItAll shared canvas framework.
...
- Do not create a second parallel abstraction next to `ChipBadgePrimitive` once the dedicated boundary exists.
...
- `ChipBadgePrimitive` exists as a first-class boundary in the recommended location.
