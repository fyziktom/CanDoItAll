...
MinimapOverview is a P2 shared high-level component in the category `Layout and navigation components`.
...
Large scenes in Project Structure and Prompt Factory will become harder to navigate without an overview. The current framework has no such component.
...
MinimapOverview is currently missing as a first-class component. The implementation should introduce it at the framework boundary defined in this bundle and wire it into the existing pages/services through the listed integration seams.
...
2. Create the new `MinimapOverview` boundary in the recommended target path(s).
