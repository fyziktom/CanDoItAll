# Implementation Prompt — TooltipPopoverHost

Implement `TooltipPopoverHost` for the CanDoItAll shared canvas framework.
...
- Do not create a second parallel abstraction next to `TooltipPopoverHost` once the dedicated boundary exists.
...
- `TooltipPopoverHost` exists as a first-class boundary in the recommended location.
