# Implementation Prompt — GridBackdrop

Implement `GridBackdrop` for the CanDoItAll shared canvas framework.
...
- Do not create a second parallel abstraction next to `GridBackdrop` once the dedicated boundary exists.
...
- `GridBackdrop` exists as a first-class boundary in the recommended location.
