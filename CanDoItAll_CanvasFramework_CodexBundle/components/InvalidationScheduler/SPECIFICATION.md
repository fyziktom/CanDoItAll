# InvalidationScheduler Specification
...
Frame scheduler with dirty flags for geometry, viewport, selection overlay, and diagnostics layers.

### Target advanced version

Dirty-rect tracking, background measurement queues, adaptive throttling for large scenes, and profiling counters.
...
Never synchronously rebuild the full workbench during pointermove; schedule and coalesce instead.

## UX/UI notes

Consistent scheduling is necessary for smooth drag, zoom, and hover feedback without jank.

## Accessibility notes

State publishes to DOM mirrors should be throttled but not skipped when focus or semantic selection changes.
...
Stress-test drag and zoom loops with synthetic high-frequency events and verify bounded render passes.
...
```csharp
public sealed class InvalidationSchedulerOptions
{
...
public interface IInvalidationScheduler
{
    ValueTask AttachAsync(ElementReference host, InvalidationSchedulerOptions options);
