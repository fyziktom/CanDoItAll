# Implementation Prompt — PromptFactoryCatalogToolbox

Implement `PromptFactoryCatalogToolbox` for the CanDoItAll shared canvas framework.
...
- Do not create a second parallel abstraction next to `PromptFactoryCatalogToolbox` once the dedicated boundary exists.
...
- `PromptFactoryCatalogToolbox` exists as a first-class boundary in the recommended location.
