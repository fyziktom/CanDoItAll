# Implementation Prompt — CalendarCrudBridge

Implement `CalendarCrudBridge` for the CanDoItAll shared canvas framework.
...
- Do not create a second parallel abstraction next to `CalendarCrudBridge` once the dedicated boundary exists.
...
- `CalendarCrudBridge` exists as a first-class boundary in the recommended location.
