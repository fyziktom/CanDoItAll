# Implementation Prompt — ProjectStructureValidationOverlay

Implement `ProjectStructureValidationOverlay` for the CanDoItAll shared canvas framework.
...
- Do not create a second parallel abstraction next to `ProjectStructureValidationOverlay` once the dedicated boundary exists.
...
- `ProjectStructureValidationOverlay` exists as a first-class boundary in the recommended location.
