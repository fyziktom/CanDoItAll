# CalendarTimeGridRenderer Specification
...
Extract renderTimedView responsibilities behind a named module boundary within the calendar runtime.

### Target advanced version

Reusable timed-view renderer with collision strategies, drag-resize hooks, and virtualization for dense schedules.
...
Virtualize or window dense time ranges and cache lane calculations.

## UX/UI notes

Event placement accuracy and readability are essential to trust.

## Accessibility notes

Expose timed events and slots through semantic summaries and keyboard navigation.
...
Cover collision layout, all-day interactions, and timezone-sensitive rendering.
...
```csharp
public sealed class CalendarTimeGridRendererOptions
{
...
public sealed record CalendarTimeGridRendererState(string View, DateOnly VisibleDate, string? SelectedId);
```
