...
Shared hit-test API returning semantic targets with scene coordinates and z-aware precedence.

### Target advanced version

Spatial indexing, expanded hit regions, hit caching, and debug visualization hooks.
...
```csharp
public sealed class HitTestServiceOptions
{
    public bool IsEnabled { get; init; } = true;
    public double Tolerance { get; init; } = 8;
}

public sealed record HitTestServiceChangedEventArgs(string[] TargetIds, string Reason);
```
