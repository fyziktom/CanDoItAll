# Implementation Prompt — InlineEditorComposer

Implement `InlineEditorComposer` for the CanDoItAll shared canvas framework.
...
- Do not create a second parallel abstraction next to `InlineEditorComposer` once the dedicated boundary exists.
...
- `InlineEditorComposer` exists as a first-class boundary in the recommended location.
