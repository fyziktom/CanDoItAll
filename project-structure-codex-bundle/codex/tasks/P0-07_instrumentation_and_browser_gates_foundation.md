...
Add the measurement and screenshot infrastructure required for safe refactoring.
...
- Existing Playwright artifact capture
- Existing diagnostics UI
- Existing browser smoke coverage
...
1. Add or expand debug counters on `host.__canvasWorkbenchState` for render count, full rebuild count, patched count, and state publish count.
2. Add browser assertions and screenshots for toolbox behavior, window drag, single-select, multi-select, and key modal flows.
3. Create a large deterministic graph fixture for browser and local performance validation.
...
- [ ] Codex can prove improvements with counters and screenshots rather than only with anecdotes.
- [ ] A failing browser gate is considered a failed task.
