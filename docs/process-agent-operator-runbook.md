...
dotnet test tests\CanDoItAll.Tests.Integration\CanDoItAll.Tests.Integration.csproj --no-build --filter "FullyQualifiedName~Process_manager_diagnostics_operator_smoke_serializes_large_screen_api_readback_with_audit_contract|FullyQualifiedName~Process_run_detail_verification_audit_readback_projects_process_step_audit_and_denial_metadata_without_mutation"
dotnet test tests\CanDoItAll.Tests.Playwright\CanDoItAll.Tests.Playwright.csproj --no-build --filter "FullyQualifiedName~Process_start_large_screen_imports_template_and_executes_ready_launch_from_ui|FullyQualifiedName~Process_run_detail_recovery_large_screen_displays_blocked_recovery_and_artifact_readback|FullyQualifiedName~Project_structure_process_run_output_opens_project_processes_from_output_folder_node"
```
