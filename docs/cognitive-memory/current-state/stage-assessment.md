...
The honest stage after the Qdrant validation follow-up is **P1 beta for the core memory and Qdrant-backed recall path**.

It is past prototype, past P0 hardening, and past the previous beta-candidate blocker: the module has durable EF models, PostgreSQL migrations, production-shaped services, legacy plus v1 HTTP API surfaces, contract metadata/examples, a Blazor operator route, source ingestion, consolidation, recall, review approval, projection rebuild, retention cleanup, explicit automation execution, operator audit, probes, self-regulation, MAF context contribution policy, child-tab UI components, targeted tests, and live Docker Qdrant proof. The beta claim is scoped to the source-ingestion -> durable-memory -> projection-rebuild -> Qdrant -> vector-recall path. Advanced surfaces such as cross-project promotion, distributed compute, professor review, and autonomous scheduling remain alpha/P2.
...
| Durable schema | Alpha complete | 109 cognitive-memory entity records and PostgreSQL migrations exist. | Schema needs stabilization review before treating it as long-lived external contract. |
| Source ingestion | P1 beta for public file/web and source-boundary paths | Workbench, process, workflow, external file, and web-link ingestion paths exist. External source ingestion has centralized limits, contextual extraction failures, sensitive URL/content rejection, and focused tests. Live beta proof ingested an external Markdown file through `/api/cognitive-memory/v1/external-sources/files`. | More source-provider coverage and operational retry/diagnostic UX before production. |
| Consolidation | P1 beta for deterministic source-backed materialization | `CognitiveMemoryConsolidationEngine` creates candidates, mutation commands, review rows, and can materialize memory records through the candidate applicator. The applicator now creates context-frame/entity metadata required by projection rebuild. | Current fact extraction is deterministic/rule-based; model execution profiles are settings, not a fully wired model-assisted consolidation pipeline. |
...
| Recall | P1 beta for lexical plus Qdrant vector recall | `CognitiveMemoryRecallOrchestrator` persists traces, candidates, context packs, source refs, and warnings. Live proof used `projectionCollectionName`, `projectionProfileId`, and `embeddingProfileId` through `/api/cognitive-memory/v1/recall` and produced a completed `rag:qdrant:search:2` stage. | Vector channel is still optional by request/config. Future callers must keep explicit skipped/unavailable traces. |
| Projection/RAG | P1 beta for Docker Qdrant projection | RAG adapter, projection lifecycle, and `ICognitiveMemoryProjectionRebuildService` exist. `/api/cognitive-memory/projections/rebuild` and `/api/cognitive-memory/v1/projections/rebuild` consume stale/failed projection records and can project missing durable records when projection defaults are supplied. Rebuild has adapter-backed proof, deterministic provider-failure proof, and live Docker Qdrant proof. | Qdrant remains a rebuildable projection only. CI/containerized repeatability and operational recovery drills move to P2. |
...
| API surface | P1 beta for local/agent automation | The API is split into endpoint groups/DTOs and exposes 35 routes under both `/api/cognitive-memory` and `/api/cognitive-memory/v1`, plus contract metadata/examples under `/contract`. Projection rebuild and recall DTOs expose Qdrant collection/profile/embedding settings. | Stable external-client review and SDK/client generation remain P2. |
...
| Validation | P1 beta core path | Current proof includes focused unit tests, web build, Docker PostgreSQL/Qdrant health, public API ingestion/consolidation/projection/recall, direct Qdrant collection/point inspection, and desktop/mobile browser proof. | Add containerized CI repeatability, richer API contract tests, and broader workflow browser proof before production. |
...
- Missing durable memory records can now be projected explicitly when collection, projection profile, embedding profile, target provider, store kind, and vector dimension settings are provided.
- The composition root registers deterministic local hashing embeddings with Qdrant when `Rag:Qdrant:Enabled`, so local Docker validation does not silently skip vectors because no embedding service is registered.
...
- Vector recall is not automatic for every call. It requires projection collection/profile/embedding profile inputs or configured defaults and a provider with typed filters. When those are supplied, the live Docker Qdrant path is beta-proven.
...
- **UI size risk:** the page is split into child tabs, but the parent still owns several render fragments. Broader browser-facing workflows need proof before production-grade release.
- **Projection provider risk:** rebuild orchestration, adapter-backed proof, deterministic failure proof, audit visibility, and live Docker Qdrant validation exist. Residual risk is CI repeatability and operational recovery beyond the local Docker profile.
...
Current P1 beta Qdrant validation evidence is recorded in `codex/bundles/cognitive-memory-beta-qdrant-validation/reviews/01-execution-report.md`.

Current P1 validation evidence records:

- Docker Qdrant beta proof: public file ingestion 2 chunks, consolidation 2 candidates, projection rebuild 2/2 projected, Qdrant collection `candoitall-knowledge` green with 384-dimensional cosine vectors, public recall vector stage `rag:qdrant:search:2`.
- Focused unit tests after Qdrant patch: 26/26 passed.
- Web project build after Qdrant patch: passed with 0 warnings and 0 errors.
- Browser proof after Qdrant patch: `/cognitive-memory` loaded dashboard and health tab at 1440x1000 and 390x900; console contained only normal Blazor startup/WebSocket info.
