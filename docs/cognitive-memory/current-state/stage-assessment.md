...
The current stage is **memory provider extraction release candidate**.
...
| Runtime ledgers and workers | Release candidate | SB03, SB06, SB09, SB15, and SB33 ledger snapshot proof. | Operational dashboards can be expanded, but core records are observable. |
...
| MAF integration | Release candidate | Generic tool provider, workflow executor, context contributor, SB19, and SB33 MAF tests. | Future MAF APIs must keep memory provider policy typed and explicit. |
...
- Base `appsettings.json` disables deterministic mock, HTTP, and native-remote memory drivers by default.
- MAF memory tools, workflow execution, and context contribution are generic.
- Provider operation dispatch is centralized in the shared memory operation handler.
- Source ingestion uses the existing MAF `MemorySourceSnapshot` contract family.
...
- MCP memory provider registration exists as a package-level extension but is not enabled by base appsettings today.
