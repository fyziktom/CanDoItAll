...
public sealed class GenericProcessStepBriefBuilder : IProcessStepBriefBuilder
{
    ...
    public string Build(ProcessStepBriefBuildRequest request)
    {
        ...
        Browser evidence path rule: when a browser tool accepts `filename` and its output will be cited as process evidence, use `{request.ManagedArtifactRoot}/browser/<evidence-name>.<ext>`. A bare filename is provider-native interaction state and cannot satisfy governed evidence.
    ...
    private static string BuildAcceptanceCriteriaGuidance(ProcessStepBriefBuildRequest request)
    {
        ...
        return acceptanceBranchKeys.Count == 0
            ...
            : $"For these final-acceptance branches only: {string.Join(", ", acceptanceBranchKeys)}. Populate acceptanceCriteriaEvidence in the final structured result for every criterion in ProductAcceptanceCriteriaContract marked kind=ProductAcceptance and required=true. Legacy criterion lines without kind/required markers default to required ProductAcceptance and need the same evidence. Each entry must use the exact criterionId, status Passed, a concise criterion-specific summary, and at least one current-run evidenceRefs entry. {evidenceShapeGuidance} Preserve kind=DeliveryPlanning entries as nonblocking context; do not submit Passed or Failed acceptance evidence for them, and do not route no-go, escalation, or human reconfirmation solely because they lack product proof unless a separate typed decision gate explicitly requires it. Do not select one of those branches when any required ProductAcceptance criterion cannot be recorded this way; select the template's applicable non-acceptance branch instead. {failedCriterionGuidance}";
    }
