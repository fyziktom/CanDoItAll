...
internal sealed partial class ProcessRunAutomationDispatchService
{
        ...
        int attemptNumber)
    {
        ...
        {
            var browserEvidenceRoot = BuildCurrentRunManagedArtifactRoot(candidate);
            ...
            builder.AppendLine($"For canvas, game, custom-control, or keyboard-first surfaces, use `browser_evaluate` with a `.json`, `.txt`, or `.md` filename under `{browserEvidenceRoot}` when ordinary click/fill helpers cannot express the workflow; dispatch representative keyboard or pointer events and inspect visible state, DOM text, or client-side storage.");
            ...
            builder.AppendLine($"Capture fresh bounded browser evidence with durable filenames before you conclude this retry: call `browser_snapshot` with depth 2, boxes disabled, and a `.yml` filename under `{browserEvidenceRoot}` unless the contract requires depth 3 or 4; call `browser_take_screenshot` with a `.png` filename under `{browserEvidenceRoot}` and fullPage false or no fullPage argument; call `browser_console_messages` with a `.log` filename under `{browserEvidenceRoot}`. Do not retry a policy-denied browser call with the same arguments.");
            builder.AppendLine("Browser screenshots, snapshots, console logs, and state outputs must be current-run artifacts. Cite the returned filenames in the durable evidence artifact so the process can import them and expose the output folder through linked project structure.");
