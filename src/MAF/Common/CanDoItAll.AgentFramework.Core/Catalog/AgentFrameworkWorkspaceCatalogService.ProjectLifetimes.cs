...
internal sealed partial class AgentFrameworkWorkspaceCatalogService {
    public Task GrantAgentProjectStructureLifetimeAsync(Guid agentId, AgentProjectStructureLifetime lifetime,
        ...
        return UpdateAgentProjectLifetimeConfigurationAsync(agentId,
            configuration => AgentProjectStructureAccessMetadata.GrantProjectLifetime(configuration, lifetime), cancellationToken);
    }

    public Task RevokeAgentProjectStructureLifetimeAsync(Guid agentId, AgentProjectStructureLifetime lifetime,
        CancellationToken cancellationToken = default) {
        ArgumentNullException.ThrowIfNull(lifetime);
        return UpdateAgentProjectLifetimeConfigurationAsync(agentId, configuration => AgentProjectStructureAccessMetadata.RevokeProjectLifetime(
            configuration, AgentProjectStructureRevocationTarget.ForLifetime(lifetime)).ConfigurationJson, cancellationToken);
    }

    private Task UpdateAgentProjectLifetimeConfigurationAsync(Guid agentId, Func<string, string> update,
        CancellationToken cancellationToken) => UpdateCatalogAsync(catalog => {
        var agent = catalog.Agents.FirstOrDefault(item => item.Id == agentId)
            ?? throw new InvalidOperationException("Agent was not found.");
        var configuration = update(agent.ConfigurationJson);
