    ...
    private sealed partial class ToolCapabilityBuilder
    {
            ...
            bool suppressApprovalRequirements = false)
        {
            ...
            if (attachFileTools && (access.CanReadFiles || access.CanWriteFiles))
            {
                ...
                AddConfiguredToolIfAllowed(tools, "workspace_list_files", () => AIFunctionFactory.Create(workspacePlugin.ListWorkspaceFiles, "workspace_list_files", "Lists files and directories from the managed workspace, a configured external workspace root, or an absolute external path grounded by the current prompt."));
                AddConfiguredToolIfAllowed(tools, "workspace_search", () => AIFunctionFactory.Create(workspacePlugin.SearchWorkspace, "workspace_search", "Searches text across the managed workspace, a configured external workspace root, or an absolute external path grounded by the current prompt."));
                AddConfiguredToolIfAllowed(tools, "workspace_read_file", () => AIFunctionFactory.Create(workspacePlugin.ReadWorkspaceTextFile, "workspace_read_file", "Reads text files from the managed workspace, a configured external workspace root, or an absolute external path grounded by the current prompt."));
                AddConfiguredToolIfAllowed(tools, "workspace_stat_path", () => AIFunctionFactory.Create(workspacePlugin.StatWorkspacePath, "workspace_stat_path", "Returns file or directory metadata for a managed workspace path, configured external workspace root, or prompt-grounded absolute external path."));
                AddConfiguredToolIfAllowed(tools, "workspace_diff_text", () => AIFunctionFactory.Create(workspacePlugin.DiffWorkspaceTextFiles, "workspace_diff_text", "Computes a bounded line-level diff between two allowed workspace text files."));
            ...
            {
                AddConfiguredToolIfAllowed(tools, "workspace_create_directory", () => WrapWithApproval(AIFunctionFactory.Create(workspacePlugin.CreateWorkspaceDirectory, "workspace_create_directory", "Creates a directory in the managed workspace or configured external workspace root."), suppressApprovalRequirements));
                AddConfiguredToolIfAllowed(tools, "workspace_write_file", () => WrapWithApproval(AIFunctionFactory.Create(workspacePlugin.WriteWorkspaceTextFile, "workspace_write_file", "Creates or overwrites a text file in the managed workspace or configured external workspace root."), suppressApprovalRequirements));
                AddConfiguredToolIfAllowed(tools, "workspace_append_file", () => WrapWithApproval(AIFunctionFactory.Create(workspacePlugin.AppendWorkspaceTextFile, "workspace_append_file", "Appends text to a workspace file in the managed workspace or configured external workspace root."), suppressApprovalRequirements));
            }

            if (attachFileTools && access.CanManageWorkspacePaths)
            {
                AddConfiguredToolIfAllowed(tools, "workspace_copy_path", () => WrapWithApproval(AIFunctionFactory.Create(workspacePlugin.CopyWorkspacePath, "workspace_copy_path", "Copies a file or directory inside allowed workspace roots."), suppressApprovalRequirements));
                AddConfiguredToolIfAllowed(tools, "workspace_move_path", () => WrapWithApproval(AIFunctionFactory.Create(workspacePlugin.MoveWorkspacePath, "workspace_move_path", "Moves or renames a file or directory inside allowed workspace roots."), suppressApprovalRequirements));
                AddConfiguredToolIfAllowed(tools, "workspace_delete_path", () => WrapWithApproval(AIFunctionFactory.Create(workspacePlugin.DeleteWorkspacePath, "workspace_delete_path", "Deletes a file or directory inside allowed workspace roots."), suppressApprovalRequirements));
            ...
            {
                AddConfiguredToolIfAllowed(tools, "workspace_convert_document", () => WrapWithApproval(AIFunctionFactory.Create(workspacePlugin.ConvertDocumentToMarkdown, "workspace_convert_document", "Converts a workspace document such as a PDF to markdown using markitdown. Do not use for image assets; use workspace_inspect_image or workspace_analyze_image."), suppressApprovalRequirements));
        ...
        private IReadOnlyList<AITool> CreateWorkspacePluginTools(bool suppressApprovalRequirements)
        {
            ...
            AddWorkspacePluginTool(tools, "workspace_convert_document", () => WrapWithApproval(AIFunctionFactory.Create(workspacePlugin.ConvertDocumentToMarkdown, "workspace_convert_document", "Converts a workspace document such as a PDF to markdown using markitdown. Do not use for image assets; use workspace_inspect_image or workspace_analyze_image."), suppressApprovalRequirements));
