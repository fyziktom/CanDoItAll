    ...
    RuntimeCapabilityAccessPlan capabilityAccessPlan)
{
            ...
            bool suppressApprovalRequirements = false)
        {
            ...
            {
                AddConfiguredToolIfAllowed(tools, "workspace_dotnet_new", () => WrapWithApproval(AIFunctionFactory.Create(workspacePlugin.DotnetWorkspaceNew, "workspace_dotnet_new", "Creates a bounded dotnet project or solution in the managed workspace or configured external workspace root."), suppressApprovalRequirements));
            }
        ...
        public IReadOnlyList<AITool> CreateWorkspacePluginTools(bool suppressApprovalRequirements)
        {
            ...
            AddWorkspacePluginTool(tools, "workspace_create_directory", () => WrapWithApproval(AIFunctionFactory.Create(filesystemPlugin.CreateWorkspaceDirectory, "workspace_create_directory", "Creates a directory in the managed workspace or a grounded external-target alias. In external-target process runs, keep product material under a grounded external-target alias and evidence under current-run artifact folders."), suppressApprovalRequirements));
            AddWorkspacePluginTool(tools, "workspace_write_file", () => WrapWithApproval(AIFunctionFactory.Create(filesystemPlugin.WriteWorkspaceTextFile, "workspace_write_file", "Creates or overwrites a text file in the managed workspace or a grounded external-target alias. In external-target process runs, write product material only under a grounded external-target alias and evidence only under current-run artifact folders."), suppressApprovalRequirements));
            AddWorkspacePluginTool(tools, "workspace_append_file", () => WrapWithApproval(AIFunctionFactory.Create(filesystemPlugin.AppendWorkspaceTextFile, "workspace_append_file", "Appends text to a workspace file. In external-target process runs, write product material only under a grounded external-target alias and evidence only under current-run artifact folders."), suppressApprovalRequirements));
            ...
            AddWorkspacePluginTool(tools, "workspace_dotnet_new", () => WrapWithApproval(AIFunctionFactory.Create(workspacePlugin.DotnetWorkspaceNew, "workspace_dotnet_new", "Creates a bounded dotnet project or solution in the managed workspace or a grounded external-target alias. Use an approved SDK template, inspect an unsuccessful result before retrying, and pass an allowed workspace parentDirectory. Timeout arguments are seconds, not milliseconds; prefer the default or use normal second values such as 120 or 300. In an external-target process run, do not substitute an ungrounded external path or unrelated managed path for the product target."), suppressApprovalRequirements));
