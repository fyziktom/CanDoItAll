...
internal static class ProcessStepContractPromptBuilder
{
        ...
        else
        {
            foreach (var artifact in stepContract.RequiredArtifacts)
            {
                ...
                builder
                    ...
                    .Append(FormatPayloadSchema(descriptor))
                    .Append("; availability ")
    ...
    private static string FormatPayloadSchema(ProcessArtifactSlotDescriptor? descriptor)
        => string.IsNullOrWhiteSpace(descriptor?.PayloadSchema)
            ? "unspecified"
        ...
        ProcessSubprocessChildOutputContract output)
    {
        ...
        {
            builder
                .Append("; parent branch route ")
                .Append(output.ParentBranchOutcomeKey);
        }
