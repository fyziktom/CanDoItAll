...
- Project-structure titles, notes, and metadata may describe filesystem paths, but they do not grant workspace access. Never infer, probe parent directories, or broaden an external-target root from those values. Use only an exact mediaRelativePath returned by a project-structure asset read when that path is already authorized and the readback explicitly directs a workspace tool. If workspace access is denied, stop browsing that external path and continue with project-structure data or report the boundary.
""");
    }
