...
internal interface IProviderRuntimeProfileSnapshotLoader
{
...
internal sealed record CanonicalProviderRuntimeProfile(
    ...
    IProviderRuntimeProfileSnapshotLoader
{
        ...
        LoadAllAsync(CancellationToken cancellationToken = default)
    {
        ...
        return providers
            .Select(Map)
            .Append(new CanonicalProviderRuntimeProfile(
        ...
        CancellationToken cancellationToken = default)
    {
        await using var dbContext =
            await dbContextFactory.CreateDbContextAsync(cancellationToken);
        var revision = await dbContext.Set<WorkspaceProviderProfile>()
            .AsNoTracking()
            .Where(item => item.Id == providerId)
            .Select(item => (Guid?)item.ConcurrencyToken)
            .SingleOrDefaultAsync(cancellationToken);
        return revision.HasValue
            ? new ProviderConfigurationRevision(revision.Value)
            : null;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        await using var dbContext =
            await dbContextFactory.CreateDbContextAsync(cancellationToken);
        var revisions = await dbContext.Set<WorkspaceProviderProfile>()
            .AsNoTracking()
            .Where(item =>
                item.Id != WorkspaceAgentProviderProfileMapper
                    .RuntimeFallbackOllamaProviderId)
        ...
        return revisions.ToDictionary(
            item => item.Id,
            item => new ProviderConfigurationRevision(
                item.ConcurrencyToken));
    }
