...
public static class PromptsCuratorAgentRuntimeAuthorizationPolicy
{
    ...
    {
        return PromptsCuratorAgentIdentity.Matches(agent) &&
        ...
        string toolName)
    {
        ...
        if (!PromptsCuratorAgentCapabilityKeys.ToolNameToCapabilityKey.TryGetValue(toolName, out var capabilityKey))
        {
            throw new InvalidOperationException(
                $"Prompts Curator runtime tool '{toolName}' does not have a capability key mapping.");
        }
...
public sealed class PromptsCuratorAgentRuntimeAuthorizationService(
    IAgentFrameworkWorkspaceService workspaceService)
{
    ...
    {
        if (actorAgentId != PromptsCuratorAgentIdentity.AgentId)
        {
        ...
        if (!PromptsCuratorAgentRuntimeAuthorizationPolicy.IsManagedCuratorActor(actor))
        {
            throw CreateDeniedException(toolName);
        }
    ...
    private static UnauthorizedAccessException CreateDeniedException(string toolName)
    {
        return new UnauthorizedAccessException(
            $"The current managed Prompts Curator agent is not authorized to invoke '{toolName}'.");
    }
}
