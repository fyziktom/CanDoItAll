(function() {
  ...
  function drawTimedGrid(ctx, options) {
    ...
    settings.days.forEach(function(day, index) {
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(asText(day.label), columnX + (dayWidth / 2), settings.y + 13);
      ctx.font = '600 11px "DM Sans", sans-serif';
      ctx.fillStyle = colors.axisText;
      ctx.fillText(asText(day.subLabel), columnX + (dayWidth / 2), settings.y + 28);
      ctx.restore();
    ...
    for (var hour = settings.startHour; hour <= settings.endHour; hour += 1) {
      ...
      if (hour < settings.endHour) {
        ...
        ctx.textAlign = 'right';
        ctx.textBaseline = 'middle';
        ctx.fillText(labelHour + ' ' + suffix, settings.x + leftAxisWidth - 10, lineY + 2);
        ctx.restore();
      }
    }
