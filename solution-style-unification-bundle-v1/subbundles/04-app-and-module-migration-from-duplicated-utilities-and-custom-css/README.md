...
- Migrate remaining high-value app and module surfaces from repeated raw utility strings and safe custom CSS to the shared Tailwind classes and BaseLib primitives, while recording measurable duplication reduction, preserving behavior, and decomposing overly long page files into logical Razor components.

## Covered Inputs

- `REQ-01`, `REQ-03`, `REQ-04`, `REQ-12`, `REQ-13`, `REQ-16`, `REQ-17`, `REQ-18`
- Raw prompt steps `5` and `6`
- Follow-up feedback: `Home.razor` still contains raw `button` and `span` action markup.
- Follow-up feedback: `ProjectsPage.razor` still contains excessive raw markup, poor indentation, and needs logical component extraction.
- Follow-up feedback: `PromptFactoryPage.razor` still contains excessive raw markup and needs logical page-level component extraction without changing CanvasLib internals or drawing behavior.
...
3. Extract logical Razor components from the remaining overly long route files instead of leaving the new shared styles trapped inside page-local markup walls.
4. Review custom CSS hotspot files and remove or convert only the rules that are safe to absorb into the shared system.
5. Record progress metrics after the main migration wave.
6. Browser-validate the migrated route matrix and repair regressions immediately.
...
- Do not edit `CanDoItAll.Components.CanvasLib` internals or change drawing behavior on the prompt-factory canvas surface.
...
- The reopened long-page hotspots are split into logical Razor components instead of remaining as monolithic route files.
...
- `Home.razor`: page-level raw `button` / `span` action markup removed.
- `ProjectsPage.razor`: page split into focused components and materially shortened.
- `PromptFactoryPage.razor`: shared action rows and several page-level components were extracted, but the page still retains significant raw non-canvas layout markup and remains only partially closed.

## Proof Required

- `npm run build` from `C:\repositories\CanDoItAll\Tailwind`
- `dotnet build C:\repositories\CanDoItAll\CanDoItAll.slnx`
- Playwright screenshots for `/dashboard`, `/projects`, `/prompt-factory`, `/resources`, `/prompts`, `/validation`, `/activity`, and `/settings`
- Updated metrics in the execution report and workbook refresh if the census materially changes

## Browser Validation Logging

- Target routes: `/dashboard`, `/projects`, `/prompt-factory`, `/resources`, `/prompts`, `/validation`, `/activity`, `/settings`
