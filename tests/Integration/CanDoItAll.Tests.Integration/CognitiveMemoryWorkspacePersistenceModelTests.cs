...
public sealed class CognitiveMemoryWorkspacePersistenceModelTests
{
    ...
    private static async Task<WorkspaceFixture> CreateFixtureAsync()
    {
        ...
        var database = PostgresTestDatabaseLease.Create("cognitivememoryworkspacepersistencemodeltests");

        var options = database.CreateAppDbContextOptions();
        var dbContext = new AppDbContext(options);
        await dbContext.Database.EnsureCreatedAsync();
        return new WorkspaceFixture(database, new TestDbContextFactory(options), dbContext, new FixedClock());
    }
