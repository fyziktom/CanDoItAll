...
public sealed class AgentEditorAdapterIntegrationTests {
    ...
    public async Task Registered_commands_round_trip_settings_and_preserve_optimistic_version() {
        ...
        var projectDraft = await projects.GetAsync(null);
        projectDraft.Name = "Agent editor round trip";
        var projectSave = await projects.SaveAsync(projectDraft);
        Assert.True(projectSave.IsSuccess);
        var projectId = projectSave.Value;
