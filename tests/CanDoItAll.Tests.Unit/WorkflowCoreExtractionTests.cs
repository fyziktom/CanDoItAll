...
public sealed class WorkflowCoreExtractionTests
{
    ...
    public void WorkflowCoreImplementationFilesMovedOutOfAgentFrameworkCoreProject()
    {
        ...
        {
            Assert.False(
                File.Exists(Path.Combine(root, "src", "CanDoItAll.AgentFramework.Core", "Workflows", movedFile)),
                $"{movedFile} must not remain in AgentFramework.Core.");
            Assert.True(
                File.Exists(Path.Combine(root, "src", "CanDoItAll.AgentFramework.Workflows.Core", movedFile)),
