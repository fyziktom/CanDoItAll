...
public sealed class ProcessAgentExecutionBoundaryArchitectureTests
{
    ...
    [Fact]
    public void Process_architecture_tests_do_not_carry_historical_fixture_quarantine_skips()
    {
        var source = ReadRepositoryFile(
            "tests",
            "CanDoItAll.Tests.Unit",
            "ProcessAgentExecutionBoundaryArchitectureTests.cs");
        var skipMarker = "[Fact(" + "Skip";
        var quarantineMarker = "HistoricalBundleFixture" + "QuarantineReason";

        Assert.DoesNotContain(skipMarker, source, StringComparison.Ordinal);
        Assert.DoesNotContain(quarantineMarker, source, StringComparison.Ordinal);
    }
