...
public sealed class ProviderEditorSubmissionTests {
    [Fact]
    public void Submission_preserves_every_public_property_and_owns_mutable_descendants() {
        var source = (ProviderProfileEditorModel)Sentinel(typeof(ProviderProfileEditorModel), null);
        var submission = ProviderEditorSubmission.Capture(source);
        var request = submission.CreateRequest();
        Assert.Equal(JsonSerializer.Serialize(source), JsonSerializer.Serialize(request));
        AssertIndependent(source, request);
        var original = JsonSerializer.Serialize(request);
        source.Name = "Changed original";
        source.ModelPrices.Clear();
        request.Name = "Changed returned copy";
        request.ModelThinkingEffortCapabilities!.Clear();
        Assert.Equal(original, JsonSerializer.Serialize(submission.CreateRequest()));
    }

    [Fact]
    public void Reconciliation_preserves_later_edits_and_updates_only_identity_and_token() {
        var draft = new ProviderProfileEditorModel { Name = "Submitted", DefaultModel = "model" };
        var submission = ProviderEditorSubmission.Capture(draft);
        draft.Name = "Later";
        draft.ModelPrices.Add(new() { Model = "later", InputPerMillionTokensUsd = 12 });
        var id = Guid.NewGuid();
        var token = Guid.NewGuid();
        submission.Reconcile(draft, new() { Id = id, ExpectedConcurrencyToken = token, Name = "Canonical" });
        Assert.Equal("Later", draft.Name);
        Assert.Single(draft.ModelPrices);
        Assert.Equal(id, draft.Id);
        Assert.Equal(token, draft.ExpectedConcurrencyToken);
    }
