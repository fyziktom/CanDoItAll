...
public sealed class SchedulerWorkspaceDbContextTests {
    ...
    public void Scheduler_model_contains_only_plans_and_runs_with_the_canonical_cascade_and_legacy_columns() {
        ...
        Assert.Equal(new[] { typeof(SchedulerPlan), typeof(SchedulerPlanRun), typeof(SchedulerFireAdmissionRecord) }.OrderBy(type => type.Name),
        ...
        Assert.Empty(context.Model.FindEntityType(typeof(SchedulerFireAdmissionRecord))!.GetForeignKeys());
