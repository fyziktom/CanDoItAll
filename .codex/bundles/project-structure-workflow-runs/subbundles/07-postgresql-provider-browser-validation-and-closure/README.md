...
- Provider proof: `proof/providers/provider-validation-results.json` records `gpt-5-mini` and local Ollama `gptoss20b64k:latest` provider chat probes and workflow runs; `proof/providers/provider-validation-results-rerun.json` records the stronger API rerun where OpenAI reconciled the real Mouser XLS/PDF with invoice/part/price/total validation and local Ollama summarized the real SEAMARK folder through `source.ingest`, created project-structure markdown assets, and passed strict device/price mapping checks.
- Repairs made during closure: the add-workflow input preview no longer truncates preview rows, add-dialog async refreshes are versioned so older refreshes cannot overwrite newer source/manual input edits, SEAMARK workflow instructions now explicitly prevent X-6600/X-6600A price swaps, and provider proof validation now checks the device/price mapping near the produced table rows.
