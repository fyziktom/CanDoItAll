...
| R27 | Real scenarios must prove multi-step payload transfer across executor nodes, LLM calls, and downstream executors. | PostgreSQL proof includes executor -> `gpt-5-mini` -> executor scenarios that read project structure or spreadsheets, transform the payload, and save the LLM output back to project structure or workspace storage. |
