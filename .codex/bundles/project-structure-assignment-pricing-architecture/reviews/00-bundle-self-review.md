...
- Agent price means historical provider/runtime spend mapped from the selected CRM AI party to its technical agent. It is not silently combined with a CRM workforce rate.
- Expected task cost remains one selected resource basis rather than a multi-resource additive total; additive/cross-currency costing requires a separate model.
- Legacy task execution state is `Unknown`; users or trusted runtime data must explicitly reconcile it before automatic repricing.
- Missing authoritative price on an explicitly `NotStarted` task clears amount and currency and returns typed unavailable feedback.

## Final Decision

`Completed — the prepared and completed canonical validators pass, and the independent C# architecture gate passed with non-blocking follow-up.`
