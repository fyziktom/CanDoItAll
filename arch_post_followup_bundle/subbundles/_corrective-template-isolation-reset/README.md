# Corrective playbook - template isolation reset
...
`	ext
Execute only corrective subbundle _corrective-template-isolation-reset. Repair the specific blocking defect, rerun the failed proof on fresh artifacts, update the live execution report and gate memo, and do not unblock downstream work until the gate passes.
`

## Preserved Bundle Notes

- Repair any remaining duplicated template mapping rules or unsafe pack caching/thread-safety assumptions.
- Rerun the template isolation proof and Gate C review before reopening final cleanup work.
