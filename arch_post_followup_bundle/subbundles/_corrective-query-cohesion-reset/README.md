# Corrective playbook - query cohesion reset
...
`	ext
Execute only corrective subbundle _corrective-query-cohesion-reset. Repair the specific blocking defect, rerun the failed proof on fresh artifacts, update the live execution report and gate memo, and do not unblock downstream work until the gate passes.
`

## Preserved Bundle Notes

- Repair any workspace or run-details read path that still stitches user-visible state from weakly related snapshots.
- Rerun the query cohesion proof and Gate C review before reopening downstream cleanup work.
