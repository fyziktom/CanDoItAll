# Corrective playbook - workspace quiescence reset
...
`	ext
Execute only corrective subbundle _corrective-workspace-quiescence-reset. Repair the specific blocking defect, rerun the failed proof on fresh artifacts, update the live execution report and gate memo, and do not unblock downstream work until the gate passes.
`

## Preserved Bundle Notes

- Repair any workspace action that still bypasses pending-persistence quiescence or permits stale publish/delete/export behavior.
- Rerun the workspace ordering tests and Gate B review before reopening downstream work.
