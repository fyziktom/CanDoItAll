...
def main() -> int:
    ...
    required_files = [
        "README.md",
        "analysis/01-phase14-hidden-gap-summary.md",
        "requirements/bundle14-scope.md",
        "plan/01-phase14-execution-plan.md",
        ...
        "reviews/02-phase14-failure-map.md",
        "reviews/03-validation-method.md",
        "scripts/gate_check_phase14.py",
    ]
    subbundles = [
    ...
    phase_plan = (root / "plan/01-phase14-execution-plan.md").read_text(encoding="utf-8")
    for required_phrase in [
        ...
        if required_phrase.lower() not in phase_plan.lower():
            print("Bundle validation FAILED.")
            print(f"- plan/01-phase14-execution-plan.md is missing required execution metadata: {required_phrase}")
            return 1
