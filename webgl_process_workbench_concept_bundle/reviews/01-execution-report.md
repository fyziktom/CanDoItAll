...
- `Completed after corrective reopen`
...
| `08-architecture-review-gate-b` | `Passed on 2026-04-20` | `Passed on 2026-04-20; reread after corrective` | `Interactive readability, sandbox isolation, and proof-worthiness review completed` | `Passed` | Gate B passed on the initial sandbox proof and was later reread after `_corrective-scene-contract-and-layout-reset` refreshed the scene readability evidence. |
...
| `_corrective-scene-contract-and-layout-reset` | `Passed on 2026-04-20 after post-proof reopen` | `Passed on 2026-04-20` | `Gate B reread, dense-scene browser proof refreshed, label clamp and collision-safe drag validated, and representative template pack expanded` | `Completed` | The corrective pass strengthened primary-path emphasis, added recomposition plus spacing controls, fixed label scaling under unzoom, added more complex process scenes, and closed collision protection on fresh proof. |
...
| `_corrective-scene-contract-and-layout-reset` | `/webgl/process-workbench?template=branching-code-review` | `1900x1200` | `Live Playwright MCP proof on the running sandbox: primary-path emphasis range `1.58-1.92` vs role-link `0.74-0.96`, alternating-arc recomposition, `1.3x` spacing, label clamp `0.68 -> 0.46`, and collision-safe drag without overlap` | `output/playwright/webgl-sandbox/02-webgl-dense-template.png`, `output/playwright/webgl-sandbox/06-webgl-3d-navigation-overlay.png`, `output/playwright/webgl-sandbox/07-webgl-alternating-arc-clamp.png` | `Passed` |
...
- Post-closure feedback reopened scene proof once more; the corrective pass kept the central route but made primary-path edges materially stronger than role links (`1.58-1.92` emphasis vs `0.74-0.96`).
- The label-overlay rule is now usable in real depth views: the tracked dense-scene step started at `0.68` scale and shrank only to the stable floor `0.46` during unzoom instead of pinning immediately at the minimum.
- Recomposition and spacing controls are now first-class proof surfaces: `center-lane`, `alternating-arc`, and `layered-orbit` are available, and the live browser proof confirmed spacing expansion from `1.0x` to `1.3x`.
- The representative pack is no longer limited to three scenes; dense and complex process templates now cover software delivery, AI-assisted change delivery, and release readiness/deployment in addition to the original simple/medium/dense trio.
- Collision-safe dragging is now explicit proof, not an assumption: the rerun browser proof found a valid probe move, advanced it without overlap, and left the target node unobstructed.
...
- `python codex\\skills\\bundles\\candoitall-bundle-preparation\\scripts\\validate_bundle.py webgl_process_workbench_concept_bundle --stage completed`
...
| `IN-12` | `Completed` | Fresh screenshots were captured for default, dense, semantic-proof, desktop-review, mobile-review, perspective-navigation, and corrective alternating-arc clamp states, including `output/playwright/webgl-sandbox/06-webgl-3d-navigation-overlay.png` and `output/playwright/webgl-sandbox/07-webgl-alternating-arc-clamp.png`. |
...
| `IN-16` | `Completed` | Corrective playbooks stayed available and `_corrective-scene-contract-and-layout-reset` was reopened and closed on fresh proof when post-closure feedback showed the scene still needed stronger 3D readability safeguards. |
| `IN-17` | `Completed` | Current process/canvas identifiers, ports, and categories were reused through `ProcessWebGlSceneAdapter`. |
| `IN-18` | `Completed` | The representative simple, medium, and dense core set remained `customer-onboarding`, `architecture-decision-governance`, and `branching-code-review`, and the corrective pass expanded the pack with `software-delivery`, `ai-assisted-change-delivery`, and `release-readiness-and-deployment` for denser proof. |
